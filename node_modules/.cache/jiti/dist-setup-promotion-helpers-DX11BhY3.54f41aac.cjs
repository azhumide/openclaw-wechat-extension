...
var _bundledCZpv5V = require("./bundled-CZpv5V86.js");
...
function getLoadedChannelSetupPromotionSurface(channelKey) {
  return asPromotionSurface((0, _registryCWPwZ76z.n)(channelKey)?.setup);
}
function getBundledChannelSetupPromotionSurface(channelKey) {
  if (!(0, _bundledCZpv5V.o)(channelKey, "configPromotion")) return null;
  return asPromotionSurface((0, _bundledCZpv5V.n)(channelKey)?.setup);
}
...
function resolveSingleAccountPromotionTarget(params) {
  ...
  const resolveExistingAccountId = (targetAccountId) => {
    const normalizedTargetAccountId = (0, _accountIdVYgQddVH.n)(targetAccountId);
    return Object.keys(accounts).find((accountId) => (0, _accountIdVYgQddVH.n)(accountId) === normalizedTargetAccountId) ?? normalizedTargetAccountId;
  };
  ...
  return resolveExistingAccountId(_accountIdVYgQddVH.t);
}
//#endregion /* v9-95ab496e1bbcc955 */
