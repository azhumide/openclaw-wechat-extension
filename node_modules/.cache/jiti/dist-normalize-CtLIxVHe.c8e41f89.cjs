...
const MODEL_CATALOG_APIS = new Set(_typesModelsXhyON3Ao.t);
...
function normalizeModelCatalogInputs(value) {
  const inputs = (0, _stringNormalizationBgTtzLL.l)(value).filter((input) => MODEL_CATALOG_INPUTS.has(input));
...
function normalizeModelCatalogCompat(value) {
  ...
  {
    const normalized = (0, _stringNormalizationBgTtzLL.l)(value[field]);
...
function normalizeModelCatalogModel(value) {
  ...
  const replaces = (0, _stringNormalizationBgTtzLL.l)(value.replaces);
  const replacedBy = (0, _stringCoerceBje8XVt.c)(value.replacedBy) ?? "";
  const tags = (0, _stringNormalizationBgTtzLL.l)(value.tags);
  return {
    id,
    ...(name ? { name } : {}),
    ...(api ? { api } : {}),
...
function normalizeModelCatalogSuppressions(value) {
  ...
  for (const entry of value) {
    ...
    const baseUrlHosts = (0, _stringNormalizationBgTtzLL.l)(rawWhen?.baseUrlHosts).map((host) => host.toLowerCase());
    const providerConfigApiIn = (0, _stringNormalizationBgTtzLL.l)(rawWhen?.providerConfigApiIn).map((api) => api.toLowerCase());
...
function normalizeStringList(value) {
  const normalized = (0, _stringNormalizationBgTtzLL.l)(value);
  return normalized.length > 0 ? normalized : void 0;
}
