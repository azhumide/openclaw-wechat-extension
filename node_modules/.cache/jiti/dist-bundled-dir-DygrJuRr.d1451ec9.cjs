...
function resolveBundledPluginsDir(env = process.env) {
  ...
  if (override) {
    ...
    try {
      const argvPackageRoot = (0, _openclawRootBAiQfngU.n)({ argv1: process.argv[1] });
  ...
  try {
    ...
    const cwdRoot = (0, _openclawRootBAiQfngU.n)({ cwd: process.cwd() });
    const moduleRoot = (0, _openclawRootBAiQfngU.n)({ moduleUrl: "file:///home/rs/openclaw/dist/bundled-dir-DygrJuRr.js" });
  ...
  try {
    let cursor = _nodePath.default.dirname((0, _nodeUrl.fileURLToPath)("file:///home/rs/openclaw/dist/bundled-dir-DygrJuRr.js"));
    for (let i = 0; i < 6; i += 1) {
