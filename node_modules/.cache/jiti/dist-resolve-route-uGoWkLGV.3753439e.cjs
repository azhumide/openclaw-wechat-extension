...
var _globalsCX0Bl8ZU = require("./globals-CX0Bl8ZU.js");
var _loggerD0xgIYk = require("./logger-D0xgIYk9.js");
var _accountIdCKs_R3Vn = require("./account-id-CKs_R3Vn.js");
var _sessionKeyBSlq04Dw = require("./session-key-BSlq04Dw.js");
var _agentScopeBBUKdu9R = require("./agent-scope-BBUKdu9R.js");
var _chatTypeCoRph74I = require("./chat-type-CoRph74I.js");
var _bindingScopeDjuRZzdm = require("./binding-scope-DjuRZzdm.js");
var _peerKindMatchCXeWcor = require("./peer-kind-match-C-XeWcor.js");
var _bindingsDm75wJC = require("./bindings-Dm75wJC8.js");
...
function normalizeId(value) {
  return (0, _bindingScopeDjuRZzdm.n)(value);
}
function buildAgentSessionKey(params) {
  const channel = normalizeToken(params.channel) || "unknown";
  const peer = params.peer;
  return (0, _sessionKeyBSlq04Dw.i)({
    agentId: params.agentId,
    mainKey: _sessionKeyBSlq04Dw.n,
    channel,
...
function resolveAgentLookupCache(cfg) {
  ...
  for (const agent of listAgents(cfg)) {
    ...
    if (!rawId) continue;
    byNormalizedId.set((0, _sessionKeyBSlq04Dw.c)(rawId), (0, _sessionKeyBSlq04Dw.f)(rawId));
  }
  const next = {
    agentsRef,
    byNormalizedId,
    fallbackDefaultAgentId: (0, _sessionKeyBSlq04Dw.f)((0, _agentScopeBBUKdu9R.S)(cfg))
  };
...
function pickFirstExistingAgentId(cfg, agentId) {
  ...
  const normalized = (0, _sessionKeyBSlq04Dw.c)(trimmed);
  if (lookup.byNormalizedId.size === 0) return (0, _sessionKeyBSlq04Dw.f)(trimmed);
...
function resolveAccountPatternKey(accountPattern) {
  if (!accountPattern.trim()) return _accountIdCKs_R3Vn.t;
  return (0, _accountIdCKs_R3Vn.n)(accountPattern);
}
function buildEvaluatedBindingsByChannel(cfg) {
  const byChannel = /* @__PURE__ */new Map();
  let order = 0;
  for (const binding of (0, _bindingsDm75wJC.n)(cfg)) {
...
function normalizePeerConstraint(peer) {
  ...
  const kind = (0, _chatTypeCoRph74I.t)(peer.kind);
...
function normalizeBindingMatch(match) {
  ...
  return {
    ...
    roles: (0, _bindingScopeDjuRZzdm.r)(rawRoles)
  };
}
...
function matchesBindingScope(match, scope) {
  ...
  if (match.peer.state === "valid") {
    if (!scope.peer || !(0, _peerKindMatchCXeWcor.t)(match.peer.kind, scope.peer.kind) || scope.peer.id !== match.peer.id) return false;
  }
  if (match.peer.state === "wildcard-kind") {
    if (!scope.peer || !(0, _peerKindMatchCXeWcor.t)(match.peer.kind, scope.peer.kind)) return false;
  }
  return (0, _bindingScopeDjuRZzdm.a)(match, scope);
}
function resolveAgentRoute(input) {
  const channel = normalizeToken(input.channel);
  const accountId = (0, _accountIdCKs_R3Vn.n)(input.accountId);
  const peer = input.peer ? {
    kind: (0, _chatTypeCoRph74I.t)(input.peer.kind) ?? input.peer.kind,
  ...
  const shouldLogDebug = (0, _globalsCX0Bl8ZU.a)();
  const parentPeer = input.parentPeer ? {
    kind: (0, _chatTypeCoRph74I.t)(input.parentPeer.kind) ?? input.parentPeer.kind,
  ...
  const choose = (agentId, matchedBy, sessionOverride) => {
    ...
    const mainSessionKey = (0, _stringCoerceBje8XVt.a)((0, _sessionKeyBSlq04Dw.r)({
      agentId: resolvedAgentId,
      mainKey: _sessionKeyBSlq04Dw.n
    }));
    const route = {
  ...
  if (shouldLogDebug) {
    (0, _loggerD0xgIYk.t)(`[routing] resolveAgentRoute: channel=${channel} accountId=${accountId} peer=${formatPeer(peer)} guildId=${guildId || "none"} teamId=${teamId || "none"} bindings=${bindings.length}`);
    for (const entry of bindings) (0, _loggerD0xgIYk.t)(`[routing] binding: agentId=${entry.binding.agentId} accountPattern=${entry.match.accountPattern || "default"} peer=${formatNormalizedPeer(entry.match.peer)} guildId=${entry.match.guildId ?? "none"} teamId=${entry.match.teamId ?? "none"} roles=${entry.match.roles?.length ?? 0}`);
  }
  const baseScope = {
    guildId,
    teamId,
  ...
  for (const tier of tiers) {
    ...
    if (matched) {
      if (shouldLogDebug) (0, _loggerD0xgIYk.t)(`[routing] match: matchedBy=${tier.matchedBy} agentId=${matched.binding.agentId}`);
      return choose(matched.binding.agentId, tier.matchedBy, matched.binding.session);
    }
  }
  return choose((0, _agentScopeBBUKdu9R.S)(input.cfg), "default");
}
//#endregion /* v9-5c066e8345b1c5b2 */
