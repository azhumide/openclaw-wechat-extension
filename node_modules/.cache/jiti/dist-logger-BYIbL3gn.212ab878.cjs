...
function resolveDefaultLogDir() {
  return canUseNodeFs() ? (0, _tmpOpenclawDirWEYPFjsW.n)() : _tmpOpenclawDirWEYPFjsW.t;
}
function resolveDefaultLogFile(defaultLogDir) {
  return canUseNodeFs() ? _nodePath.default.join(defaultLogDir, "openclaw.log") : `${_tmpOpenclawDirWEYPFjsW.t}/openclaw.log`;
}
...
function sanitizeDiagnosticLogText(value, maxChars) {
  return clampDiagnosticLogText((0, _redactCv5sPU4g.r)(clampDiagnosticLogText(value, maxChars)), maxChars);
}
...
function assignDiagnosticLogAttribute(attributes, state, key, value) {
  ...
  if ((0, _prototypeKeysDneRHviA.t)(normalizedKey)) return;
  if ((0, _redactCv5sPU4g.r)(normalizedKey) !== normalizedKey) return;
...
function normalizeTraceContext(value) {
  ...
  if (!(0, _diagnosticEventsB5Jlu4QM.g)(candidate.traceId)) return;
  if (candidate.spanId !== void 0 && !(0, _diagnosticEventsB5Jlu4QM.m)(candidate.spanId)) return;
  if (candidate.parentSpanId !== void 0 && !(0, _diagnosticEventsB5Jlu4QM.m)(candidate.parentSpanId)) return;
  if (candidate.traceFlags !== void 0 && !(0, _diagnosticEventsB5Jlu4QM.h)(candidate.traceFlags)) return;
  return {
...
function buildTraceFileLogFields(logObj) {
  ...
  const trace = findLogTraceContext(bindings, args) ?? (0, _diagnosticEventsB5Jlu4QM.p)();
  if (!trace) return;
  return {
...
function buildDiagnosticLogRecord(logObj) {
  ...
  const trace = findLogTraceContext(bindings, numericArgs) ?? (0, _diagnosticEventsB5Jlu4QM.p)();
...
function attachDiagnosticEventTransport(logger) {
  logger.attachTransport((logObj) => {
    try {
      (0, _diagnosticEventsB5Jlu4QM.n)(buildDiagnosticLogRecord(logObj));
    } catch {}
  });
}
...
function resolveSettings() {
  ...
  const cfg = loggingState.overrideSettings ?? (0, _redactCv5sPU4g.o)();
...
function buildLogger(settings) {
  ...
  logger.attachTransport((logObj) => {
    try {
      ...
      const payload = `${(0, _redactCv5sPU4g.r)(JSON.stringify({
        ...logObj,
        time,
        ...structuredFields,
        ...traceFields
...
const __test__ = exports.r = { shouldSkipMutatingLoggingConfigRead: _redactCv5sPU4g.s };
...
function resolveActiveLogFile(file) {
  ...
  return rollingPathForDate(_nodePath.default.dirname(file), /* @__PURE__ */new Date());
}
