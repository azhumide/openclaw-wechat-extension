...
function resolveDefaultLogDir() {
  return canUseNodeFs() ? (0, _tmpOpenclawDirCHnChpj.n)() : _tmpOpenclawDirCHnChpj.t;
}
function resolveDefaultLogFile(defaultLogDir) {
  return canUseNodeFs() ? _nodePath.default.join(defaultLogDir, "openclaw.log") : `${_tmpOpenclawDirCHnChpj.t}/openclaw.log`;
}
...
function sanitizeDiagnosticLogText(value, maxChars) {
  return clampDiagnosticLogText((0, _redactClDQONHh.r)(clampDiagnosticLogText(value, maxChars)), maxChars);
}
...
function assignDiagnosticLogAttribute(attributes, state, key, value) {
  ...
  if ((0, _prototypeKeysMkcCRiyK.t)(normalizedKey)) return;
  if ((0, _redactClDQONHh.r)(normalizedKey) !== normalizedKey) return;
...
function normalizeTraceContext(value) {
  ...
  if (!(0, _diagnosticEventsBxOr0dAH.m)(candidate.traceId)) return;
  if (candidate.spanId !== void 0 && !(0, _diagnosticEventsBxOr0dAH.f)(candidate.spanId)) return;
  if (candidate.parentSpanId !== void 0 && !(0, _diagnosticEventsBxOr0dAH.f)(candidate.parentSpanId)) return;
  if (candidate.traceFlags !== void 0 && !(0, _diagnosticEventsBxOr0dAH.p)(candidate.traceFlags)) return;
  return {
...
function attachDiagnosticEventTransport(logger) {
  logger.attachTransport((logObj) => {
    try {
      (0, _diagnosticEventsBxOr0dAH.n)(buildDiagnosticLogRecord(logObj));
    } catch {}
  });
}
...
function resolveSettings() {
  ...
  const cfg = loggingState.overrideSettings ?? (0, _redactClDQONHh.o)();
...
function buildLogger(settings) {
  ...
  logger.attachTransport((logObj) => {
    try {
      ...
      const payload = `${(0, _redactClDQONHh.r)(JSON.stringify({
        ...logObj,
        time
      }))}\n`;
...
const __test__ = exports.r = { shouldSkipMutatingLoggingConfigRead: _redactClDQONHh.s };
