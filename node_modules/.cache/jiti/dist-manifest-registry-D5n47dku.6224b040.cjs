"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = toPluginActivationState;exports.a = clearPluginManifestRegistryCache;exports.c = resolveEffectivePluginActivationState;exports.d = isBundledChannelEnabledByChannelConfig$1;exports.f = normalizePluginsConfigWithResolver$1;exports.g = resolvePluginActivationDecisionShared;exports.h = resolveMemorySlotDecisionShared;exports.i = resolveManifestContractPluginIdsByCompatibilityRuntimePath;exports.l = resolveMemorySlotDecision;exports.m = createPluginEnableStateResolver;exports.n = resolveManifestContractOwnerPluginId;exports.o = void 0;exports.p = createEffectiveEnableStateResolver;exports.r = resolveManifestContractPluginIds;exports.s = normalizePluginsConfigWithResolver;exports.t = loadPluginManifestRegistry;exports.u = hasExplicitPluginConfig$1;var _stringCoerceCjxCKZ6B = require("./string-coerce-CjxCKZ6B.js");
var _utilsCB8xp0O = require("./utils-CB8xp0O4.js");
var _idsBPsM98Uu = require("./ids-BPsM98Uu.js");
var _versionBym9kFjy = require("./version-Bym9kFjy.js");
var _stringNormalization5F0OqbT = require("./string-normalization-5F-0OqbT.js");
var _manifestDgyExZJE = require("./manifest-DgyExZJE.js");
var _pathSafetyBa_J5G0M = require("./path-safety-Ba_J5G0M.js");
var _discoveryDoO9rWnu = require("./discovery-DoO9rWnu.js");
var _slotsDkFhJWYQ = require("./slots-DkFhJWYQ.js");
var _minHostVersionLEP3RI0K = require("./min-host-version-lEP3RI0K.js");
...
function normalizeSlotValue(value) {
  const trimmed = (0, _stringCoerceCjxCKZ6B.c)(value);
  if (!trimmed) return;
  if ((0, _stringCoerceCjxCKZ6B.s)(trimmed) === "none") return null;
  return trimmed;
}
function normalizePluginEntries(entries, normalizePluginId) {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    const subagent = subagentRaw && typeof subagentRaw === "object" && !Array.isArray(subagentRaw) ? {
      ...
      allowedModels: Array.isArray(subagentRaw.allowedModels) ? subagentRaw.allowedModels.map((model) => (0, _stringCoerceCjxCKZ6B.c)(model)).filter((model) => Boolean(model)) : void 0
    } : void 0;
...
function normalizePluginsConfigWithResolver$1(config, normalizePluginId = identityNormalizePluginId) {
  ...
  return {
    ...
    slots: {
      memory: memorySlot === void 0 ? (0, _slotsDkFhJWYQ.n)("memory") : memorySlot,
...
function isBundledChannelEnabledByChannelConfig$1(cfg, pluginId) {
  if (!cfg) return false;
  const channelId = (0, _idsBPsM98Uu.r)(pluginId);
...
function resolveManifestContractOwnerPluginId(params) {
  const normalizedValue = (0, _stringCoerceCjxCKZ6B.s)(params.value);
  if (!normalizedValue) return;
  return loadPluginManifestRegistry({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env
  }).plugins.find((plugin) => (!params.origin || plugin.origin === params.origin) && listContractValues(plugin, params.contract).some((candidate) => (0, _stringCoerceCjxCKZ6B.s)(candidate) === normalizedValue))?.id;
}
...
function buildCacheKey(params) {
  const { roots, loadPaths } = (0, _discoveryDoO9rWnu.r)({
    workspaceDir: params.workspaceDir,
    loadPaths: params.plugins.loadPaths,
    env: params.env
  });
  return `${roots.workspace ?? ""}::${roots.global}::${roots.stock ?? ""}::${(0, _versionBym9kFjy.o)(params.env)}::${JSON.stringify(loadPaths)}`;
}
...
function normalizePreferredPluginIds(raw) {
  return (0, _stringNormalization5F0OqbT.a)(raw);
}
function mergePackageChannelMetaIntoChannelConfigs(params) {
  ...
  const label = existing.label ?? (0, _stringCoerceCjxCKZ6B.c)(params.packageChannel?.label) ?? "";
  const description = existing.description ?? (0, _stringCoerceCjxCKZ6B.c)(params.packageChannel?.blurb) ?? "";
...
function buildRecord(params) {
  ...
  return {
    ...
    name: (0, _stringCoerceCjxCKZ6B.c)(params.manifest.name) ?? params.candidate.packageName,
    description: (0, _stringCoerceCjxCKZ6B.c)(params.manifest.description) ?? params.candidate.packageDescription,
    version: (0, _stringCoerceCjxCKZ6B.c)(params.manifest.version) ?? params.candidate.packageVersion,
...
function buildBundleRecord(params) {
  return {
    id: params.manifest.id,
    name: (0, _stringCoerceCjxCKZ6B.c)(params.manifest.name) ?? params.candidate.idHint,
    description: (0, _stringCoerceCjxCKZ6B.c)(params.manifest.description),
    version: (0, _stringCoerceCjxCKZ6B.c)(params.manifest.version),
    format: "bundle",
    bundleFormat: params.candidate.bundleFormat,
...
function matchesInstalledPluginRecord(params) {
  ...
  const candidateSource = (0, _utilsCB8xp0O.m)(params.candidate.source, params.env);
  const trackedPaths = [record.installPath, record.sourcePath].filter((entry) => typeof entry === "string" && entry.trim().length > 0).map((entry) => (0, _utilsCB8xp0O.m)(entry, params.env));
  if (trackedPaths.length === 0) return false;
  return trackedPaths.some((trackedPath) => {
    return candidateSource === trackedPath || (0, _pathSafetyBa_J5G0M.n)(trackedPath, candidateSource);
  });
}
...
function loadPluginManifestRegistry(params = {}) {
  ...
  const currentHostVersion = (0, _versionBym9kFjy.o)(env);
  for (const candidate of candidates) {
    ...
    } : isBundleRecord && candidate.bundleFormat ? (0, _pathSafetyBa_J5G0M.l)({
      rootDir: candidate.rootDir,
      bundleFormat: candidate.bundleFormat,
      rejectHardlinks
    }) : (0, _manifestDgyExZJE.i)(candidate.rootDir, rejectHardlinks);
    if (!manifestRes.ok) {
      diagnostics.push({
        level: "error",
    ...
    const minHostVersionCheck = (0, _minHostVersionLEP3RI0K.t)({
    ...
    if (existing) {
      ...
      if ((() => {
        ...
        const existingReal = (0, _pathSafetyBa_J5G0M.r)(existing.candidate.rootDir, realpathCache);
        const candidateReal = (0, _pathSafetyBa_J5G0M.r)(candidate.rootDir, realpathCache);
