"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = enableConsoleCapture;exports.c = routeLogsToStderr;exports.d = setConsoleTimestampPrefix;exports.f = shouldLogSubsystemToConsole;exports.i = stripRedundantSubsystemPrefixForConsole;exports.l = setConsoleConfigLoaderForTests;exports.n = createSubsystemRuntime;exports.o = getConsoleSettings;exports.r = runtimeForLogger;exports.s = getResolvedConsoleSettings;exports.t = createSubsystemLogger;exports.u = setConsoleSubsystemFilter;var _redactBl2deF7j = require("./redact-Bl2deF7j.js");
var _stringCoerceC1IzJjqi = require("./string-coerce-C1IzJjqi.js");
var _progressLineXDw3PVgH = require("./progress-line-xDw3PVgH.js");
var _loggerX0IvPL2B = require("./logger-x0IvPL2B.js");
...
const requireConfig = (0, _redactBl2deF7j.o)("file:///home/rs/openclaw/dist/subsystem-CWI_MDy_.js");
...
function resolveConsoleSettings() {
  const envLevel = (0, _loggerX0IvPL2B.m)();
  if (process.env.VITEST === "true" && process.env.OPENCLAW_TEST_CONSOLE !== "1" && !(0, _loggerX0IvPL2B.S)() && !envLevel && !_loggerX0IvPL2B.h.overrideSettings) return {
    level: "silent",
    style: normalizeConsoleStyle(void 0)
  };
  let cfg = _loggerX0IvPL2B.h.overrideSettings ?? (0, _loggerX0IvPL2B.b)();
  if (!cfg && !(0, _loggerX0IvPL2B.x)()) if (_loggerX0IvPL2B.h.resolvingConsoleSettings) cfg = void 0;else
  {
    _loggerX0IvPL2B.h.resolvingConsoleSettings = true;
    try {
      cfg = loadConfigFallback();
    } finally {
      _loggerX0IvPL2B.h.resolvingConsoleSettings = false;
    }
  }
  return {
...
function getConsoleSettings() {
  ...
  const cached = _loggerX0IvPL2B.h.cachedConsoleSettings;
  if (!cached || consoleSettingsChanged(cached, settings)) _loggerX0IvPL2B.h.cachedConsoleSettings = settings;
  return _loggerX0IvPL2B.h.cachedConsoleSettings;
}
function getResolvedConsoleSettings() {
  return getConsoleSettings();
}
function routeLogsToStderr() {
  _loggerX0IvPL2B.h.forceConsoleToStderr = true;
}
function setConsoleSubsystemFilter(filters) {
  if (!filters || filters.length === 0) {
    _loggerX0IvPL2B.h.consoleSubsystemFilter = null;
    return;
  }
  const normalized = filters.map((value) => value.trim()).filter((value) => value.length > 0);
  _loggerX0IvPL2B.h.consoleSubsystemFilter = normalized.length > 0 ? normalized : null;
}
function setConsoleTimestampPrefix(enabled) {
  _loggerX0IvPL2B.h.consoleTimestampPrefix = enabled;
}
...
function shouldLogSubsystemToConsole(subsystem) {
  const filter = _loggerX0IvPL2B.h.consoleSubsystemFilter;
...
function shouldSuppressConsoleMessage(message) {
  if ((0, _loggerX0IvPL2B.S)()) return false;
...
function formatConsoleTimestamp(style) {
  ...
  if (style === "pretty") return (0, _loggerX0IvPL2B.f)(now, { style: "short" }).replace(/[+-]\d{2}:\d{2}$/, "");
  return (0, _loggerX0IvPL2B.d)(now);
}
...
function enableConsoleCapture() {
  if (_loggerX0IvPL2B.h.consolePatched) return;
  _loggerX0IvPL2B.h.consolePatched = true;
  if (!_loggerX0IvPL2B.h.streamErrorHandlersInstalled) {
    _loggerX0IvPL2B.h.streamErrorHandlersInstalled = true;
  ...
  let logger = null;
  const getLoggerLazy = () => {
    if (!logger) logger = (0, _loggerX0IvPL2B.a)();
    return logger;
  };
  const original = {
    log: console.log,
  ...
  const forward = (level, orig) => (...args) => {
    ...
    const trimmed = (0, _ansiBZHMLcUk.r)(formatted).trimStart();
    const timestamp = _loggerX0IvPL2B.h.consoleTimestampPrefix && trimmed.length > 0 && !hasTimestampPrefix(trimmed) ? formatConsoleTimestamp(getConsoleSettings().style) : "";
    try {
    ...
    if (_loggerX0IvPL2B.h.forceConsoleToStderr) try {
...
function shouldLogToConsole(level, settings) {
  ...
  return (0, _loggerX0IvPL2B._)(level) >= (0, _loggerX0IvPL2B._)(settings.level);
}
const inspectValue = (() => {
...
function isRichConsoleEnv() {
  const term = (0, _stringCoerceC1IzJjqi.a)(process.env.TERM);
...
function isChannelSubsystemPrefix(value) {
  const normalized = (0, _stringCoerceC1IzJjqi.a)(value);
  if (!normalized) return false;
  return (0, _ids2bZIXJNM.r)(normalized) === normalized || normalized === "webchat";
}
...
function stripRedundantSubsystemPrefixForConsole(message, displaySubsystem) {
  ...
  if (message.startsWith("[")) {
    ...
    if (closeIdx > 1) {
      if ((0, _stringCoerceC1IzJjqi.a)(message.slice(1, closeIdx)) === (0, _stringCoerceC1IzJjqi.a)(displaySubsystem)) {
        let i = closeIdx + 1;
        while (message[i] === " ") i += 1;
  ...
  if ((0, _stringCoerceC1IzJjqi.a)(message.slice(0, displaySubsystem.length)) !== (0, _stringCoerceC1IzJjqi.a)(displaySubsystem)) return message;
...
function formatConsoleLine(opts) {
  ...
  return `${[(() => {
    ...
    if (_loggerX0IvPL2B.h.consoleTimestampPrefix) return color.gray(formatConsoleTimestamp(opts.style));
    return "";
...
function writeConsoleLine(level, line) {
  ...
  const sink = _loggerX0IvPL2B.h.rawConsole ?? console;
  if (_loggerX0IvPL2B.h.forceConsoleToStderr || level === "error" || level === "fatal") (sink.error ?? console.error)(sanitized);else
...
function createSubsystemLogger(subsystem) {
  ...
  const emitLog = (level, message, meta) => {
    ...
    const fileEnabled = (0, _loggerX0IvPL2B.s)(level);
    ...
    if (fileEnabled) {
      if (!fileLogger) fileLogger = (0, _loggerX0IvPL2B.i)({ subsystem: resolvedSubsystem });
  ...
  return {
    ...
    isEnabled(level, target = "any") {
      ...
      const isFileEnabled = (0, _loggerX0IvPL2B.s)(level);
