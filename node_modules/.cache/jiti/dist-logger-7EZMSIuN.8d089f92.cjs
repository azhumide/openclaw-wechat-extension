...
var _diagnosticEventsCKGnBU6G = require("./diagnostic-events-CKGnBU6G.js");
var _tmpOpenclawDirCN8v1wId = require("./tmp-openclaw-dir-CN8v1wId.js");
...
function resolveDefaultLogDir() {
  return canUseNodeFs() ? (0, _tmpOpenclawDirCN8v1wId.n)() : _tmpOpenclawDirCN8v1wId.t;
}
function resolveDefaultLogFile(defaultLogDir) {
  return canUseNodeFs() ? _nodePath.default.join(defaultLogDir, "openclaw.log") : `${_tmpOpenclawDirCN8v1wId.t}/openclaw.log`;
}
...
function assignDiagnosticLogAttribute(attributes, state, key, value) {
  ...
  if ((0, _prototypeKeysRBElZn9Q.t)(normalizedKey)) return;
...
function normalizeTraceContext(value) {
  ...
  if (!(0, _diagnosticEventsCKGnBU6G._)(candidate.traceId)) return;
  if (candidate.spanId !== void 0 && !(0, _diagnosticEventsCKGnBU6G.h)(candidate.spanId)) return;
  if (candidate.parentSpanId !== void 0 && !(0, _diagnosticEventsCKGnBU6G.h)(candidate.parentSpanId)) return;
  if (candidate.traceFlags !== void 0 && !(0, _diagnosticEventsCKGnBU6G.g)(candidate.traceFlags)) return;
  return {
...
function buildTraceFileLogFields(logObj) {
  ...
  const trace = findLogTraceContext(bindings, args) ?? (0, _diagnosticEventsCKGnBU6G.m)();
  if (!trace) return;
  return {
...
function buildDiagnosticLogRecord(logObj) {
  ...
  const trace = findLogTraceContext(bindings, numericArgs) ?? (0, _diagnosticEventsCKGnBU6G.m)();
...
function attachDiagnosticEventTransport(logger) {
  logger.attachTransport((logObj) => {
    try {
      (0, _diagnosticEventsCKGnBU6G.n)(buildDiagnosticLogRecord(logObj));
    } catch {}
  });
}
...
const __test__ = exports.r = { shouldSkipMutatingLoggingConfigRead: _redact1fZUZMlV.l };
