"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveChannelDefaultAccountId;exports.n = formatPairingApproveHint;exports.r = parseOptionalDelimitedEntries;exports.t = buildAccountScopedDmSecurityPolicy;var _commandFormatBFuugklF = require("./command-format-BFuugklF.js");
...
function formatPairingApproveHint(channelId) {
  return `Approve via: ${(0, _commandFormatBFuugklF.t)(`openclaw pairing list ${channelId}`)} / ${(0, _commandFormatBFuugklF.t)(`openclaw pairing approve ${channelId} <code>`)}`;
}
...
function buildAccountScopedDmSecurityPolicy(params) {
  ...
  const defaultBasePath = `channels.${params.channelKey}.accounts.${_accountIdC3j_3_su.t}.`;
  const accountConfig = channelConfig?.accounts?.[resolvedAccountId];
  const defaultAccountConfig = params.inheritSharedDefaultsFromDefaultAccount && resolvedAccountId !== "default" ? channelConfig?.accounts?.[_accountIdC3j_3_su.t] : void 0;
