...
async function loadSharp() {
  sharpFactoryPromise ??= Promise.resolve().then(() => jitiImport("sharp").then((m) => _interopRequireWildcard(m))).then((mod) => {
...
async function withTempDir(fn) {
  const dir = await _promises.default.mkdtemp(_nodePath.default.join((0, _tmpOpenclawDirCoGSA7K.n)(), "openclaw-img-"));
  try {
    return await fn(dir);
  } finally {
...
async function sipsMetadataFromBuffer(buffer) {
  return await withTempDir(async (dir) => {
    ...
    const { stdout } = await (0, _execBaXt0vsL.i)("/usr/bin/sips", [
    "-g",
    "pixelWidth",
    "-g",
    "pixelHeight",
...
async function sipsResizeToJpeg(params) {
  return await withTempDir(async (dir) => {
    ...
    await (0, _execBaXt0vsL.i)("/usr/bin/sips", [
    "-Z",
