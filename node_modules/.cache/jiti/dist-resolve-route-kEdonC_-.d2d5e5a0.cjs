...
function resolveAgentLookupCache(cfg) {
  ...
  const next = {
    agentsRef,
    byNormalizedId,
    fallbackDefaultAgentId: (0, _sessionKey7OXmevzf.f)((0, _agentScopeYDjJfO9L.x)(cfg))
  };
...
function buildEvaluatedBindingsByChannel(cfg) {
  ...
  let order = 0;
  for (const binding of (0, _bindingsDZvGYKEX.n)(cfg)) {
...
function resolveAgentRoute(input) {
  ...
  const shouldLogDebug = (0, _globalsDCctf2Pw.a)();
  ...
  if (shouldLogDebug) {
    (0, _loggerC7FmQ1yA.t)(`[routing] resolveAgentRoute: channel=${channel} accountId=${accountId} peer=${formatPeer(peer)} guildId=${guildId || "none"} teamId=${teamId || "none"} bindings=${bindings.length}`);
    for (const entry of bindings) (0, _loggerC7FmQ1yA.t)(`[routing] binding: agentId=${entry.binding.agentId} accountPattern=${entry.match.accountPattern || "default"} peer=${formatNormalizedPeer(entry.match.peer)} guildId=${entry.match.guildId ?? "none"} teamId=${entry.match.teamId ?? "none"} roles=${entry.match.roles?.length ?? 0}`);
  }
  const baseScope = {
    guildId,
    teamId,
  ...
  for (const tier of tiers) {
    ...
    if (matched) {
      if (shouldLogDebug) (0, _loggerC7FmQ1yA.t)(`[routing] match: matchedBy=${tier.matchedBy} agentId=${matched.binding.agentId}`);
      return choose(matched.binding.agentId, tier.matchedBy, matched.binding.session);
    }
  }
  return choose((0, _agentScopeYDjJfO9L.x)(input.cfg), "default");
}
//#endregion /* v9-5aedcde6893ada54 */
