...
function scopedHeartbeatWakeOptions(sessionKey, wakeOptions) {
  return (0, _sessionKeyUtilsB9qLc2A.o)(sessionKey) ? {
    ...wakeOptions,
    sessionKey
  } : wakeOptions;
}
...
function toAgentRequestSessionKey(storeKey) {
  ...
  if (!raw) return;
  return (0, _sessionKeyUtilsB9qLc2A.o)(raw)?.rest ?? raw;
}
function toAgentStoreSessionKey(params) {
  ...
  const parsed = (0, _sessionKeyUtilsB9qLc2A.o)(raw);
  if (parsed) return `agent:${parsed.agentId}:${parsed.rest}`;
  if (lowered.startsWith("agent:")) return lowered;
  return `agent:${normalizeAgentId(params.agentId)}:${lowered}`;
}
function resolveAgentIdFromSessionKey(sessionKey) {
  return normalizeAgentId((0, _sessionKeyUtilsB9qLc2A.o)(sessionKey)?.agentId ?? "main");
}
function classifySessionKeyShape(sessionKey) {
  const raw = (sessionKey ?? "").trim();
  if (!raw) return "missing";
  if ((0, _sessionKeyUtilsB9qLc2A.o)(raw)) return "agent";
