"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var fs = _interopRequireWildcard(require("fs"));
var path = _interopRequireWildcard(require("path"));
var _buffer = require("buffer");
var _nodeCrypto = require("node:crypto");
var _ws = require("ws");

var _core = require("openclaw/plugin-sdk/core");
var _channel = require("./src/channel.js");
var _config = require("./src/config.js");
var _redaction = require("./src/redaction.js");
var _runtime = require("./src/runtime.js");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
let bridgeClosePromise = null;
let bridgeClientReconnectTimer = null;
...
function getGlobalState() {
  ...
  if (!(existing.boundApis instanceof Set)) existing.boundApis = new Set();
  if (!(existing.recentToolNoticeAt instanceof Map)) existing.recentToolNoticeAt = new Map();
  if (!(existing.recentToolAuthLogAt instanceof Map)) existing.recentToolAuthLogAt = new Map();
  if (!(existing.recentReplyMediaAt instanceof Map)) existing.recentReplyMediaAt = new Map();
  if (!("clientConnecting" in existing)) existing.clientConnecting = false;
  return existing;
}

function markWechatApiBound(api) {
...
function formatBridgeStateSnapshot(state = getGlobalState()) {
  const activeSocket = (0, _runtime.getActiveBridgeSocket)();
  return [
...
function logBridgeState(api, phase, state = getGlobalState()) {
  api?.logger.debug?.(`[WeChat] Bridge state (${phase}): ${formatBridgeStateSnapshot(state)}`);
}

function hasActiveBridgeClient(state = getGlobalState()) {
  const activeSocket = (0, _runtime.getActiveBridgeSocket)();
  return Boolean(
    state.clientConnecting ||
    activeSocket && activeSocket.readyState === _ws.WebSocket.OPEN
  );
}

function buildFrame(direction, event, payload = {}) {
  return {
    direction,
    event,
    payload,
...
function restoreServedFilePath(rawPath) {
...
function stripFalseWechatMediaFailureSuffix(text)
 ...
 */
function mergeOverlappingStrings(current, next) {
...
function collapseRepeatedReplyText(text) {
...
function summarizeWechatTextForLog(text, maxLength = 160) {
...
config)
{
  return (0, _redaction.redactWechatWxids)(text, {
...
config)
{
  return (0, _redaction.redactWechatUnknownText)(value, {
    enabled: config?.redactWxidsInLogs !== false,
    exactMatches: config?.redactExtraWxids
  });
}

function rewriteWechatNonOwnerAddressing(text, params)
...
function isWechatLocalMediaReference(value) {
...
function isWechatSafeLocalAttachmentPath(filePath) {
...
function buildWechatLocalMediaDedupKey(filePath, logger) {
  const absolutePath = path.resolve(filePath.trim());

  try {
    const stat = fs.statSync(absolutePath);
    const hash = (0, _nodeCrypto.createHash)("sha1");
    ...
    try {
      ...
      if (sampleBytes > 0) {
        const headBuffer = _buffer.Buffer.alloc(sampleBytes);
        ...
        if (stat.size > sampleBytes) {
          ...
          if (tailBytes > 0) {
            const tailBuffer = _buffer.Buffer.alloc(tailBytes);
...
function normalizeGuardedToolNameList(value) {
  return new Set(
    (value || []).
...
function normalizeWechatIdAllowList(value) {
  return new Set(
    (value || []).
    map((entry) => entry.trim().toLowerCase()).
    filter(Boolean)
...
function normalizeWechatSkillIdList(value) {
  return new Set(
    (value || []).
    map((entry) => entry.trim().toLowerCase()).
    filter(Boolean)
...
function buildWechatBlockedSkillIntentAliases(skillId) {
...
function matchWechatBlockedSkillIntent(text, blockedSkills)
...
function getWechatSkillRootCandidates(config) {
...
function resolveWechatInstalledSkillsSnapshot(config)
...
function splitWechatShellTokens(command) {
  const tokens = command.match(/"([^"\\]|\\.)*"|'[^']*'|\S+/g);
  return tokens ? tokens.map((token) => token.trim()).filter(Boolean) : [];
}

function unquoteWechatShellToken(token) {
...
function splitWechatShellSegments(command) {
  const segments = [];
  let current = "";
  let inSingle = false;
  let inDouble = false;
...
function normalizeWechatExecutableBase(token) {
...
function isWechatEnvAssignmentToken(token) {
...
function unwrapWechatInstalledSkillCommand(command, workdir)
...
function resolveWechatCommandPathCandidate(token, workdir) {
...
function isWechatPathWithinRoots(filePath, roots) {
...
function resolveWechatSkillIdFromPath(filePath, skillRoots) {
...
function resolveWechatSkillPreludeReadSegment(segment, workdir, skillRoots)
...
function isWechatSkillPreludeMarkerSegment(segment) {
  const tokens = splitWechatShellTokens(segment);
...
function resolveWechatSkillCdSegment(segment, workdir, skillRoots) {
  const tokens = splitWechatShellTokens(segment);
...
function isWechatSkillSourceSegment(segment, workdir, skillRoots) {
  const tokens = splitWechatShellTokens(segment);
  if (tokens.length !== 2) {
    return false;
  }
...
function resolveWechatInstalledSkillCommandMatch(command, workdir, config) {
...
function summarizeWechatInstalledSkillMatch(match) {
  if (!match.matched) {
    return "";
  }
  const parts = [];
...
function summarizeWechatSkillRootsForLog(roots) {
  if (roots.length === 0) {
    return "roots=0";
  }
  const preview = roots.
...
function buildWechatInstalledSkillDebugSummary(command, workdir, config) {
  const trimmed = command.trim();
  if (!trimmed) {
...
function isWechatLogRecord(value) {
  return Boolean(value) && typeof value === "object" && !Array.isArray(value);
}

function summarizeWechatUrlForLog(value, maxLength = 160) {
  if (typeof value !== "string") {
    return "";
  }
  const trimmed = value.trim();
...
function summarizeWechatToolParamsForLog(toolName, params) {
  if (!params) {
    return "params=none";
  }
...
function summarizeWechatToolResultForLog(toolName, result, error) {
...
function isWechatInternalStatusReply(text) {
  const normalized = text.replace(/\r\n/g, "\n").trim();
  if (!normalized) {
    return { matched: false };
  }
...
function isWechatPermissionDeniedReply(text) {
  const normalized = text.replace(/\r\n/g, "\n").trim();
  if (!normalized) {
    return false;
  }
...
function isWechatToolFailureSummaryReply(text) {
  const normalized = text.replace(/\r\n/g, "\n").trim();
  if (!normalized) {
    return false;
  }
...
function getWechatToolNoticeStateLabel(state) {
  switch (state) {
    case "queued":
      return "已提交审批";
    case "allow-once":
...
function getWechatChatTypeLabel(chatType) {
  return chatType === "group" ? "群聊" : "私聊";
}

function renderWechatToolNoticeTemplate(template, params)
...
function buildWechatToolApprovalDescription(toolName, authContext)
...
function shouldSendWechatToolAuthNotice(config, params)
...
function hasRecentWechatReplyMedia(dedupKey) {
  const now = Date.now();
...
function claimWechatReplyMediaDedup(dedupKey) {
  const now = Date.now();
  pruneWechatReplyMediaDedupMap(now);
  const dedupMap = getGlobalState().recentReplyMediaAt;
...
function releaseWechatReplyMediaDedup(dedupKey) {
  getGlobalState().recentReplyMediaAt.delete(dedupKey);
}

async function sendWechatToolAuthNotice(api, authContext,
...
text) {
  ...
  try {
    ...
    await _channel.wechatPlugin.outbound?.sendText?.({
      to,
      text: trimmedText,
...
function resolveWechatFallbackNoticeContextFromSessionKey(sessionKey) {
  const trimmed = sessionKey?.trim();
  if (!trimmed) {
    return null;
  }

  const fallbackAuth = (0, _runtime.getWechatToolAuthFallbackForSession)(trimmed);
...

{
  ...
  if (sessionKey && (
  (0, _runtime.getWechatToolAuthForSession)(sessionKey) ||
  (0, _runtime.getWechatToolAuthFallbackForSession)(sessionKey)))
  {
    return true;
  }
  const runId = params.runId?.trim();
  if (runId && (0, _runtime.getWechatToolAuthForRun)(runId)) {
    return true;
  }
  return false;
}
...
function resolveWechatContextSessionKey(ctx) {
...
async function closeBridgeResources(api, reason = "shutdown") {
  ...
  bridgeClosePromise = (async () => {
    ...
    if (activeSocket) {
      ...
      (0, _runtime.setActiveBridgeSocket)(null);
    }

    const wsServer = bridgeWss ?? state.wsServer ?? null;
    bridgeWss = null;
    state.wsServer = null;
    (0, _runtime.setWechatWsServer)(null);
    if (wsServer) {
      await closeWebSocketServer(wsServer);
    }

    clearModuleRefs();
    (0, _runtime.clearBridgeRuntimeState)();
...
function attachBridgeClientSocketHandlers(api, socket) {
  (0, _runtime.setActiveBridgeSocket)(socket);
  (0, _runtime.markBridgePong)();
  ...
  socket.on("close", (code) => {
    const state = getGlobalState();
    if ((0, _runtime.getActiveBridgeSocket)() === socket) {
      (0, _runtime.setActiveBridgeSocket)(null);
    }
...
function installBridgeHeartbeat(api, state = getGlobalState()) {
  if (bridgeHeartbeatTimer) {
    clearInterval(bridgeHeartbeatTimer);
    bridgeHeartbeatTimer = null;
  }
  ...
  bridgeHeartbeatTimer = setInterval(() => {
    const socket = (0, _runtime.getActiveBridgeSocket)();
    if (!socket || socket.readyState !== socket.OPEN) return;
    if (Date.now() - (0, _runtime.getBridgeLastPongAt)() > 70000) {
...
function buildWechatBridgeServerUrl(wsConfig) {
  return `ws://${wsConfig.host}:${wsConfig.port}${wsConfig.path}`;
}

function scheduleBridgeClientReconnect(api, reason) {
...
async function connectBridgeClient(api, wsConfig) {
  const state = getGlobalState();
  if (state.clientConnecting) {
    return;
  }
  const activeSocket = (0, _runtime.getActiveBridgeSocket)();
  if (activeSocket && activeSocket.readyState === _ws.WebSocket.OPEN) {
    return;
  }
  ...
  await new Promise((resolve, reject) => {
    const socket = new _ws.WebSocket(targetUrl);
    let settled = false;
...
function handleBridgeStartFailure(api, err) {
  const state = getGlobalState();
...
async function ensureBridgeStarted(api) {
  ...
  const performStart = async () => {
    ...
    const bridgeConfig = (0, _config.resolveWechatExtensionConfig)(cfg, api.logger);
    const wsConfig = {
...
function triggerBridgeStart(api) {
  void ensureBridgeStarted(api).catch((err) => {
    handleBridgeStartFailure(api, err);
  });
}

function isWechatTruthyEnv(value) {
  if (typeof value !== "string") {
    return undefined;
  }
...
function hasWechatSupervisorHintEnv() {
  const keys = [
  "LAUNCH_JOB_LABEL",
  "LAUNCH_JOB_NAME",
  "XPC_SERVICE_NAME",
...
function maybeTriggerWechatBridgeStart(api, reason) {
 ...
 */
function getAllowedTmpDir(cfg, logger) {
  const bridgeConfig = (0, _config.resolveWechatExtensionConfig)(cfg, logger);
...
async function handleInboundMessage(api, body) {
  const logBody = { ...body };
  ...
  if (media) {
    try {
      if (media.data) {
        const buffer = _buffer.Buffer.from(media.data, "base64");
        ...
        if (media.path.startsWith("http://") || media.path.startsWith("https://")) {
          ...
          if (response.ok) {
            const buffer = _buffer.Buffer.from(await response.arrayBuffer());
            fs.writeFileSync(dest, buffer);
            mediaPath = dest;
  ...
  (0, _runtime.enqueueWechatInboundToolAuth)({
    sessionKey,
    from,
    accountId: accountId || "default",
    chatType,
  ...
  if (blockedSkillIntent.matched && blockedSkillIntent.skillId) {
    ...
    (0, _runtime.markWechatBlockedReplyForSession)({
      sessionKey,
  ...
  let pendingBlockMediaPaths = [];
  let pendingBlockMediaTimer = null;
  const pendingBlockMediaDelayMs = 1200;
  const mediaDedupKeyCache = new Map();
  const replyMediaDispatchId = upstreamMessageTraceId || messageId;

  const resolveMediaDedupKey = (mediaUrl) => {
    const trimmed = mediaUrl.trim();
  ...
  const buildReplyMediaScopeKey = (mediaDedupKey) =>
  ...
  const isRecentlySentReplyMedia = (mediaDedupKey) =>
  ...
  source) =>
  {
    if (!_channel.wechatPlugin.outbound?.sendMedia) {
      return false;
    }
    ...
    try {
      const sendResult = await _channel.wechatPlugin.outbound.sendMedia({
        to: from,
  ...
  const flushPendingBlockMediaPaths = async (reason) => {
  ...
  const dispatchResult = await runtime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
    ...
    dispatcherOptions: {
      ...
      deliver: async (...args) => {
        ...
        (0, _runtime.getWechatBlockedReplyForSession)(sessionKey) :
        undefined;
        ...
        while ((match = mediaRegex.exec(textToProcess)) !== null) {
          ...
          if (precedingText && _channel.wechatPlugin.outbound?.sendText) {
            await _channel.wechatPlugin.outbound.sendText({
              to: from,
              text: precedingText,
              msg_id: messageId,
        ...
        if (remainingText && _channel.wechatPlugin.outbound?.sendText) {
          await _channel.wechatPlugin.outbound.sendText({
            to: from,
            text: remainingText,
            msg_id: messageId,
  ...
  unseenText;
  if (!cumulativeSentText && finalUnseenText && _channel.wechatPlugin.outbound?.sendText) {
    const bridgeConfig = (0, _config.resolveWechatExtensionConfig)(cfg, api.logger);
...
const plugin = {
  ...
  configSchema: (0, _core.emptyPluginConfigSchema)(),
  register(api) {
    try {
      ...
      if (sharedState.registering) {
        (0, _runtime.setWechatRuntime)(api.runtime);
    ...
    function bindApiHandlers() {
...

      {
        ...
        const existingRunAuth = (0, _runtime.getWechatToolAuthForRun)(runId);
        if (existingRunAuth) {
          return existingRunAuth;
        }

        const boundAuth = (0, _runtime.bindWechatToolAuthToRun)({
          sessionKey,
          runId
        });
        if (boundAuth) {
          api.logger.debug?.(
            `[WeChat ToolAuth] Bound auth to run via ${params.hookName} ${(0, _runtime.summarizeWechatToolAuthDebugState)({
              sessionKey,
              runId
            })}`
          );
        ...
        if (shouldApplyWechatToolAuth({ sessionKey, runId })) {
          const existingSessionAuth = (0, _runtime.getWechatToolAuthForSession)(sessionKey);
          const debugState = (0, _runtime.summarizeWechatToolAuthDebugState)({
            sessionKey,
            runId
          });
      ...
      api.on("before_dispatch", (event, ctx) => {
        ...
        const promotedAuth = (0, _runtime.promoteWechatToolAuthForDispatch)({
          sessionKey: sessionKey,
          senderId: resolveWechatContextSenderId(ctx, event.senderId),
          content: resolveWechatContextBody(ctx, event.body || event.content)
        });
        if (!promotedAuth) {
          const dispatchSenderId = resolveWechatContextSenderId(ctx, event.senderId) || "";
          api.logger.warn?.(
            `[WeChat ToolAuth] No pending auth matched during dispatch ${(0, _runtime.summarizeWechatToolAuthDebugState)({
              sessionKey
      ...
      api.on("before_tool_call", (event, ctx) => {
        ...
        const effectiveSessionKey = resolveWechatContextSessionKey(ctx);
        ...
        let runBoundAuth = effectiveRunId ? (0, _runtime.getWechatToolAuthForRun)(effectiveRunId) : undefined;
        let sessionBoundAuth = effectiveSessionKey ? (0, _runtime.getWechatToolAuthForSession)(effectiveSessionKey) : undefined;
        if (!runBoundAuth && !sessionBoundAuth && effectiveRunId && effectiveSessionKey) {
          ...
          sessionBoundAuth = effectiveSessionKey ? (0, _runtime.getWechatToolAuthForSession)(effectiveSessionKey) : undefined;
        }
        ...
        if (!authContext && effectiveSessionKey) {
          const fallbackAuthContext = (0, _runtime.getWechatToolAuthFallbackForSession)(effectiveSessionKey);
        ...
        if (guardedTools.has(toolName) && !effectiveRunId) {
          api.logger.warn?.(
            `[WeChat ToolAuth] Guarded tool missing runId tool=${toolName} ${(0, _runtime.summarizeWechatToolAuthDebugState)({
              sessionKey: effectiveSessionKey,
              runId: event.runId
            })}`
          );
        ...
        if (toolName === "write" || toolName === "read") {
          const targetPath = String(event.params?.path || "").trim().replace(/\\/g, "/");
        ...
        if (!authContext) {
          const debugSummary = (0, _runtime.summarizeWechatToolAuthDebugState)({
            sessionKey: effectiveSessionKey,
            runId: effectiveRunId
          });
          ...
          (0, _runtime.getWechatToolAuthFallbackForSession)(effectiveSessionKey) :
          undefined;
        ...
        processSessionId ?
        (0, _runtime.getWechatSkillToolSession)(processSessionId) :
        undefined;
      ...
      api.on("after_tool_call", (event, ctx) => {
        ...
        const toolName = event.toolName.trim().toLowerCase();
        const authContext = (0, _runtime.getWechatToolAuthForRun)(ctx.runId);
        ...
        (0, _runtime.rememberWechatSkillToolSession)({
          sessionId: skillSessionId,
          skillId: installedSkillMatch.skillId,
          sessionKey: resolveWechatContextSessionKey(ctx)
        });
      ...
      api.on("agent_end", (_event, ctx) => {
        if (ctx.runId) {
          (0, _runtime.clearWechatToolAuthForRun)(ctx.runId);
        }
      }, { priority: 100 });
      ...
      api.on("subagent_spawned", (_event, ctx) => {
        ...
        (0, _runtime.inheritWechatToolAuthForChildSession)({
          requesterSessionKey,
          childSessionKey
        });
      }, { priority: 100 });
      ...
      api.on("session_end", (_event, ctx) => {
        const sessionKey = resolveWechatContextSessionKey(ctx);
        if (sessionKey) {
          (0, _runtime.clearWechatToolAuthForSession)(sessionKey);
        }
      }, { priority: 100 });
      ...
      api.registerChannel({ plugin: _channel.wechatPlugin });
    }
  },
  unregister(api) {
    const state = getGlobalState();
    state.boundApis.delete(api);
