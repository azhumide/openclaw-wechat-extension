"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolvePackageExtensionEntries;exports.c = planManifestModelCatalogRows;exports.i = loadPluginManifest;exports.l = loadOpenClawProviderIndex;exports.n = void 0;exports.o = resolveManifestCommandAliasOwnerInRegistry;exports.r = getPackageManifestMetadata;exports.s = planProviderIndexModelCatalogRows;exports.t = void 0;exports.u = normalizeModelCatalogProviderId;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _utilsBYAfe1GS = require("./utils-BYAfe1GS.js");
var _prototypeKeysMkcCRiyK = require("./prototype-keys-mkcCRiyK.js");
var _boundaryFileReadCgZlf7kr = require("./boundary-file-read-cgZlf7kr.js");
var _legacyNamesDfDGLO9Q = require("./legacy-names-DfDGLO9Q.js");
var _typesModelsZqK5Y4x = require("./types.models-ZqK5-y4x.js");
var _stringNormalizationDEQ0pIpk = require("./string-normalization-DEQ0pIpk.js");
var _npmRegistrySpecCPQeYXu = require("./npm-registry-spec-CPQe-YXu.js");
...
const MODEL_CATALOG_APIS = new Set(_typesModelsZqK5Y4x.t);
...
function normalizeSafeRecordKey(value) {
  ...
  return key && !(0, _prototypeKeysMkcCRiyK.t)(key) ? key : "";
}
...
function normalizeStringMap(value) {
  if (!(0, _utilsBYAfe1GS.c)(value)) return;
  const normalized = {};
...
function normalizeModelCatalogInputs(value) {
  const inputs = (0, _stringNormalizationDEQ0pIpk.l)(value).filter((input) => MODEL_CATALOG_INPUTS.has(input));
...
function normalizeModelCatalogTieredCost(value) {
  ...
  for (const entry of value) {
    if (!(0, _utilsBYAfe1GS.c)(entry) || !Array.isArray(entry.range)) continue;
...
function normalizeModelCatalogCompat(value) {
  ...
  {
    const normalized = (0, _stringNormalizationDEQ0pIpk.l)(value[field]);
...
function normalizeModelCatalogModel(value) {
  ...
  const replaces = (0, _stringNormalizationDEQ0pIpk.l)(value.replaces);
  const replacedBy = (0, _stringCoerceBje8XVt.c)(value.replacedBy) ?? "";
  const tags = (0, _stringNormalizationDEQ0pIpk.l)(value.tags);
  return {
    id,
    ...(name ? { name } : {}),
    ...(api ? { api } : {}),
...
function normalizeModelCatalogAliases(value, ownedProviders) {
  ...
  for (const [rawAlias, rawTarget] of Object.entries(value)) {
    ...
    if (!alias || !(0, _utilsBYAfe1GS.c)(rawTarget)) continue;
...
function normalizeModelCatalogSuppressions(value) {
  ...
  const suppressions = [];
  for (const entry of value) {
    if (!(0, _utilsBYAfe1GS.c)(entry)) continue;
...
function normalizeStringList(value) {
  const normalized = (0, _stringNormalizationDEQ0pIpk.l)(value);
  return normalized.length > 0 ? normalized : void 0;
}
...
function normalizeInstall(value) {
  ...
  const parsed = npmSpec ? (0, _npmRegistrySpecCPQeYXu.r)(npmSpec) : null;
  if (!parsed) return;
...
function normalizeCategories(value) {
  return [...new Set((0, _stringNormalizationDEQ0pIpk.l)(value))];
}
...
function normalizeOnboardingScopes(value) {
  const scopes = (0, _stringNormalizationDEQ0pIpk.l)(value).filter((scope) => scope === "text-inference" || scope === "image-generation");
...
function normalizeAuthChoice(params) {
  if (!(0, _utilsBYAfe1GS.c)(params.value)) return;
...
function normalizeProvider(rawProviderId, value) {
  ...
  const id = normalizeModelCatalogProviderId((0, _stringCoerceBje8XVt.c)(value.id) ?? "");
...
function normalizeOpenClawProviderIndex(value) {
  if (!(0, _utilsBYAfe1GS.c)(value) || value.version !== OPENCLAW_PROVIDER_INDEX_VERSION) return;
  if (!(0, _utilsBYAfe1GS.c)(value.providers)) return;
  const providers = {};
  for (const [rawProviderId, rawProvider] of Object.entries(value.providers)) {
    const providerId = normalizeModelCatalogProviderId(rawProviderId);
    if (!providerId || (0, _prototypeKeysMkcCRiyK.t)(providerId)) continue;
...
function planManifestModelCatalogPluginEntries(params) {
  ...
  return Object.entries(providers).flatMap(([provider, providerCatalog]) => {
    ...
    if (!normalizedProvider || params.providerFilter && normalizedProvider !== params.providerFilter) return [];
...
function normalizeStringListRecord(value) {
  ...
  for (const [key, rawValues] of Object.entries(value)) {
    ...
    if (!providerId || (0, _prototypeKeysMkcCRiyK.t)(providerId)) continue;
    const values = (0, _stringNormalizationDEQ0pIpk.l)(rawValues);
...
function normalizeStringRecord(value) {
  ...
  for (const [rawKey, rawValue] of Object.entries(value)) {
    ...
    if (!key || (0, _prototypeKeysMkcCRiyK.t)(key) || !value) continue;
...
function normalizeMediaUnderstandingCapabilities(value) {
  const values = (0, _stringNormalizationDEQ0pIpk.l)(value).filter((entry) => MEDIA_UNDERSTANDING_CAPABILITIES.has(entry));
  return values.length > 0 ? values : void 0;
}
function normalizeMediaUnderstandingNativeDocumentInputs(value) {
  const values = (0, _stringNormalizationDEQ0pIpk.l)(value).filter((entry) => entry === "pdf");
  return values.length > 0 ? values : void 0;
}
function normalizeMediaUnderstandingProviderMetadata(value) {
  ...
  for (const [rawProviderId, rawMetadata] of Object.entries(value)) {
    ...
    if (!providerId || (0, _prototypeKeysMkcCRiyK.t)(providerId) || !(0, _utilsBYAfe1GS.c)(rawMetadata)) continue;
...
function normalizeManifestContracts(value) {
  if (!(0, _utilsBYAfe1GS.c)(value)) return;
  const embeddedExtensionFactories = (0, _stringNormalizationDEQ0pIpk.l)(value.embeddedExtensionFactories);
  const agentToolResultMiddleware = (0, _stringNormalizationDEQ0pIpk.l)(value.agentToolResultMiddleware);
  const externalAuthProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.externalAuthProviders);
  const memoryEmbeddingProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.memoryEmbeddingProviders);
  const speechProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.speechProviders);
  const realtimeTranscriptionProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.realtimeTranscriptionProviders);
  const realtimeVoiceProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.realtimeVoiceProviders);
  const mediaUnderstandingProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.mediaUnderstandingProviders);
  const documentExtractors = (0, _stringNormalizationDEQ0pIpk.l)(value.documentExtractors);
  const imageGenerationProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.imageGenerationProviders);
  const videoGenerationProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.videoGenerationProviders);
  const musicGenerationProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.musicGenerationProviders);
  const webContentExtractors = (0, _stringNormalizationDEQ0pIpk.l)(value.webContentExtractors);
  const webFetchProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.webFetchProviders);
  const webSearchProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.webSearchProviders);
  const tools = (0, _stringNormalizationDEQ0pIpk.l)(value.tools);
  const contracts = {
...
function normalizeManifestConfigContracts(value) {
  if (!(0, _utilsBYAfe1GS.c)(value)) return;
  const compatibilityMigrationPaths = (0, _stringNormalizationDEQ0pIpk.l)(value.compatibilityMigrationPaths);
  const compatibilityRuntimePaths = (0, _stringNormalizationDEQ0pIpk.l)(value.compatibilityRuntimePaths);
  const rawSecretInputs = (0, _utilsBYAfe1GS.c)(value.secretInputs) ? value.secretInputs : void 0;
...
function normalizeManifestModelSupport(value) {
  if (!(0, _utilsBYAfe1GS.c)(value)) return;
  const modelPrefixes = (0, _stringNormalizationDEQ0pIpk.l)(value.modelPrefixes);
  const modelPatterns = (0, _stringNormalizationDEQ0pIpk.l)(value.modelPatterns);
  const modelSupport = {
...
function normalizeManifestProviderEndpoints(value) {
  ...
  for (const rawEndpoint of value) {
    if (!(0, _utilsBYAfe1GS.c)(rawEndpoint)) continue;
    const endpointClass = (0, _stringCoerceBje8XVt.c)(rawEndpoint.endpointClass);
    if (!endpointClass) continue;
    const hosts = (0, _stringNormalizationDEQ0pIpk.l)(rawEndpoint.hosts).map((host) => host.toLowerCase());
    const baseUrls = (0, _stringNormalizationDEQ0pIpk.l)(rawEndpoint.baseUrls);
...
function normalizeManifestActivation(value) {
  if (!(0, _utilsBYAfe1GS.c)(value)) return;
  const onProviders = (0, _stringNormalizationDEQ0pIpk.l)(value.onProviders);
  const onAgentHarnesses = (0, _stringNormalizationDEQ0pIpk.l)(value.onAgentHarnesses);
  const onCommands = (0, _stringNormalizationDEQ0pIpk.l)(value.onCommands);
  const onChannels = (0, _stringNormalizationDEQ0pIpk.l)(value.onChannels);
  const onRoutes = (0, _stringNormalizationDEQ0pIpk.l)(value.onRoutes);
  const onCapabilities = (0, _stringNormalizationDEQ0pIpk.l)(value.onCapabilities).filter((capability) => capability === "provider" || capability === "channel" || capability === "tool" || capability === "hook");
  const activation = {
...
function normalizeManifestSetupProviders(value) {
  ...
  for (const entry of value) {
    ...
    if (!id) continue;
    const authMethods = (0, _stringNormalizationDEQ0pIpk.l)(entry.authMethods);
    const envVars = (0, _stringNormalizationDEQ0pIpk.l)(entry.envVars);
    normalized.push({
      id,
...
function normalizeManifestSetup(value) {
  ...
  const cliBackends = (0, _stringNormalizationDEQ0pIpk.l)(value.cliBackends);
  const configMigrations = (0, _stringNormalizationDEQ0pIpk.l)(value.configMigrations);
...
function normalizeProviderAuthChoices(value) {
  ...
  for (const entry of value) {
    ...
    const deprecatedChoiceIds = (0, _stringNormalizationDEQ0pIpk.l)(entry.deprecatedChoiceIds);
    ...
    const onboardingScopes = (0, _stringNormalizationDEQ0pIpk.l)(entry.onboardingScopes).filter((scope) => scope === "text-inference" || scope === "image-generation");
    normalized.push({
      provider,
      method,
      choiceId,
...
function normalizeChannelConfigs(value) {
  ...
  for (const [key, rawEntry] of Object.entries(value)) {
    ...
    if (!channelId || (0, _prototypeKeysMkcCRiyK.t)(channelId) || !(0, _utilsBYAfe1GS.c)(rawEntry)) continue;
    const schema = (0, _utilsBYAfe1GS.c)(rawEntry.schema) ? rawEntry.schema : null;
    if (!schema) continue;
    const uiHints = (0, _utilsBYAfe1GS.c)(rawEntry.uiHints) ? rawEntry.uiHints : void 0;
    const runtime = (0, _utilsBYAfe1GS.c)(rawEntry.runtime) && typeof rawEntry.runtime.safeParse === "function" ? rawEntry.runtime : void 0;
    const label = (0, _stringCoerceBje8XVt.c)(rawEntry.label) ?? "";
    const description = (0, _stringCoerceBje8XVt.c)(rawEntry.description) ?? "";
    const preferOver = (0, _stringNormalizationDEQ0pIpk.l)(rawEntry.preferOver);
...
function loadPluginManifest(rootDir, rejectHardlinks = true) {
  ...
  const opened = (0, _boundaryFileReadCgZlf7kr.i)({
  ...
  if (!opened.ok) return (0, _boundaryFileReadCgZlf7kr.n)(opened, {
    path: () => ({
      ok: false,
  ...
  if (!(0, _utilsBYAfe1GS.c)(raw)) return {
    ok: false,
  ...
  const configSchema = (0, _utilsBYAfe1GS.c)(raw.configSchema) ? raw.configSchema : null;
  ...
  const legacyPluginIds = (0, _stringNormalizationDEQ0pIpk.l)(raw.legacyPluginIds);
  const autoEnableWhenConfiguredProviders = (0, _stringNormalizationDEQ0pIpk.l)(raw.autoEnableWhenConfiguredProviders);
  ...
  const channels = (0, _stringNormalizationDEQ0pIpk.l)(raw.channels);
  const providers = (0, _stringNormalizationDEQ0pIpk.l)(raw.providers);
  ...
  const cliBackends = (0, _stringNormalizationDEQ0pIpk.l)(raw.cliBackends);
  const syntheticAuthRefs = (0, _stringNormalizationDEQ0pIpk.l)(raw.syntheticAuthRefs);
  const nonSecretAuthMarkers = (0, _stringNormalizationDEQ0pIpk.l)(raw.nonSecretAuthMarkers);
  ...
  const skills = (0, _stringNormalizationDEQ0pIpk.l)(raw.skills);
  ...
  let uiHints;
  if ((0, _utilsBYAfe1GS.c)(raw.uiHints)) uiHints = raw.uiHints;
  return {
    ok: true,
    manifest: {
      id,
...
function getPackageManifestMetadata(manifest) {
  if (!manifest) return;
  return manifest[_legacyNamesDfDGLO9Q.n];
}
