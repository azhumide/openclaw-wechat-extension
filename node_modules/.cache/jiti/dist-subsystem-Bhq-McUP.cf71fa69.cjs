...
var _idsDUzZX4p = require("./ids-DUzZX4p8.js");
var _runtimeDv8n03pi = require("./runtime-Dv8n03pi.js");
var _logger7EZMSIuN = require("./logger-7EZMSIuN.js");
var _consoleBTtQ75qF = require("./console-BTtQ75qF.js");
...
function shouldLogToConsole(level, settings) {
  ...
  return (0, _logger7EZMSIuN._)(level) >= (0, _logger7EZMSIuN._)(settings.level);
}
const inspectValue = (() => {
...
function isChannelSubsystemPrefix(value) {
  ...
  return (0, _idsDUzZX4p.r)(normalized) === normalized || normalized === "webchat";
}
...
function formatConsoleLine(opts) {
  ...
  if (opts.style === "json") return (0, _redact1fZUZMlV.i)(JSON.stringify({
    time: (0, _consoleBTtQ75qF.n)("json"),
    level: opts.level,
  ...
  return `${[(() => {
    if (opts.style === "pretty") return color.gray((0, _consoleBTtQ75qF.n)("pretty"));
    if (_logger7EZMSIuN.h.consoleTimestampPrefix) return color.gray((0, _consoleBTtQ75qF.n)(opts.style));
    return "";
...
function writeConsoleLine(level, line, opts = {}) {
  ...
  const sink = _logger7EZMSIuN.h.rawConsole ?? console;
  if (_logger7EZMSIuN.h.forceConsoleToStderr || level === "error" || level === "fatal") (sink.error ?? console.error)(redacted);else
...
function shouldSuppressProbeConsoleLine(params) {
  if ((0, _logger7EZMSIuN.b)()) return false;
...
function createSubsystemLogger(subsystem) {
  ...
  const emitLog = (level, message, meta) => {
    const consoleSettings = (0, _consoleBTtQ75qF.r)();
    const consoleEnabled = shouldLogToConsole(level, { level: consoleSettings.level }) && (0, _consoleBTtQ75qF.l)(resolvedSubsystem);
    const fileEnabled = (0, _logger7EZMSIuN.s)(level);
    ...
    if (fileEnabled) logToFile((0, _logger7EZMSIuN.i)({ subsystem: resolvedSubsystem }), level, message, fileMeta);
  ...
  return {
    ...
    isEnabled(level, target = "any") {
      const isConsoleEnabled = shouldLogToConsole(level, { level: (0, _consoleBTtQ75qF.r)().level }) && (0, _consoleBTtQ75qF.l)(resolvedSubsystem);
      const isFileEnabled = (0, _logger7EZMSIuN.s)(level);
    ...
    raw(message) {
      if ((0, _logger7EZMSIuN.s)("info")) logToFile((0, _logger7EZMSIuN.i)({ subsystem: resolvedSubsystem }), "info", message, { raw: true });
      if (shouldLogToConsole("info", { level: (0, _consoleBTtQ75qF.r)().level }) && (0, _consoleBTtQ75qF.l)(resolvedSubsystem)) {
        if (shouldSuppressProbeConsoleLine({
          level: "info",
          subsystem: resolvedSubsystem,
          message
...
function runtimeForLogger(logger, exit = _runtimeDv8n03pi.n.exit) {
  return {
    log(...args) {
...
function createSubsystemRuntime(subsystem, exit = _runtimeDv8n03pi.n.exit) {
  return runtimeForLogger(createSubsystemLogger(subsystem), exit);
}
//#endregion /* v9-5fa4cffd463f94d3 */
