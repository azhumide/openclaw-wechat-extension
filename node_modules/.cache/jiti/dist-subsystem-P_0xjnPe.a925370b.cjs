"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = enableConsoleCapture;exports.c = routeLogsToStderr;exports.d = setConsoleTimestampPrefix;exports.f = shouldLogSubsystemToConsole;exports.i = stripRedundantSubsystemPrefixForConsole;exports.l = setConsoleConfigLoaderForTests;exports.n = createSubsystemRuntime;exports.o = getConsoleSettings;exports.r = runtimeForLogger;exports.s = getResolvedConsoleSettings;exports.t = createSubsystemLogger;exports.u = setConsoleSubsystemFilter;var _redactDGGawMpx = require("./redact-DGGawMpx.js");
var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _progressLineSEdbgtl = require("./progress-line-S-edbgtl.js");
var _idsBNV8Hqfu = require("./ids-BNV8Hqfu.js");
var _loggerDwIqATVO = require("./logger-DwIqATVO.js");
var _runtimeBcHHbzZw = require("./runtime-BcHHbzZw.js");
var _ansiCSbP14w = require("./ansi-cSbP14w0.js");
...
function resolveConsoleSettings() {
  const envLevel = (0, _loggerDwIqATVO.m)();
  if (process.env.VITEST === "true" && process.env.OPENCLAW_TEST_CONSOLE !== "1" && !(0, _loggerDwIqATVO.b)() && !envLevel && !_loggerDwIqATVO.h.overrideSettings) return {
    level: "silent",
    style: normalizeConsoleStyle(void 0)
  };
  let cfg = _loggerDwIqATVO.h.overrideSettings ?? (0, _redactDGGawMpx.o)();
  if (!cfg && !(0, _redactDGGawMpx.s)()) if (_loggerDwIqATVO.h.resolvingConsoleSettings) cfg = void 0;else
  {
    _loggerDwIqATVO.h.resolvingConsoleSettings = true;
    try {
      cfg = loadConfigFallback();
    } finally {
      _loggerDwIqATVO.h.resolvingConsoleSettings = false;
    }
  }
  return {
...
function getConsoleSettings() {
  ...
  const cached = _loggerDwIqATVO.h.cachedConsoleSettings;
  if (!cached || consoleSettingsChanged(cached, settings)) _loggerDwIqATVO.h.cachedConsoleSettings = settings;
  return _loggerDwIqATVO.h.cachedConsoleSettings;
}
function getResolvedConsoleSettings() {
  return getConsoleSettings();
}
function routeLogsToStderr() {
  _loggerDwIqATVO.h.forceConsoleToStderr = true;
}
function setConsoleSubsystemFilter(filters) {
  if (!filters || filters.length === 0) {
    _loggerDwIqATVO.h.consoleSubsystemFilter = null;
    return;
  }
  const normalized = filters.map((value) => value.trim()).filter((value) => value.length > 0);
  _loggerDwIqATVO.h.consoleSubsystemFilter = normalized.length > 0 ? normalized : null;
}
function setConsoleTimestampPrefix(enabled) {
  _loggerDwIqATVO.h.consoleTimestampPrefix = enabled;
}
...
function shouldLogSubsystemToConsole(subsystem) {
  const filter = _loggerDwIqATVO.h.consoleSubsystemFilter;
...
function shouldSuppressConsoleMessage(message) {
  if ((0, _loggerDwIqATVO.b)()) return false;
...
function formatConsoleTimestamp(style) {
  ...
  if (style === "pretty") return (0, _loggerDwIqATVO.f)(now, { style: "short" }).replace(/[+-]\d{2}:\d{2}$/, "");
  return (0, _loggerDwIqATVO.d)(now);
}
...
function enableConsoleCapture() {
  if (_loggerDwIqATVO.h.consolePatched) return;
  _loggerDwIqATVO.h.consolePatched = true;
  if (!_loggerDwIqATVO.h.streamErrorHandlersInstalled) {
    _loggerDwIqATVO.h.streamErrorHandlersInstalled = true;
  ...
  let logger = null;
  const getLoggerLazy = () => {
    if (!logger) logger = (0, _loggerDwIqATVO.a)();
    return logger;
  };
  const original = {
    log: console.log,
  ...
  const forward = (level, orig) => (...args) => {
    ...
    const trimmed = (0, _ansiCSbP14w.r)(formatted).trimStart();
    const timestamp = _loggerDwIqATVO.h.consoleTimestampPrefix && trimmed.length > 0 && !hasTimestampPrefix(trimmed) ? formatConsoleTimestamp(getConsoleSettings().style) : "";
    try {
    ...
    if (_loggerDwIqATVO.h.forceConsoleToStderr) try {
      const redacted = (0, _redactDGGawMpx.r)(formatted);
...
function shouldLogToConsole(level, settings) {
  ...
  return (0, _loggerDwIqATVO._)(level) >= (0, _loggerDwIqATVO._)(settings.level);
}
const inspectValue = (() => {
...
function isChannelSubsystemPrefix(value) {
  ...
  return (0, _idsBNV8Hqfu.r)(normalized) === normalized || normalized === "webchat";
}
...
function formatConsoleLine(opts) {
  ...
  return `${[(() => {
    ...
    if (_loggerDwIqATVO.h.consoleTimestampPrefix) return color.gray(formatConsoleTimestamp(opts.style));
    return "";
  })(), prefixColor(prefix)].filter(Boolean).join(" ")} ${levelColor(displayMessage)}`;
}
function writeConsoleLine(level, line) {
  (0, _progressLineSEdbgtl.t)();
  const sanitized = process.platform === "win32" && process.env.GITHUB_ACTIONS === "true" ? line.replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "?").replace(/[\uD800-\uDFFF]/g, "?") : line;
  const sink = _loggerDwIqATVO.h.rawConsole ?? console;
  if (_loggerDwIqATVO.h.forceConsoleToStderr || level === "error" || level === "fatal") (sink.error ?? console.error)(sanitized);else
...
function createSubsystemLogger(subsystem) {
  ...
  const emitLog = (level, message, meta) => {
    ...
    const fileEnabled = (0, _loggerDwIqATVO.s)(level);
    ...
    if (fileEnabled) logToFile((0, _loggerDwIqATVO.i)({ subsystem: resolvedSubsystem }), level, message, fileMeta);
  ...
  return {
    ...
    isEnabled(level, target = "any") {
      ...
      const isFileEnabled = (0, _loggerDwIqATVO.s)(level);
    ...
    raw(message) {
      if ((0, _loggerDwIqATVO.s)("info")) logToFile((0, _loggerDwIqATVO.i)({ subsystem: resolvedSubsystem }), "info", message, { raw: true });
...
function runtimeForLogger(logger, exit = _runtimeBcHHbzZw.n.exit) {
  return {
    log(...args) {
...
function createSubsystemRuntime(subsystem, exit = _runtimeBcHHbzZw.n.exit) {
  return runtimeForLogger(createSubsystemLogger(subsystem), exit);
}
//#endregion /* v9-f07a0f7095cc24e4 */
