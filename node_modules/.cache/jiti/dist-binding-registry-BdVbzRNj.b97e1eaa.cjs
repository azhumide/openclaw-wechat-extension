...
var _registryD_T9eteF = require("./registry-D_T9eteF.js");
require("./plugins-DMPVpaLC.js");
var _bindingsC_6djqHo = require("./bindings-C_6djqHo.js");
var _resolveRouteUGoWkLGV = require("./resolve-route-uGoWkLGV.js");
...
function buildConfiguredAcpSessionKey(spec) {
  ...
  return `agent:${(0, _sessionKeyBSlq04Dw.f)(spec.agentId)}:acp:binding:${spec.channel}:${spec.accountId}:${hash}`;
}
...
function parseConfiguredAcpSessionKey(sessionKey) {
  ...
  return {
    channel,
    accountId: (0, _accountIdCKs_R3Vn.n)(tokens[3] ?? "default")
  };
}
function resolveConfiguredAcpBindingSpecFromRecord(record) {
  ...
  const agentId = (0, _stringCoerceBje8XVt.c)(record.metadata?.agentId) ?? (0, _sessionKeyBSlq04Dw.u)(record.targetSessionKey);
  if (!agentId) return null;
  return {
    channel: record.conversation.channel,
    accountId: (0, _accountIdCKs_R3Vn.n)(record.conversation.accountId),
    conversationId,
...
function resolveConfiguredBindingWorkspaceCwd(params) {
  if ((0, _stringCoerceBje8XVt.c)((0, _agentScopeBBUKdu9R.v)(params.cfg, params.agentId)?.workspace)) return (0, _agentScopeBBUKdu9R.x)(params.cfg, params.agentId);
  if (params.agentId === (0, _agentScopeBBUKdu9R.S)(params.cfg)) {
    if ((0, _stringCoerceBje8XVt.c)(params.cfg.agents?.defaults?.workspace)) return (0, _agentScopeBBUKdu9R.x)(params.cfg, params.agentId);
  }
}
...
function resolveLoadedChannelPlugin(channel) {
  ...
  return (0, _registryD_T9eteF.t)(normalized);
}
...
function compileConfiguredBindingRule(params) {
  const agentId = (0, _resolveRouteUGoWkLGV.r)(params.cfg, params.binding.agentId ?? "main");
...
function compileConfiguredBindingRegistry(params) {
  ...
  for (const binding of (0, _bindingsC_6djqHo.r)(params.cfg)) {
...
function resolveAccountMatchPriority(match, actual) {
  ...
  if (trimmed === "*") return 1;
  return (0, _accountIdCKs_R3Vn.n)(trimmed) === actual ? 2 : 0;
}
...
function toConfiguredBindingConversationRef(conversation) {
  ...
  return {
    channel,
    accountId: (0, _accountIdCKs_R3Vn.n)(conversation.accountId),
    conversationId,
...
function materializeConfiguredBindingRecord(params) {
  return params.rule.targetFactory.materialize({
    accountId: (0, _accountIdCKs_R3Vn.n)(params.accountId),
