...
var _registryYTN80EXi = require("./registry-yTN80EXi.js");
require("./plugins-D-K4uCVI.js");
var _runtimeDf1Zlb_ = require("./runtime-Df1Zlb_-.js");
var _bindingsCW4fwMhh = require("./bindings-CW4fwMhh.js");
var _resolveRouteC2h77Rnt = require("./resolve-route-C2h77Rnt.js");
...
function buildConfiguredAcpSessionKey(spec) {
  ...
  return `agent:${(0, _sessionKeyHxP9B3Or.f)(spec.agentId)}:acp:binding:${spec.channel}:${spec.accountId}:${hash}`;
}
...
function parseConfiguredAcpSessionKey(sessionKey) {
  ...
  return {
    channel,
    accountId: (0, _accountIdDhSD7lhD.n)(tokens[3] ?? "default")
  };
}
function resolveConfiguredAcpBindingSpecFromRecord(record) {
  ...
  const agentId = (0, _stringCoerceBje8XVt.c)(record.metadata?.agentId) ?? (0, _sessionKeyHxP9B3Or.u)(record.targetSessionKey);
  if (!agentId) return null;
  return {
    channel: record.conversation.channel,
    accountId: (0, _accountIdDhSD7lhD.n)(record.conversation.accountId),
    conversationId,
...
function resolveConfiguredBindingWorkspaceCwd(params) {
  if ((0, _stringCoerceBje8XVt.c)((0, _agentScopeI10se9ty.v)(params.cfg, params.agentId)?.workspace)) return (0, _agentScopeI10se9ty.x)(params.cfg, params.agentId);
  if (params.agentId === (0, _agentScopeI10se9ty.S)(params.cfg)) {
    if ((0, _stringCoerceBje8XVt.c)(params.cfg.agents?.defaults?.workspace)) return (0, _agentScopeI10se9ty.x)(params.cfg, params.agentId);
  }
}
...
function resolveLoadedChannelPlugin(channel) {
  ...
  return (0, _registryYTN80EXi.t)(normalized);
}
...
function compileConfiguredBindingRule(params) {
  const agentId = (0, _resolveRouteC2h77Rnt.r)(params.cfg, params.binding.agentId ?? "main");
...
function compileConfiguredBindingRegistry(params) {
  ...
  for (const binding of (0, _bindingsCW4fwMhh.r)(params.cfg)) {
...
function resolveCompiledBindingRegistry(cfg) {
  const activeRegistry = (0, _runtimeDf1Zlb_.g)();
  const registryVersion = (0, _runtimeDf1Zlb_.n)();
...
function primeCompiledBindingRegistry(cfg) {
  ...
  compiledRegistryCache.set(cfg, {
    registryRef: activeRegistry,
    registryVersion: (0, _runtimeDf1Zlb_.n)(),
    registry
  });
  return registry;
}
...
function resolveAccountMatchPriority(match, actual) {
  ...
  if (trimmed === "*") return 1;
  return (0, _accountIdDhSD7lhD.n)(trimmed) === actual ? 2 : 0;
}
...
function toConfiguredBindingConversationRef(conversation) {
  ...
  return {
    channel,
    accountId: (0, _accountIdDhSD7lhD.n)(conversation.accountId),
    conversationId,
...
function materializeConfiguredBindingRecord(params) {
  return params.rule.targetFactory.materialize({
    accountId: (0, _accountIdDhSD7lhD.n)(params.accountId),
