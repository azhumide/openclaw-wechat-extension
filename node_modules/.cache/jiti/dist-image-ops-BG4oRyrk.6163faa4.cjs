...
const mediaAttachmentImageOpsLoader = (0, _lazyPromiseBWWZqHu.n)(async () => {
  const { loadBundledPluginPublicArtifactModuleSync } = await Promise.resolve().then(() => jitiImport("./public-surface-loader-DbE_PdYS.js").then((m) => _interopRequireWildcard(m)));
...
async function withTempDir(fn) {
  const dir = await _promises.default.mkdtemp(_nodePath.default.join((0, _tmpOpenclawDirCN8v1wId.n)(), "openclaw-img-"));
  try {
    return await fn(dir);
  } finally {
...
async function sipsMetadataFromBuffer(buffer) {
  return await withTempDir(async (dir) => {
    ...
    const { stdout } = await (0, _execDIR5K8F.i)("/usr/bin/sips", [
    "-g",
    "pixelWidth",
    "-g",
    "pixelHeight",
...
async function sipsResizeToJpeg(params) {
  return await withTempDir(async (dir) => {
    ...
    await (0, _execDIR5K8F.i)("/usr/bin/sips", [
    "-Z",
