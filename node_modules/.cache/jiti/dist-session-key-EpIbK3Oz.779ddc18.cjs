...
function scopedHeartbeatWakeOptions(sessionKey, wakeOptions) {
  return (0, _sessionKeyUtilsBT0y7mVK.o)(sessionKey) ? {
    ...wakeOptions,
    sessionKey
  } : wakeOptions;
}
function normalizeMainKey(value) {
  return (0, _stringCoerceC1IzJjqi.a)(value) || "main";
}
function toAgentRequestSessionKey(storeKey) {
  const raw = (storeKey ?? "").trim();
  if (!raw) return;
  return (0, _sessionKeyUtilsBT0y7mVK.o)(raw)?.rest ?? raw;
}
function toAgentStoreSessionKey(params) {
  const raw = (params.requestKey ?? "").trim();
  const lowered = (0, _stringCoerceC1IzJjqi.a)(raw);
  if (!raw || lowered === "main") return buildAgentMainSessionKey({
    agentId: params.agentId,
    mainKey: params.mainKey
  });
  const parsed = (0, _sessionKeyUtilsBT0y7mVK.o)(raw);
  if (parsed) return `agent:${parsed.agentId}:${parsed.rest}`;
  if (lowered.startsWith("agent:")) return lowered;
  return `agent:${normalizeAgentId(params.agentId)}:${lowered}`;
}
function resolveAgentIdFromSessionKey(sessionKey) {
  return normalizeAgentId((0, _sessionKeyUtilsBT0y7mVK.o)(sessionKey)?.agentId ?? "main");
}
function classifySessionKeyShape(sessionKey) {
  const raw = (sessionKey ?? "").trim();
  if (!raw) return "missing";
  if ((0, _sessionKeyUtilsBT0y7mVK.o)(raw)) return "agent";
  return (0, _stringCoerceC1IzJjqi.a)(raw).startsWith("agent:") ? "malformed_agent" : "legacy_or_alias";
}
function normalizeAgentId(value) {
  const trimmed = (value ?? "").trim();
  if (!trimmed) return DEFAULT_AGENT_ID;
  const normalized = (0, _stringCoerceC1IzJjqi.a)(trimmed);
...
function buildAgentPeerSessionKey(params) {
  ...
  if (peerKind === "direct") {
    ...
    peerId = (0, _stringCoerceC1IzJjqi.a)(peerId);
    if (dmScope === "per-account-channel-peer" && peerId) {
      const channel = (0, _stringCoerceC1IzJjqi.a)(params.channel) || "unknown";
  ...
  const channel = (0, _stringCoerceC1IzJjqi.a)(params.channel) || "unknown";
  const peerId = (0, _stringCoerceC1IzJjqi.a)(params.peerId) || "unknown";
...
function resolveThreadSessionKeys(params) {
  ...
  const normalizedThread = params.normalizeThreadId?.(threadId) ?? (0, _stringCoerceC1IzJjqi.a)(threadId);
  return {
    sessionKey: params.useSuffix ?? true ? `${params.baseSessionKey}:thread:${normalizedThread}` : params.baseSessionKey,
    parentSessionKey: params.parentSessionKey
  };
}
//#endregion /* v9-1ab554a814accba1 */
