"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = listBundledChannelCatalogEntries;exports.n = void 0;exports.r = normalizeChatChannelId;exports.t = void 0;var _stringCoerceC1IzJjqi = require("./string-coerce-C1IzJjqi.js");
...
function listPackageRoots() {
  return [(0, _openclawRootBNWw3cXT.n)({ cwd: process.cwd() }), (0, _openclawRootBNWw3cXT.n)({ moduleUrl: "file:///home/rs/openclaw/dist/ids-2bZIXJNM.js" })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index);
}
function listBundledExtensionPackageJsonPaths(env = process.env) {
  const extensionsRoot = (0, _bundledDirDXKS8sgC.t)(env);
...
function toBundledChannelEntry(entry) {
  ...
  const id = (0, _stringCoerceC1IzJjqi.s)(channel?.id);
  if (!id || !channel) return null;
  return {
    id,
    channel,
    aliases: Array.isArray(channel.aliases) ? channel.aliases.map((alias) => (0, _stringCoerceC1IzJjqi.s)(alias)).filter((alias) => Boolean(alias)) : [],
...
function listBundledChatChannelEntries() {
  return listBundledChannelCatalogEntries().map((entry) => ({
    id: (0, _stringCoerceC1IzJjqi.s)(entry.id) ?? entry.id,
