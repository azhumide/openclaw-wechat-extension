...
var _globalsDAPTRKx = require("./globals-DAPTR-Kx.js");
var _loggerC44RY = require("./logger-C4--4RY8.js");
var _accountIdVYgQddVH = require("./account-id-vYgQddVH.js");
...
var _bindingsCw7Va99C = require("./bindings-Cw7Va99C.js");
var _peerKindMatch7VvW1n1I = require("./peer-kind-match-7VvW1n1I.js");
...
function normalizeId(value) {
  return (0, _bindingScopeBDwz_qH.n)(value);
}
function buildAgentSessionKey(params) {
  const channel = normalizeToken(params.channel) || "unknown";
  const peer = params.peer;
  return (0, _sessionKeyBd0xquXF.i)({
    agentId: params.agentId,
    mainKey: _sessionKeyBd0xquXF.n,
    channel,
...
function resolveAgentLookupCache(cfg) {
  ...
  for (const agent of listAgents(cfg)) {
    ...
    if (!rawId) continue;
    byNormalizedId.set((0, _sessionKeyBd0xquXF.c)(rawId), (0, _sessionKeyBd0xquXF.f)(rawId));
  }
  const next = {
    agentsRef,
    byNormalizedId,
    fallbackDefaultAgentId: (0, _sessionKeyBd0xquXF.f)((0, _agentScopeDf_s1jDI.S)(cfg))
  };
...
function pickFirstExistingAgentId(cfg, agentId) {
  ...
  const normalized = (0, _sessionKeyBd0xquXF.c)(trimmed);
  if (lookup.byNormalizedId.size === 0) return (0, _sessionKeyBd0xquXF.f)(trimmed);
...
function resolveAccountPatternKey(accountPattern) {
  if (!accountPattern.trim()) return _accountIdVYgQddVH.t;
  return (0, _accountIdVYgQddVH.n)(accountPattern);
}
function buildEvaluatedBindingsByChannel(cfg) {
  const byChannel = /* @__PURE__ */new Map();
  let order = 0;
  for (const binding of (0, _bindingsCw7Va99C.n)(cfg)) {
...
function normalizeBindingMatch(match) {
  ...
  return {
    ...
    roles: (0, _bindingScopeBDwz_qH.r)(rawRoles)
  };
}
...
function matchesBindingScope(match, scope) {
  ...
  if (match.peer.state === "valid") {
    if (!scope.peer || !(0, _peerKindMatch7VvW1n1I.t)(match.peer.kind, scope.peer.kind) || scope.peer.id !== match.peer.id) return false;
  }
  if (match.peer.state === "wildcard-kind") {
    if (!scope.peer || !(0, _peerKindMatch7VvW1n1I.t)(match.peer.kind, scope.peer.kind)) return false;
  }
  return (0, _bindingScopeBDwz_qH.a)(match, scope);
}
function resolveAgentRoute(input) {
  const channel = normalizeToken(input.channel);
  const accountId = (0, _accountIdVYgQddVH.n)(input.accountId);
  ...
  const shouldLogDebug = (0, _globalsDAPTRKx.a)();
  ...
  const choose = (agentId, matchedBy, sessionOverride) => {
    ...
    const mainSessionKey = (0, _stringCoerceBje8XVt.a)((0, _sessionKeyBd0xquXF.r)({
      agentId: resolvedAgentId,
      mainKey: _sessionKeyBd0xquXF.n
    }));
    const route = {
  ...
  if (shouldLogDebug) {
    (0, _loggerC44RY.t)(`[routing] resolveAgentRoute: channel=${channel} accountId=${accountId} peer=${formatPeer(peer)} guildId=${guildId || "none"} teamId=${teamId || "none"} bindings=${bindings.length}`);
    for (const entry of bindings) (0, _loggerC44RY.t)(`[routing] binding: agentId=${entry.binding.agentId} accountPattern=${entry.match.accountPattern || "default"} peer=${formatNormalizedPeer(entry.match.peer)} guildId=${entry.match.guildId ?? "none"} teamId=${entry.match.teamId ?? "none"} roles=${entry.match.roles?.length ?? 0}`);
  }
  const baseScope = {
    guildId,
    teamId,
  ...
  for (const tier of tiers) {
    ...
    if (matched) {
      if (shouldLogDebug) (0, _loggerC44RY.t)(`[routing] match: matchedBy=${tier.matchedBy} agentId=${matched.binding.agentId}`);
      return choose(matched.binding.agentId, tier.matchedBy, matched.binding.session);
    }
  }
  return choose((0, _agentScopeDf_s1jDI.S)(input.cfg), "default");
}
//#endregion /* v9-13e4d1bc5a7c3bff */
