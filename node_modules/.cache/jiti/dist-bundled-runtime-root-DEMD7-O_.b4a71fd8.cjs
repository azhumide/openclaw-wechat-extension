"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = waitForBundledRuntimeDepsInstallIdle;exports.a = installBundledRuntimeDeps;exports.c = repairBundledRuntimeDepsInstallRootAsync;exports.d = resolveBundledRuntimeDependencyPackageInstallRootPlan;exports.f = resolveBundledRuntimeDependencyPackageRoot;exports.g = getActiveBundledRuntimeDepsInstallCount;exports.h = withBundledRuntimeDepsFilesystemLock;exports.i = ensureBundledPluginRuntimeDeps;exports.l = resolveBundledRuntimeDependencyInstallRootPlan;exports.m = shouldMaterializeBundledRuntimeMirrorDistFile;exports.n = prepareBundledPluginRuntimeRoot;exports.o = materializeBundledRuntimeMirrorDistFile;exports.p = scanBundledPluginRuntimeDeps;exports.r = createBundledRuntimeDepsWritableInstallSpecs;exports.s = registerBundledRuntimeDependencyNodePath;exports.t = isBuiltBundledPluginRuntimeRoot;exports.u = resolveBundledRuntimeDependencyPackageInstallRoot;exports.v = createLowDiskSpaceWarning;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _homeDirG5LU3LmA = require("./home-dir-g5LU3LmA.js");
var _pathsB2cMKWd = require("./paths-B2cMK-wd.js");
var _npmInstallEnvC2UCLEG = require("./npm-install-env-C2UCLEG0.js");
var _safeTextBsGBhnDf = require("./safe-text-BsGBhnDf.js");
var _configStateBw_lAn0M = require("./config-state-Bw_lAn0M.js");
...
const semver = (0, _nodeModule.createRequire)("file:///home/rs/openclaw/dist/bundled-runtime-root-DEMD7-O_.js")("semver");
...
function materializeBundledRuntimeMirrorDistFile(sourcePath, targetPath) {
  ...
  try {
    if (_nodeFs.default.realpathSync(sourcePath) === _nodeFs.default.realpathSync(targetPath) && !_nodeFs.default.lstatSync(targetPath).isSymbolicLink()) return;
  } catch {}
...
function collectMirroredPackageRuntimeDeps(packageRoot) {
  ...
  const packageJson = readJsonObject(_nodePath.default.join(packageRoot, "package.json"));
...
function resolveBundledRuntimeDepsExternalBaseDirs(env) {
  ...
  if (explicit) {
    const roots = explicit.split(_nodePath.default.delimiter).map((entry) => entry.trim()).filter((entry) => entry.length > 0).map((entry) => _nodePath.default.resolve((0, _homeDirG5LU3LmA.r)(entry, {
      env,
    ...
    if (roots.length > 0) {
      ...
      for (const root of roots) {
        const existingIndex = uniqueRoots.findIndex((entry) => _nodePath.default.resolve(entry) === _nodePath.default.resolve(root));
  ...
  return [_nodePath.default.join((0, _pathsB2cMKWd._)(env, _nodeOs.default.homedir), "plugin-runtime-deps")];
}
...
function resolveExternalBundledRuntimeDepsInstallRoots(params) {
  ...
  return resolveBundledRuntimeDepsExternalBaseDirs(params.env).map((baseDir) => _nodePath.default.join(baseDir, packageKey));
}
function resolveExistingExternalBundledRuntimeDepsRoots(params) {
  ...
  for (const externalBaseDir of externalBaseDirs) {
    const relative = _nodePath.default.relative(_nodePath.default.resolve(externalBaseDir), packageRoot);
    if (relative === "" || relative.startsWith("..") || _nodePath.default.isAbsolute(relative) || relative.includes(_nodePath.default.sep)) continue;
    const packageKey = _nodePath.default.basename(packageRoot);
    return packageKey.startsWith("openclaw-") ? externalBaseDirs.map((baseDir) => _nodePath.default.join(baseDir, packageKey)) : null;
  }
  return null;
}
...
function dependencyPackageDir(rootDir, depName) {
  const normalizedDepName = normalizeInstallableRuntimeDepName(depName);
  if (!normalizedDepName) throw new Error(`Invalid bundled runtime dependency name: ${depName}`);
  return _nodePath.default.join(rootDir, "node_modules", ...normalizedDepName.split("/"));
}
function createBundledRuntimeDepsInstallRootPlan(params) {
  const searchRoots = [];
  for (const root of params.searchRoots) {
    const resolved = _nodePath.default.resolve(root);
    if (!searchRoots.some((entry) => _nodePath.default.resolve(entry) === resolved)) searchRoots.push(root);
  }
  if (!searchRoots.some((entry) => _nodePath.default.resolve(entry) === _nodePath.default.resolve(params.installRoot))) searchRoots.push(params.installRoot);
  return {
    installRoot: params.installRoot,
    searchRoots,
...
function createBundledRuntimeDepsWritableInstallSpecs(params) {
  const readOnlyRoots = params.searchRoots.filter((rootDir) => _nodePath.default.resolve(rootDir) !== _nodePath.default.resolve(params.installRoot));
...
function linkBundledRuntimeDepsFromSearchRoots(params) {
  for (const dep of params.deps) {
    ...
    if (!sourceRoot || _nodePath.default.resolve(sourceRoot) === _nodePath.default.resolve(params.installRoot)) continue;
    const sourceDir = dependencyPackageDir(sourceRoot, dep.name);
    const targetDir = dependencyPackageDir(params.installRoot, dep.name);
    _nodeFs.default.mkdirSync(_nodePath.default.dirname(targetDir), { recursive: true });
    _nodeFs.default.rmSync(targetDir, {
      recursive: true,
      force: true
    });
    try {
      _nodeFs.default.symlinkSync(sourceDir, targetDir, process.platform === "win32" ? "junction" : "dir");
    } catch {
      _nodeFs.default.cpSync(sourceDir, targetDir, { recursive: true });
    }
  }
}
...
function createBundledRuntimeDepsInstallEnv(env, options = {}) {
  return {
    ...(0, _npmInstallEnvC2UCLEG.t)(env, options),
...
function isBundledPluginConfiguredForRuntimeDeps(params) {
  const plugins = (0, _configStateBw_lAn0M.o)(params.config.plugins);
  ...
  return {
    ...
    missing: allDeps.filter((dep) => {
      ...
      return dep.pluginIds.every((pluginId) => {
        return !hasDependencySentinel(resolveBundledRuntimeDependencyInstallRootPlan(_nodePath.default.join(extensionsDir, pluginId), { env: params.env }).searchRoots, dep);
      });
    }),
    conflicts
  };
...
function emitBundledRuntimeDepsOutputProgress(chunk, stream, onProgress) {
  ...
  const lines = chunk.toString("utf8").split(/\r\n|\n|\r/u).map((line) => (0, _safeTextBsGBhnDf.t)(line).trim()).filter((line) => line.length > 0).slice(-3);
...
async function withBundledRuntimeDepsInstallRootLockAsync(installRoot, run) {
  _nodeFs.default.mkdirSync(installRoot, { recursive: true });
  const lockDir = _nodePath.default.join(installRoot, BUNDLED_RUNTIME_DEPS_LOCK_DIR);
  const startedAt = Date.now();
  let locked = false;
  while (!locked) try {
    _nodeFs.default.mkdirSync(lockDir);
...
function ensureBundledPluginRuntimeDeps(params) {
  ...
  const packageRuntimeDeps = packageRoot && _nodePath.default.resolve(installRoot) !== _nodePath.default.resolve(params.pluginRoot) ? collectMirroredPackageRuntimeDeps(packageRoot) : [];
  ...
  return withBundledRuntimeDepsInstallRootLock(installRoot, () => {
    ...
    const readonlySearchRoots = installRootPlan.searchRoots.filter((rootDir) => _nodePath.default.resolve(rootDir) !== _nodePath.default.resolve(installRoot));
...
function mirrorBundledPluginRuntimeRoot(params) {
  return withBundledRuntimeDepsFilesystemLock(params.installRoot, BUNDLED_RUNTIME_MIRROR_LOCK_DIR, () => {
    ...
    if (_nodePath.default.resolve(mirrorRoot) === _nodePath.default.resolve(params.pluginRoot)) return mirrorRoot;
