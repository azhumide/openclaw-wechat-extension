"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveInboundLastRouteSessionKey;exports.i = resolveAgentRoute;exports.n = deriveLastRoutePolicy;exports.r = pickFirstExistingAgentId;exports.t = buildAgentSessionKey;var _stringCoerceCjxCKZ6B = require("./string-coerce-CjxCKZ6B.js");
var _globalsBo8QPRgM = require("./globals-Bo8QPRgM.js");
var _loggerB_WyBdYd = require("./logger-B_WyBdYd.js");
...
var _chatTypeTqHfPwy = require("./chat-type-tqHfPwy6.js");
var _bindingScopeC4QyuU1o = require("./binding-scope-C4QyuU1o.js");
var _bindingsChivNKyZ = require("./bindings-ChivNKyZ.js");
var _peerKindMatchMfX5HPLO = require("./peer-kind-match-MfX5HPLO.js");
...
function normalizeId(value) {
  return (0, _bindingScopeC4QyuU1o.n)(value);
}
function buildAgentSessionKey(params) {
  const channel = normalizeToken(params.channel) || "unknown";
  const peer = params.peer;
  return (0, _sessionKeyCn2QoOyX.i)({
    agentId: params.agentId,
    mainKey: _sessionKeyCn2QoOyX.n,
    channel,
...
function resolveAgentLookupCache(cfg) {
  ...
  for (const agent of listAgents(cfg)) {
    ...
    if (!rawId) continue;
    byNormalizedId.set((0, _sessionKeyCn2QoOyX.c)(rawId), (0, _sessionKeyCn2QoOyX.f)(rawId));
  }
  const next = {
    agentsRef,
    byNormalizedId,
    fallbackDefaultAgentId: (0, _sessionKeyCn2QoOyX.f)((0, _agentScopeDV_DrBWg.x)(cfg))
  };
...
function pickFirstExistingAgentId(cfg, agentId) {
  ...
  const normalized = (0, _sessionKeyCn2QoOyX.c)(trimmed);
  if (lookup.byNormalizedId.size === 0) return (0, _sessionKeyCn2QoOyX.f)(trimmed);
...
function resolveAccountPatternKey(accountPattern) {
  if (!accountPattern.trim()) return _accountIdBM1T.t;
  return (0, _accountIdBM1T.n)(accountPattern);
}
function buildEvaluatedBindingsByChannel(cfg) {
  const byChannel = /* @__PURE__ */new Map();
  let order = 0;
  for (const binding of (0, _bindingsChivNKyZ.n)(cfg)) {
...
function normalizePeerConstraint(peer) {
  ...
  const kind = (0, _chatTypeTqHfPwy.t)(peer.kind);
...
function normalizeBindingMatch(match) {
  ...
  return {
    ...
    roles: (0, _bindingScopeC4QyuU1o.r)(rawRoles)
  };
}
...
function matchesBindingScope(match, scope) {
  ...
  if (match.peer.state === "valid") {
    if (!scope.peer || !(0, _peerKindMatchMfX5HPLO.t)(match.peer.kind, scope.peer.kind) || scope.peer.id !== match.peer.id) return false;
  }
  if (match.peer.state === "wildcard-kind") {
    if (!scope.peer || !(0, _peerKindMatchMfX5HPLO.t)(match.peer.kind, scope.peer.kind)) return false;
  }
  return (0, _bindingScopeC4QyuU1o.a)(match, scope);
}
function resolveAgentRoute(input) {
  const channel = normalizeToken(input.channel);
  const accountId = (0, _accountIdBM1T.n)(input.accountId);
  const peer = input.peer ? {
    kind: (0, _chatTypeTqHfPwy.t)(input.peer.kind) ?? input.peer.kind,
  ...
  const shouldLogDebug = (0, _globalsBo8QPRgM.a)();
  const parentPeer = input.parentPeer ? {
    kind: (0, _chatTypeTqHfPwy.t)(input.parentPeer.kind) ?? input.parentPeer.kind,
  ...
  const choose = (agentId, matchedBy) => {
    ...
    const sessionKey = (0, _stringCoerceCjxCKZ6B.a)(buildAgentSessionKey({
    ...
    const mainSessionKey = (0, _stringCoerceCjxCKZ6B.a)((0, _sessionKeyCn2QoOyX.r)({
      agentId: resolvedAgentId,
      mainKey: _sessionKeyCn2QoOyX.n
    }));
    const route = {
  ...
  if (shouldLogDebug) {
    (0, _loggerB_WyBdYd.t)(`[routing] resolveAgentRoute: channel=${channel} accountId=${accountId} peer=${formatPeer(peer)} guildId=${guildId || "none"} teamId=${teamId || "none"} bindings=${bindings.length}`);
    for (const entry of bindings) (0, _loggerB_WyBdYd.t)(`[routing] binding: agentId=${entry.binding.agentId} accountPattern=${entry.match.accountPattern || "default"} peer=${formatNormalizedPeer(entry.match.peer)} guildId=${entry.match.guildId ?? "none"} teamId=${entry.match.teamId ?? "none"} roles=${entry.match.roles?.length ?? 0}`);
  }
  const baseScope = {
    guildId,
    teamId,
  ...
  for (const tier of tiers) {
    ...
    if (matched) {
      if (shouldLogDebug) (0, _loggerB_WyBdYd.t)(`[routing] match: matchedBy=${tier.matchedBy} agentId=${matched.binding.agentId}`);
      return choose(matched.binding.agentId, tier.matchedBy);
    }
  }
  return choose((0, _agentScopeDV_DrBWg.x)(input.cfg), "default");
}
//#endregion /* v9-1ecd78b8bdd2d801 */
