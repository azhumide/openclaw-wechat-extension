...
function normalizeExpectedPackageName(value) {
  ...
  if (!expected) return;
  return (0, _discoveryB19Xdk1_.N)(expected)?.name ?? expected;
}
function describePluginInstallSource(install, options) {
  ...
  if (clawhubSpec) {
    const parsed = (0, _clawhubSpecTcp25s.t)(clawhubSpec);
    if (parsed) {
  ...
  if (npmSpec) {
    const parsed = (0, _discoveryB19Xdk1_.N)(npmSpec);
    if (parsed) {
