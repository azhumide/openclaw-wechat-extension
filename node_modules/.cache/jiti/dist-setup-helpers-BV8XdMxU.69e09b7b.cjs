...
function migrateBaseNameToDefaultAccount(params) {
  ...
  if (!defaultAccount.name) accounts[_accountIdBM1T.t] = {
    ...defaultAccount,
    name: baseName
  };
...
function prepareScopedSetupConfig(params) {
  ...
  if (!params.migrateBaseName || (0, _accountIdBM1T.n)(params.accountId) === "default") return namedConfig;
...
function createPatchedAccountSetupAdapter(params) {
  return {
    resolveAccountId: ({ accountId }) => (0, _accountIdBM1T.n)(accountId),
...
function resolveExistingAccountKey(accounts, targetAccountId) {
  for (const existingKey of Object.keys(accounts)) if ((0, _accountIdBM1T.n)(existingKey) === targetAccountId) return existingKey;
...
function moveSingleAccountChannelSectionToDefaultAccount(params) {
  ...
  if (Object.keys(accounts).length > 0) {
    const keysToMove = (0, _setupPromotionHelpersDMOOcn.t)({
      channelKey: params.channelKey,
      channel: base
    });
    if (keysToMove.length === 0) return params.cfg;
    const resolvedTargetAccountKey = resolveExistingAccountKey(accounts, (0, _setupPromotionHelpersDMOOcn.n)({
      channelKey: params.channelKey,
      channel: base
    }));
  ...
  return moveSingleAccountKeysIntoAccount({
    ...
    accounts,
    keysToMove,
    targetAccountId: _accountIdBM1T.t
  });
}
//#endregion /* v9-d83daf2b55213ba7 */
