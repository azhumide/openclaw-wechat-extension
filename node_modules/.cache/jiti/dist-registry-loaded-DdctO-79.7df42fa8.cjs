"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = listLoadedChannelPlugins;exports.t = getLoadedChannelPluginById;var _stringCoerceC1IzJjqi = require("./string-coerce-C1IzJjqi.js");
var _ids2bZIXJNM = require("./ids-2bZIXJNM.js");
var _runtimeChannelStateD5dsafJk = require("./runtime-channel-state-D5dsafJk.js");
require("./registry-B9khhdbq.js");
...
function coerceLoadedChannelPlugin(plugin) {
  const id = (0, _stringCoerceC1IzJjqi.c)(plugin?.id) ?? "";
...
function resolveCachedChannelPlugins() {
  const registry = (0, _runtimeChannelStateD5dsafJk.t)();
  const registryVersion = (0, _runtimeChannelStateD5dsafJk.n)();
  ...
  const sorted = dedupeChannels(channelPlugins).toSorted((a, b) => {
    const indexA = _ids2bZIXJNM.n.indexOf(a.id);
    const indexB = _ids2bZIXJNM.n.indexOf(b.id);
...
function getLoadedChannelPluginById(id) {
  const resolvedId = (0, _stringCoerceC1IzJjqi.c)(id) ?? "";
  if (!resolvedId) return;
  return resolveCachedChannelPlugins().byId.get(resolvedId);
}
//#endregion /* v9-b3beefca340546ba */
