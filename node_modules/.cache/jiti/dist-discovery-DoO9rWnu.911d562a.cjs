"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolvePluginSourceRoots;exports.n = discoverOpenClawPlugins;exports.r = resolvePluginCacheInputs;exports.t = clearPluginDiscoveryCache;var _stringCoerceCjxCKZ6B = require("./string-coerce-CjxCKZ6B.js");
var _utilsCB8xp0O = require("./utils-CB8xp0O4.js");
var _bundledDirU738ay4K = require("./bundled-dir-U738ay4K.js");
var _boundaryPathBSBSMt0u = require("./boundary-path-BSBSMt0u.js");
var _boundaryFileReadBSd9MXPy = require("./boundary-file-read-BSd9MXPy.js");
...
function resolvePluginSourceRoots(params) {
  ...
  const workspaceRoot = params.workspaceDir ? (0, _utilsCB8xp0O.m)(params.workspaceDir, env) : void 0;
  return {
    stock: (0, _bundledDirU738ay4K.t)(env),
    global: _nodePath.default.join((0, _utilsCB8xp0O.f)(env), "extensions"),
...
function resolvePluginCacheInputs(params) {
  ...
  return {
    ...
    loadPaths: (params.loadPaths ?? []).filter((entry) => typeof entry === "string").map((entry) => entry.trim()).filter(Boolean).map((entry) => (0, _utilsCB8xp0O.m)(entry, env))
  };
}
//#endregion
...
function checkSourceEscapesRoot(params) {
  const sourceRealPath = (0, _pathSafetyBa_J5G0M.r)(params.source);
  const rootRealPath = (0, _pathSafetyBa_J5G0M.r)(params.rootDir);
  if (!sourceRealPath || !rootRealPath) return null;
  if ((0, _pathSafetyBa_J5G0M.n)(rootRealPath, sourceRealPath)) return null;
  return {
...
function checkPathStatAndPermissions(params) {
  ...
  for (const targetPath of pathsToCheck) {
    ...
    seen.add(normalized);
    let stat = (0, _pathSafetyBa_J5G0M.i)(targetPath);
    if (!stat) return {
    ...
    if ((modeBits & 2) !== 0 && params.origin === "bundled") try {
      ...
      const repairedStat = (0, _pathSafetyBa_J5G0M.i)(targetPath);
...
function formatCandidateBlockMessage(issue) {
  ...
  if (issue.reason === "path_world_writable") return `blocked plugin candidate: world-writable path (${issue.targetPath}, mode=${(0, _pathSafetyBa_J5G0M.t)(issue.modeBits ?? 0)})`;
...
function isExtensionFile(filePath) {
  ...
  const baseName = (0, _stringCoerceCjxCKZ6B.a)(_nodePath.default.basename(filePath));
...
function shouldIgnoreScannedDirectory(dirName) {
  const normalized = (0, _stringCoerceCjxCKZ6B.a)(dirName);
...
function resolvesToSameDirectory(left, right) {
  ...
  const leftRealPath = (0, _pathSafetyBa_J5G0M.r)(left);
  const rightRealPath = (0, _pathSafetyBa_J5G0M.r)(right);
...
function readPackageManifest(dir, rejectHardlinks = true) {
  const opened = (0, _boundaryFileReadBSd9MXPy.i)({
...
function resolveIdHintManifestId(rootDir, rejectHardlinks) {
  const manifest = (0, _manifestDgyExZJE.i)(rootDir, rejectHardlinks);
...
function addCandidate(params) {
  ...
  const resolvedRoot = (0, _pathSafetyBa_J5G0M.r)(params.rootDir) ?? _nodePath.default.resolve(params.rootDir);
  ...
  params.candidates.push({
    ...
    workspaceDir: params.workspaceDir,
    packageName: (0, _stringCoerceCjxCKZ6B.c)(manifest?.name),
    packageVersion: (0, _stringCoerceCjxCKZ6B.c)(manifest?.version),
    packageDescription: (0, _stringCoerceCjxCKZ6B.c)(manifest?.description),
    packageDir: params.packageDir,
    packageManifest: (0, _manifestDgyExZJE.r)(manifest ?? void 0),
    bundledManifest: params.bundledManifest,
    bundledManifestPath: params.bundledManifestPath
  });
}
function discoverBundleInRoot(params) {
  const bundleFormat = (0, _pathSafetyBa_J5G0M.c)(params.rootDir);
  if (!bundleFormat) return "none";
  const bundleManifest = (0, _pathSafetyBa_J5G0M.l)({
    rootDir: params.rootDir,
    bundleFormat,
...
function resolvePackageEntrySource(params) {
  ...
  const openCandidate = (absolutePath) => {
    ...
    if (!opened.ok) return (0, _boundaryFileReadBSd9MXPy.n)(opened, {
      path: () => null,
      io: () => {
        params.diagnostics.push({
          level: "warn",
...
function isTypeScriptPackageEntry(entryPath) {
  ...
  ".mts",
  ".cts"].
  includes((0, _stringCoerceCjxCKZ6B.a)(_nodePath.default.extname(entryPath)));
}
...
function normalizePackageManifestStringList(value) {
  ...
  return value.map((entry) => (0, _stringCoerceCjxCKZ6B.c)(entry) ?? "").filter(Boolean);
}
...
function resolvePackageSetupSource(params) {
  const packageManifest = (0, _manifestDgyExZJE.r)(params.manifest ?? void 0);
  const setupEntryPath = (0, _stringCoerceCjxCKZ6B.c)(packageManifest?.setupEntry);
  if (!setupEntryPath) return null;
  return resolvePackageRuntimeEntrySource({
    packageDir: params.packageDir,
    entryPath: setupEntryPath,
    runtimeEntryPath: (0, _stringCoerceCjxCKZ6B.c)(packageManifest?.runtimeSetupEntry),
    origin: params.origin,
    sourceLabel: params.sourceLabel,
    diagnostics: params.diagnostics,
    rejectHardlinks: params.rejectHardlinks
...
function resolvePackageRuntimeExtensionEntries(params) {
  const runtimeExtensions = normalizePackageManifestStringList((0, _manifestDgyExZJE.r)(params.manifest ?? void 0)?.runtimeExtensions);
...
function discoverInDirectory(params) {
  ...
  const resolvedDir = (0, _pathSafetyBa_J5G0M.r)(params.dir) ?? _nodePath.default.resolve(params.dir);
  ...
  for (const entry of entries) {
    ...
    const extensionResolution = (0, _manifestDgyExZJE.a)(manifest ?? void 0);
    ...
    const indexFile = [..._manifestDgyExZJE.t].map((candidate) => _nodePath.default.join(fullPath, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
...
function discoverFromPath(params) {
  const resolved = (0, _utilsCB8xp0O.m)(params.rawPath, params.env);
  ...
  if (stat.isDirectory()) {
    ...
    const indexFile = [..._manifestDgyExZJE.t].map((candidate) => _nodePath.default.join(resolved, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
    if (indexFile && isExtensionFile(indexFile)) {
      addCandidate({
        candidates: params.candidates,
        diagnostics: params.diagnostics,
...
function discoverOpenClawPlugins(params) {
  ...
  const workspaceDir = (0, _stringCoerceCjxCKZ6B.c)(params.workspaceDir);
  const workspaceRoot = workspaceDir ? (0, _utilsCB8xp0O.m)(workspaceDir, env) : void 0;
