...
const state = (() => {
  ...
  let registryState = globalState[_runtimeStateIBOIXKOx.t];
  if (!registryState) {
    registryState = {
      activeRegistry: null,
      activeVersion: 0,
    ...
    globalState[_runtimeStateIBOIXKOx.t] = registryState;
  }
...
function requireActivePluginRegistry() {
  if (!state.activeRegistry) {
    state.activeRegistry = (0, _registryEmptyBzdJBxDW.t)();
    state.activeVersion += 1;
