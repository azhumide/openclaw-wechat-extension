...
var _execSafetyGwp6RRDu = require("./exec-safety-gwp6RRDu.js");
var _zodSchemaSensitiveDNDb3nAM = require("./zod-schema.sensitive-DNDb3nAM.js");
...
const FileSecretRefSchema = _zod.z.object({
  ...
  provider: _zod.z.string().regex(SECRET_PROVIDER_ALIAS_PATTERN, "Secret reference provider must match /^[a-z][a-z0-9_-]{0,63}$/ (example: \"default\")."),
  id: _zod.z.string().refine(_refContractBBcTxQFI.s, "File secret reference id must be an absolute JSON pointer (example: \"/providers/openai/apiKey\"), or \"value\" for singleValue mode.")
}).strict();
const ExecSecretRefSchema = _zod.z.object({
  source: _zod.z.literal("exec"),
  provider: _zod.z.string().regex(SECRET_PROVIDER_ALIAS_PATTERN, "Secret reference provider must match /^[a-z][a-z0-9_-]{0,63}$/ (example: \"default\")."),
  id: _zod.z.string().refine(_refContractBBcTxQFI.o, (0, _refContractBBcTxQFI.a)())
}).strict();
...
const SecretsExecProviderSchema = _zod.z.object({
  source: _zod.z.literal("exec"),
  command: _zod.z.string().min(1).refine((value) => (0, _execSafetyGwp6RRDu.t)(value), "secrets.providers.*.command is unsafe.").refine((value) => isAbsolutePath(value), "secrets.providers.*.command must be an absolute path."),
...
const ModelApiSchema = _zod.z.enum(_typesModelsZqK5Y4x.t);
...
const ConfiguredProviderRequestTlsSchema = _zod.z.object({
  ca: SecretInputSchema.optional().register(_zodSchemaSensitiveDNDb3nAM.t),
  cert: SecretInputSchema.optional().register(_zodSchemaSensitiveDNDb3nAM.t),
  key: SecretInputSchema.optional().register(_zodSchemaSensitiveDNDb3nAM.t),
  passphrase: SecretInputSchema.optional().register(_zodSchemaSensitiveDNDb3nAM.t),
...
_zod.z.object({
  ...
  token: SecretInputSchema.register(_zodSchemaSensitiveDNDb3nAM.t)
}).strict(),
_zod.z.object({
  mode: _zod.z.literal("header"),
  headerName: _zod.z.string().min(1),
  value: SecretInputSchema.register(_zodSchemaSensitiveDNDb3nAM.t),
...
const ConfiguredProviderRequestFields = {
  headers: _zod.z.record(_zod.z.string(), SecretInputSchema.register(_zodSchemaSensitiveDNDb3nAM.t)).optional(),
...
const ModelProviderSchema = _zod.z.object({
  ...
  apiKey: SecretInputSchema.optional().register(_zodSchemaSensitiveDNDb3nAM.t),
  auth: _zod.z.union([
...
const TtsProviderConfigSchema = _zod.z.object({ apiKey: SecretInputSchema.optional().register(_zodSchemaSensitiveDNDb3nAM.t) }).catchall(_zod.z.union([
_zod.z.string(),
_zod.z.number(),
_zod.z.boolean(),
_zod.z.null(),
...
const normalizeAllowFrom = (values) => (0, _stringNormalizationDEQ0pIpk.s)(values);
...
const TranscribeAudioSchema = exports.k = _zod.z.object({
  command: _zod.z.array(_zod.z.string()).superRefine((value, ctx) => {
    ...
    if (!(0, _execSafetyGwp6RRDu.t)(executable)) ctx.addIssue({
      code: _zod.z.ZodIssueCode.custom,
      path: [0],
...
const ExecutableTokenSchema = exports.s = _zod.z.string().refine(_execSafetyGwp6RRDu.t, "expected safe executable name or path");
