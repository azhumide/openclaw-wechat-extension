...
function resolvePackageRuntimeExtensionEntries(params) {
  const runtimeExtensions = normalizePackageManifestStringList((0, _manifestBRvO4Lcw.r)(params.manifest ?? void 0)?.runtimeExtensions);
...
async function validatePackageExtensionEntry(params) {
  ...
  try {
    if (!(await (0, _boundaryPathBphsbLa.n)({
      absolutePath,
  ...
  const opened = await (0, _boundaryFileRead6rU9DotD.r)({
    absolutePath,
    rootPath: params.packageDir,
    boundaryLabel: "plugin package directory"
  });
...
function resolvePackageSetupSource(params) {
  const packageManifest = (0, _manifestBRvO4Lcw.r)(params.manifest ?? void 0);
