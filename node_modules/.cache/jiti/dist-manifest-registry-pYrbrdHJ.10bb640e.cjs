"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = toPluginActivationState;exports.a = clearPluginManifestRegistryCache;exports.c = resolveEffectivePluginActivationState;exports.d = isBundledChannelEnabledByChannelConfig$1;exports.f = normalizePluginsConfigWithResolver$1;exports.g = resolvePluginActivationDecisionShared;exports.h = resolveMemorySlotDecisionShared;exports.i = resolveManifestContractPluginIdsByCompatibilityRuntimePath;exports.l = resolveMemorySlotDecision;exports.m = createPluginEnableStateResolver;exports.n = resolveManifestContractOwnerPluginId;exports.o = void 0;exports.p = createEffectiveEnableStateResolver;exports.r = resolveManifestContractPluginIds;exports.s = normalizePluginsConfigWithResolver;exports.t = loadPluginManifestRegistry;exports.u = hasExplicitPluginConfig$1;var _stringCoerceC1IzJjqi = require("./string-coerce-C1IzJjqi.js");
var _utilsBMRcljdi = require("./utils-BMRcljdi.js");
var _ids2bZIXJNM = require("./ids-2bZIXJNM.js");
var _ansiBZHMLcUk = require("./ansi-BZHMLcUk.js");
var _prototypeKeysCL1MnaY = require("./prototype-keys-CL1-mnaY.js");
var _versionDZq9J0ei = require("./version-DZq9J0ei.js");
var _manifestUasFm2SQ = require("./manifest-UasFm2SQ.js");
var _stringNormalizationBvcn03I = require("./string-normalization-Bvcn03I9.js");
var _pathSafetyDXxMDVc = require("./path-safety-DXxMD-vc.js");
var _discoveryDmIzxmAL = require("./discovery-DmIzxmAL.js");
var _slotsClU32VIo = require("./slots-ClU32VIo.js");
var _minHostVersionC5whGYvu = require("./min-host-version-C5whGYvu.js");
...
function normalizeSlotValue(value) {
  const trimmed = (0, _stringCoerceC1IzJjqi.c)(value);
  if (!trimmed) return;
  if ((0, _stringCoerceC1IzJjqi.s)(trimmed) === "none") return null;
  return trimmed;
}
function normalizePluginEntries(entries, normalizePluginId) {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    const subagent = subagentRaw && typeof subagentRaw === "object" && !Array.isArray(subagentRaw) ? {
      ...
      allowedModels: Array.isArray(subagentRaw.allowedModels) ? subagentRaw.allowedModels.map((model) => (0, _stringCoerceC1IzJjqi.c)(model)).filter((model) => Boolean(model)) : void 0
    } : void 0;
...
function normalizePluginsConfigWithResolver$1(config, normalizePluginId = identityNormalizePluginId) {
  ...
  return {
    ...
    slots: {
      memory: memorySlot === void 0 ? (0, _slotsClU32VIo.n)("memory") : memorySlot,
...
function isBundledChannelEnabledByChannelConfig$1(cfg, pluginId) {
  if (!cfg) return false;
  const channelId = (0, _ids2bZIXJNM.r)(pluginId);
...
const hasExplicitPluginConfig = exports.o = hasExplicitPluginConfig$1;
...
function resolveManifestContractOwnerPluginId(params) {
  const normalizedValue = (0, _stringCoerceC1IzJjqi.s)(params.value);
  if (!normalizedValue) return;
  return loadPluginManifestRegistry({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env
  }).plugins.find((plugin) => (!params.origin || plugin.origin === params.origin) && listContractValues(plugin, params.contract).some((candidate) => (0, _stringCoerceC1IzJjqi.s)(candidate) === normalizedValue))?.id;
}
...
function buildCacheKey(params) {
  const { roots, loadPaths } = (0, _discoveryDmIzxmAL.r)({
    workspaceDir: params.workspaceDir,
    loadPaths: params.plugins.loadPaths,
    env: params.env
  });
  return `${roots.workspace ?? ""}::${roots.global}::${roots.stock ?? ""}::${(0, _versionDZq9J0ei.o)(params.env)}::${JSON.stringify(loadPaths)}`;
}
...
function normalizePreferredPluginIds(raw) {
  return (0, _stringNormalizationBvcn03I.a)(raw);
}
function mergePackageChannelMetaIntoChannelConfigs(params) {
  const channelId = params.packageChannel?.id?.trim();
  if (!channelId || (0, _prototypeKeysCL1MnaY.t)(channelId) || !params.channelConfigs || !Object.prototype.hasOwnProperty.call(params.channelConfigs, channelId)) return params.channelConfigs;
  const existing = params.channelConfigs[channelId];
  if (!existing) return params.channelConfigs;
  const label = existing.label ?? (0, _stringCoerceC1IzJjqi.c)(params.packageChannel?.label) ?? "";
  const description = existing.description ?? (0, _stringCoerceC1IzJjqi.c)(params.packageChannel?.blurb) ?? "";
  const preferOver = existing.preferOver ?? normalizePreferredPluginIds(params.packageChannel?.preferOver);
  const merged = Object.create(null);
  for (const [key, value] of Object.entries(params.channelConfigs)) if (!(0, _prototypeKeysCL1MnaY.t)(key)) merged[key] = value;
  merged[channelId] = {
    ...existing,
    ...(label ? { label } : {}),
...
function buildRecord(params) {
  ...
  return {
    ...
    name: (0, _stringCoerceC1IzJjqi.c)(params.manifest.name) ?? params.candidate.packageName,
    description: (0, _stringCoerceC1IzJjqi.c)(params.manifest.description) ?? params.candidate.packageDescription,
    version: (0, _stringCoerceC1IzJjqi.c)(params.manifest.version) ?? params.candidate.packageVersion,
    ...
    providerDiscoverySource: params.manifest.providerDiscoveryEntry ? resolvePluginSourcePath(_nodePath.default.resolve(params.candidate.rootDir, params.manifest.providerDiscoveryEntry)) : void 0,
...
function buildBundleRecord(params) {
  return {
    id: params.manifest.id,
    name: (0, _stringCoerceC1IzJjqi.c)(params.manifest.name) ?? params.candidate.idHint,
    description: (0, _stringCoerceC1IzJjqi.c)(params.manifest.description),
    version: (0, _stringCoerceC1IzJjqi.c)(params.manifest.version),
    format: "bundle",
    bundleFormat: params.candidate.bundleFormat,
...
function pushProviderAuthEnvVarsCompatDiagnostic(params) {
  ...
  params.diagnostics.push({
    level: "warn",
    pluginId: (0, _ansiBZHMLcUk.t)(params.record.id),
    source: (0, _ansiBZHMLcUk.t)(params.record.manifestPath),
    message: `providerAuthEnvVars is deprecated compatibility metadata for provider env-var lookup; mirror ${providerIds.map(_ansiBZHMLcUk.t).join(", ")} env vars to setup.providers[].envVars before the deprecation window closes`
  });
}
function pushNonBundledChannelConfigDescriptorDiagnostic(params) {
  ...
  const safeMissingChannels = missingChannels.map(_ansiBZHMLcUk.t);
  params.diagnostics.push({
    level: "warn",
    pluginId: (0, _ansiBZHMLcUk.t)(params.record.id),
    source: (0, _ansiBZHMLcUk.t)(params.record.manifestPath),
...
function matchesInstalledPluginRecord(params) {
  ...
  const candidateSource = (0, _utilsBMRcljdi.m)(params.candidate.source, params.env);
  const trackedPaths = [record.installPath, record.sourcePath].filter((entry) => typeof entry === "string" && entry.trim().length > 0).map((entry) => (0, _utilsBMRcljdi.m)(entry, params.env));
  if (trackedPaths.length === 0) return false;
  return trackedPaths.some((trackedPath) => {
    return candidateSource === trackedPath || (0, _pathSafetyDXxMDVc.n)(trackedPath, candidateSource);
  });
}
...
function loadPluginManifestRegistry(params = {}) {
  ...
  const currentHostVersion = (0, _versionDZq9J0ei.o)(env);
  for (const candidate of candidates) {
    ...
    } : isBundleRecord && candidate.bundleFormat ? (0, _pathSafetyDXxMDVc.l)({
      rootDir: candidate.rootDir,
      bundleFormat: candidate.bundleFormat,
      rejectHardlinks
    }) : (0, _manifestUasFm2SQ.i)(candidate.rootDir, rejectHardlinks);
    if (!manifestRes.ok) {
      diagnostics.push({
        level: "error",
    ...
    const minHostVersionCheck = (0, _minHostVersionC5whGYvu.t)({
    ...
    if (existing) {
      ...
      if ((() => {
        ...
        const existingReal = (0, _pathSafetyDXxMDVc.r)(existing.candidate.rootDir, realpathCache);
        const candidateReal = (0, _pathSafetyDXxMDVc.r)(candidate.rootDir, realpathCache);
