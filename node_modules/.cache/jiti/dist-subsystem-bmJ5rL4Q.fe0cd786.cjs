...
var _stringCoerceCjxCKZ6B = require("./string-coerce-CjxCKZ6B.js");
var _progressLineB452nFKk = require("./progress-line-B452nFKk.js");
var _loggerDTX5koY = require("./logger-DTX5koY3.js");
var _idsBPsM98Uu = require("./ids-BPsM98Uu.js");
var _globalStateCGb9JSDC = require("./global-state-CGb9JSDC.js");
var _runtimeDy0KdPiM = require("./runtime-Dy0KdPiM.js");
var _ansiCis6s4Rt = require("./ansi-Cis6s4Rt.js");
...
const requireConfig = (0, _nodeRequireDvwmVoHm.t)("file:///home/rs/openclaw/dist/subsystem-bmJ5rL4Q.js");
...
function resolveConsoleSettings() {
  const envLevel = (0, _loggerDTX5koY.h)();
  if (process.env.VITEST === "true" && process.env.OPENCLAW_TEST_CONSOLE !== "1" && !(0, _globalStateCGb9JSDC.t)() && !envLevel && !_loggerDTX5koY.g.overrideSettings) return {
    level: "silent",
    style: normalizeConsoleStyle(void 0)
  };
  let cfg = _loggerDTX5koY.g.overrideSettings ?? (0, _loggerDTX5koY.x)();
  if (!cfg && !(0, _loggerDTX5koY.S)()) if (_loggerDTX5koY.g.resolvingConsoleSettings) cfg = void 0;else
  {
    _loggerDTX5koY.g.resolvingConsoleSettings = true;
    try {
      cfg = loadConfigFallback();
    } finally {
      _loggerDTX5koY.g.resolvingConsoleSettings = false;
    }
  }
  return {
...
function getConsoleSettings() {
  ...
  const cached = _loggerDTX5koY.g.cachedConsoleSettings;
  if (!cached || consoleSettingsChanged(cached, settings)) _loggerDTX5koY.g.cachedConsoleSettings = settings;
  return _loggerDTX5koY.g.cachedConsoleSettings;
}
function getResolvedConsoleSettings() {
  return getConsoleSettings();
}
function routeLogsToStderr() {
  _loggerDTX5koY.g.forceConsoleToStderr = true;
}
function setConsoleSubsystemFilter(filters) {
  if (!filters || filters.length === 0) {
    _loggerDTX5koY.g.consoleSubsystemFilter = null;
    return;
  }
  const normalized = filters.map((value) => value.trim()).filter((value) => value.length > 0);
  _loggerDTX5koY.g.consoleSubsystemFilter = normalized.length > 0 ? normalized : null;
}
function setConsoleTimestampPrefix(enabled) {
  _loggerDTX5koY.g.consoleTimestampPrefix = enabled;
}
...
function shouldLogSubsystemToConsole(subsystem) {
  const filter = _loggerDTX5koY.g.consoleSubsystemFilter;
...
function shouldSuppressConsoleMessage(message) {
  if ((0, _globalStateCGb9JSDC.t)()) return false;
...
function formatConsoleTimestamp(style) {
  ...
  if (style === "pretty") return (0, _loggerDTX5koY.p)(now, { style: "short" }).replace(/[+-]\d{2}:\d{2}$/, "");
  return (0, _loggerDTX5koY.f)(now);
}
...
function enableConsoleCapture() {
  if (_loggerDTX5koY.g.consolePatched) return;
  _loggerDTX5koY.g.consolePatched = true;
  if (!_loggerDTX5koY.g.streamErrorHandlersInstalled) {
    _loggerDTX5koY.g.streamErrorHandlersInstalled = true;
  ...
  let logger = null;
  const getLoggerLazy = () => {
    if (!logger) logger = (0, _loggerDTX5koY.a)();
    return logger;
  };
  const original = {
    log: console.log,
  ...
  const forward = (level, orig) => (...args) => {
    ...
    const trimmed = (0, _ansiCis6s4Rt.r)(formatted).trimStart();
    const timestamp = _loggerDTX5koY.g.consoleTimestampPrefix && trimmed.length > 0 && !hasTimestampPrefix(trimmed) ? formatConsoleTimestamp(getConsoleSettings().style) : "";
    try {
    ...
    if (_loggerDTX5koY.g.forceConsoleToStderr) try {
...
function shouldLogToConsole(level, settings) {
  ...
  return (0, _loggerDTX5koY.v)(level) >= (0, _loggerDTX5koY.v)(settings.level);
}
const inspectValue = (() => {
...
function isRichConsoleEnv() {
  const term = (0, _stringCoerceCjxCKZ6B.a)(process.env.TERM);
...
function isChannelSubsystemPrefix(value) {
  const normalized = (0, _stringCoerceCjxCKZ6B.a)(value);
  if (!normalized) return false;
  return (0, _idsBPsM98Uu.r)(normalized) === normalized || normalized === "webchat";
}
...
function stripRedundantSubsystemPrefixForConsole(message, displaySubsystem) {
  ...
  if (message.startsWith("[")) {
    ...
    if (closeIdx > 1) {
      if ((0, _stringCoerceCjxCKZ6B.a)(message.slice(1, closeIdx)) === (0, _stringCoerceCjxCKZ6B.a)(displaySubsystem)) {
        let i = closeIdx + 1;
        while (message[i] === " ") i += 1;
  ...
  if ((0, _stringCoerceCjxCKZ6B.a)(message.slice(0, displaySubsystem.length)) !== (0, _stringCoerceCjxCKZ6B.a)(displaySubsystem)) return message;
...
function formatConsoleLine(opts) {
  ...
  return `${[(() => {
    ...
    if (_loggerDTX5koY.g.consoleTimestampPrefix) return color.gray(formatConsoleTimestamp(opts.style));
    return "";
  })(), prefixColor(prefix)].filter(Boolean).join(" ")} ${levelColor(displayMessage)}`;
}
function writeConsoleLine(level, line) {
  (0, _progressLineB452nFKk.t)();
  const sanitized = process.platform === "win32" && process.env.GITHUB_ACTIONS === "true" ? line.replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "?").replace(/[\uD800-\uDFFF]/g, "?") : line;
  const sink = _loggerDTX5koY.g.rawConsole ?? console;
  if (_loggerDTX5koY.g.forceConsoleToStderr || level === "error" || level === "fatal") (sink.error ?? console.error)(sanitized);else
...
function createSubsystemLogger(subsystem) {
  ...
  const emitLog = (level, message, meta) => {
    ...
    const fileEnabled = (0, _loggerDTX5koY.s)(level);
    ...
    if (fileEnabled) {
      if (!fileLogger) fileLogger = (0, _loggerDTX5koY.i)({ subsystem: resolvedSubsystem });
  ...
  return {
    ...
    isEnabled(level, target = "any") {
      ...
      const isFileEnabled = (0, _loggerDTX5koY.s)(level);
...
function runtimeForLogger(logger, exit = _runtimeDy0KdPiM.n.exit) {
  return {
    log(...args) {
...
function createSubsystemRuntime(subsystem, exit = _runtimeDy0KdPiM.n.exit) {
  return runtimeForLogger(createSubsystemLogger(subsystem), exit);
}
//#endregion /* v9-33b922ad01e5886d */
