...
var _jitiLoaderCacheCWTIhI = require("./jiti-loader-cache-CWT-ihI3.js");
var _sdkAliasZeYYdq5k = require("./sdk-alias-zeYYdq5k.js");
var _publicSurfaceRuntimeBWMmpEH = require("./public-surface-runtime-BWMmpEH6.js");
...
var _manifestGzgxnRAf = require("./manifest-gzgxnRAf.js");
...
function normalizeBundledPluginStringList(value) {
  return (0, _stringNormalizationBrdBBNdu.l)(value);
}
...
function isTopLevelPublicSurfaceSource(name) {
  if (!_publicSurfaceRuntimeBWMmpEH.t.includes(_nodePath.default.extname(name))) return false;
...
function collectBundledPluginPublicSurfaceArtifacts(params) {
  const excluded = new Set((0, _stringNormalizationBrdBBNdu.l)([params.sourceEntry, params.setupEntry]).map((entry) => _nodePath.default.basename(entry)));
...
function resolveConfigSchemaExport(imported) {
  ...
  for (const [name, value] of Object.entries(imported)) {
    ...
    if (value && typeof value === "object") return (0, _configSchemaTgszMKRa.r)(value);
  }
  for (const value of Object.values(imported)) if (isBuiltChannelConfigSchema(value)) return value;
  return null;
}
function getJiti(modulePath) {
  return (0, _jitiLoaderCacheCWTIhI.t)({
    cache: jitiLoaders,
    modulePath,
    importerUrl: "file:///home/rs/openclaw/dist/bundled-plugin-metadata-BHkK13pu.js",
    preferBuiltDist: true,
    jitiFilename: "file:///home/rs/openclaw/dist/bundled-plugin-metadata-BHkK13pu.js"
  });
}
function resolveChannelConfigSchemaModulePath(pluginDir) {
  ...
  for (const basename of PUBLIC_CONFIG_SURFACE_BASENAMES) for (const extension of _publicSurfaceRuntimeBWMmpEH.t) {
...
const OPENCLAW_PACKAGE_ROOT = (0, _sdkAliasZeYYdq5k.l)({
  modulePath: (0, _nodeUrl.fileURLToPath)("file:///home/rs/openclaw/dist/bundled-plugin-metadata-BHkK13pu.js"),
  moduleUrl: "file:///home/rs/openclaw/dist/bundled-plugin-metadata-BHkK13pu.js"
}) ?? (0, _nodeUrl.fileURLToPath)(new URL("../..", "file:///home/rs/openclaw/dist/bundled-plugin-metadata-BHkK13pu.js"));
const CURRENT_MODULE_PATH = (0, _nodeUrl.fileURLToPath)("file:///home/rs/openclaw/dist/bundled-plugin-metadata-BHkK13pu.js");
...
function collectBundledPluginMetadata(packageRoot, includeChannelConfigs, includeSyntheticChannelConfigs, scanDir) {
  ...
  for (const dirName of _nodeFs.default.readdirSync(resolvedScanDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name).toSorted((left, right) => left.localeCompare(right))) {
    ...
    const manifestResult = (0, _manifestGzgxnRAf.i)(pluginDir, false);
    if (!manifestResult.ok) continue;
    const packageJson = readPackageManifest(pluginDir);
    const packageManifest = (0, _manifestGzgxnRAf.r)(packageJson);
