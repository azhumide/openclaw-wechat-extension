...
function resolvePackageRootsForBundledPlugins() {
  return [(0, _openclawRootCRSCIPqz.n)({ argv1: process.argv[1] }), (0, _openclawRootCRSCIPqz.n)({ moduleUrl: "file:///home/rs/openclaw/dist/bundled-dir-m7NsR8Gx.js" })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index);
}
...
function resolveTrustedExistingOverride(resolvedOverride) {
  ...
  const modulePackageRoot = (0, _openclawRootCRSCIPqz.n)({ moduleUrl: "file:///home/rs/openclaw/dist/bundled-dir-m7NsR8Gx.js" });
...
function resolveBundledPluginsDirUncached(env) {
  ...
  if (override) {
    const resolvedOverride = (0, _utilsCCskKJVV.p)(override, env);
  ...
  try {
    ...
    })) ? null : argvRoot, (0, _openclawRootCRSCIPqz.n)({ moduleUrl: "file:///home/rs/openclaw/dist/bundled-dir-m7NsR8Gx.js" })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index);
  ...
  try {
    let cursor = _nodePath.default.dirname((0, _nodeUrl.fileURLToPath)("file:///home/rs/openclaw/dist/bundled-dir-m7NsR8Gx.js"));
    for (let i = 0; i < 6; i += 1) {
