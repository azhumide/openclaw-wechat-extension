...
require("./plugins-DST-zIRU.js");
var _runtimeDuxCj3Hd = require("./runtime-DuxCj3Hd.js");
var _bindingsBmgOe1R_ = require("./bindings-BmgOe1R_.js");
var _resolveRouteDqU_eMP = require("./resolve-route-DqU_eMP-.js");
...
function buildConfiguredAcpSessionKey(spec) {
  ...
  return `agent:${(0, _sessionKey7OXmevzf.f)(spec.agentId)}:acp:binding:${spec.channel}:${spec.accountId}:${hash}`;
}
...
function parseConfiguredAcpSessionKey(sessionKey) {
  ...
  return {
    channel,
    accountId: (0, _accountIdB6sv0G6z.n)(tokens[3] ?? "default")
  };
}
function resolveConfiguredAcpBindingSpecFromRecord(record) {
  ...
  const agentId = (0, _stringCoerceBje8XVt.c)(record.metadata?.agentId) ?? (0, _sessionKey7OXmevzf.u)(record.targetSessionKey);
  if (!agentId) return null;
  return {
    channel: record.conversation.channel,
    accountId: (0, _accountIdB6sv0G6z.n)(record.conversation.accountId),
    conversationId,
...
function resolveConfiguredBindingWorkspaceCwd(params) {
  if ((0, _stringCoerceBje8XVt.c)((0, _agentScopeQAnuLFjf._)(params.cfg, params.agentId)?.workspace)) return (0, _agentScopeQAnuLFjf.b)(params.cfg, params.agentId);
  if (params.agentId === (0, _agentScopeQAnuLFjf.x)(params.cfg)) {
    if ((0, _stringCoerceBje8XVt.c)(params.cfg.agents?.defaults?.workspace)) return (0, _agentScopeQAnuLFjf.b)(params.cfg, params.agentId);
  }
}
...
function resolveLoadedChannelPlugin(channel) {
  ...
  return (0, _registryB4_z8DxJ.t)(normalized);
}
...
function compileConfiguredBindingRule(params) {
  const agentId = (0, _resolveRouteDqU_eMP.r)(params.cfg, params.binding.agentId ?? "main");
...
function compileConfiguredBindingRegistry(params) {
  ...
  for (const binding of (0, _bindingsBmgOe1R_.r)(params.cfg)) {
...
function resolveCompiledBindingRegistry(cfg) {
  const activeRegistry = (0, _runtimeDuxCj3Hd.g)();
  const registryVersion = (0, _runtimeDuxCj3Hd.n)();
...
function primeCompiledBindingRegistry(cfg) {
  ...
  compiledRegistryCache.set(cfg, {
    registryRef: activeRegistry,
    registryVersion: (0, _runtimeDuxCj3Hd.n)(),
    registry
  });
  return registry;
}
...
function resolveAccountMatchPriority(match, actual) {
  ...
  if (trimmed === "*") return 1;
  return (0, _accountIdB6sv0G6z.n)(trimmed) === actual ? 2 : 0;
}
...
function toConfiguredBindingConversationRef(conversation) {
  ...
  return {
    channel,
    accountId: (0, _accountIdB6sv0G6z.n)(conversation.accountId),
    conversationId,
...
function materializeConfiguredBindingRecord(params) {
  return params.rule.targetFactory.materialize({
    accountId: (0, _accountIdB6sv0G6z.n)(params.accountId),
