"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.wechatPlugin = void 0;var fs = _interopRequireWildcard(require("node:fs"));
...
var _redaction = require("./redaction.js");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
const recentOutboundMediaAt = new Map();
...
function claimWechatOutboundMediaDedup(dedupKey) {
  const now = Date.now();
...
function releaseWechatOutboundMediaDedup(dedupKey) {
  recentOutboundMediaAt.delete(dedupKey);
}

function buildWechatLocalMediaFingerprint(filePath) {
...
function redactWechatOutboundText(text, options)
...
function summarizeWechatOutboundTextForLog(text, options)


{
  return summarizeWechatTextForLog(
    typeof text === "string" ?
    (0, _redaction.redactWechatWxids)(text, {
...
const wechatPlugin = exports.wechatPlugin = {
  id: "wechat",
  meta: {
    id: "wechat",
    label: "WeChat",
  ...
  status: {
    buildChannelSummary: ({ snapshot }) => {
      return {
        ...
        connected: (0, _runtime.isBridgeConnected)(),
  ...
  actions: {
    ...
    handleAction: async ({ action, params, accountId, toolContext }) => {
      ...
      const runtime = (0, _runtime.getWechatRuntime)();
      const cfg = runtime?.config.current?.() || {};
      const bridgeConfig = (0, _config.resolveWechatExtensionConfig)(cfg, runtime?.logger ?? console);
      const to = (0, _channelActions.readStringParam)(params, "to") ?? (0, _channelActions.readStringParam)(params, "target", { required: true });
      ...
      const reactionMessageIdRaw = (0, _channelActions.resolveReactionMessageId)({
        args: params,
        toolContext: {
      ...
      const sent = (0, _runtime.sendToBridge)(buildOutboundFrame("outbound_reaction", payload));
      if (!sent.ok) {
  ...
  outbound: {
    deliveryMode: "direct",
    sendText: async ({ to, text, accountId, msg_id, original_msg_id, messageId, replyToId }) => {
      const runtime = (0, _runtime.getWechatRuntime)();
      const cfg = runtime?.config.current?.() || {};
      const bridgeConfig = (0, _config.resolveWechatExtensionConfig)(cfg, runtime?.logger ?? console);
      ...
      const sent = (0, _runtime.sendToBridge)(buildOutboundFrame("outbound_text", payload));
      if (!sent.ok) {
    ...
    sendMedia: async ({ to, mediaUrl, text, accountId, msg_id, original_msg_id, messageId, replyToId }) => {
      const runtime = (0, _runtime.getWechatRuntime)();
      const cfg = runtime.config.current();
      const bridgeConfig = (0, _config.resolveWechatExtensionConfig)(cfg, runtime.logger ?? console);
      ...
      try {
        const serveRoots = (0, _config.resolveWechatMediaServeRoots)(cfg, runtime.logger ?? console);
        ...
        if (mediaUrl && !mediaUrl.startsWith("http://") && !mediaUrl.startsWith("https://")) {
          ...
          if (!/^(?:[a-zA-Z]:[\\/]|\/)/.test(mediaUrl)) {
            const os = await Promise.resolve().then(() => jitiImport("os").then((m) => _interopRequireWildcard(m)));
          ...
          if (fs.existsSync(absolutePath) && !(0, _config.isPathWithinRoots)(absolutePath, serveRoots)) {
        ...
        const sent = (0, _runtime.sendToBridge)(buildOutboundFrame("outbound_media", payload));
        if (!sent.ok) {
