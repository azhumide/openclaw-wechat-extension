...
var _globalsCQubNFgm = require("./globals-CQubNFgm.js");
var _loggerB5KlacON = require("./logger-B5KlacON.js");
var _accountIdB6sv0G6z = require("./account-id-B6sv0G6z.js");
var _sessionKey7OXmevzf = require("./session-key-7OXmevzf.js");
var _agentScopeQAnuLFjf = require("./agent-scope-qAnuLFjf.js");
var _chatTypeIxrcH2Mx = require("./chat-type-ixrcH2Mx.js");
var _bindingScopeCUNYV6QY = require("./binding-scope-CUNYV6QY.js");
var _bindingsDeDXvcqe = require("./bindings-DeDXvcqe.js");
var _peerKindMatchC2ugXJxA = require("./peer-kind-match-C2ugXJxA.js");
...
function normalizeId(value) {
  return (0, _bindingScopeCUNYV6QY.n)(value);
}
function buildAgentSessionKey(params) {
  const channel = normalizeToken(params.channel) || "unknown";
  const peer = params.peer;
  return (0, _sessionKey7OXmevzf.i)({
    agentId: params.agentId,
    mainKey: _sessionKey7OXmevzf.n,
    channel,
...
function resolveAgentLookupCache(cfg) {
  ...
  for (const agent of listAgents(cfg)) {
    ...
    if (!rawId) continue;
    byNormalizedId.set((0, _sessionKey7OXmevzf.c)(rawId), (0, _sessionKey7OXmevzf.f)(rawId));
  }
  const next = {
    agentsRef,
    byNormalizedId,
    fallbackDefaultAgentId: (0, _sessionKey7OXmevzf.f)((0, _agentScopeQAnuLFjf.x)(cfg))
  };
...
function pickFirstExistingAgentId(cfg, agentId) {
  ...
  const normalized = (0, _sessionKey7OXmevzf.c)(trimmed);
  if (lookup.byNormalizedId.size === 0) return (0, _sessionKey7OXmevzf.f)(trimmed);
...
function resolveAccountPatternKey(accountPattern) {
  if (!accountPattern.trim()) return _accountIdB6sv0G6z.t;
  return (0, _accountIdB6sv0G6z.n)(accountPattern);
}
function buildEvaluatedBindingsByChannel(cfg) {
  const byChannel = /* @__PURE__ */new Map();
  let order = 0;
  for (const binding of (0, _bindingsDeDXvcqe.n)(cfg)) {
...
function normalizePeerConstraint(peer) {
  ...
  const kind = (0, _chatTypeIxrcH2Mx.t)(peer.kind);
...
function normalizeBindingMatch(match) {
  ...
  return {
    ...
    roles: (0, _bindingScopeCUNYV6QY.r)(rawRoles)
  };
}
...
function matchesBindingScope(match, scope) {
  ...
  if (match.peer.state === "valid") {
    if (!scope.peer || !(0, _peerKindMatchC2ugXJxA.t)(match.peer.kind, scope.peer.kind) || scope.peer.id !== match.peer.id) return false;
  }
  if (match.peer.state === "wildcard-kind") {
    if (!scope.peer || !(0, _peerKindMatchC2ugXJxA.t)(match.peer.kind, scope.peer.kind)) return false;
  }
  return (0, _bindingScopeCUNYV6QY.a)(match, scope);
}
function resolveAgentRoute(input) {
  const channel = normalizeToken(input.channel);
  const accountId = (0, _accountIdB6sv0G6z.n)(input.accountId);
  const peer = input.peer ? {
    kind: (0, _chatTypeIxrcH2Mx.t)(input.peer.kind) ?? input.peer.kind,
  ...
  const shouldLogDebug = (0, _globalsCQubNFgm.a)();
  const parentPeer = input.parentPeer ? {
    kind: (0, _chatTypeIxrcH2Mx.t)(input.parentPeer.kind) ?? input.parentPeer.kind,
  ...
  const choose = (agentId, matchedBy, sessionOverride) => {
    ...
    const mainSessionKey = (0, _stringCoerceBje8XVt.a)((0, _sessionKey7OXmevzf.r)({
      agentId: resolvedAgentId,
      mainKey: _sessionKey7OXmevzf.n
    }));
    const route = {
  ...
  if (shouldLogDebug) {
    (0, _loggerB5KlacON.t)(`[routing] resolveAgentRoute: channel=${channel} accountId=${accountId} peer=${formatPeer(peer)} guildId=${guildId || "none"} teamId=${teamId || "none"} bindings=${bindings.length}`);
    for (const entry of bindings) (0, _loggerB5KlacON.t)(`[routing] binding: agentId=${entry.binding.agentId} accountPattern=${entry.match.accountPattern || "default"} peer=${formatNormalizedPeer(entry.match.peer)} guildId=${entry.match.guildId ?? "none"} teamId=${entry.match.teamId ?? "none"} roles=${entry.match.roles?.length ?? 0}`);
  }
  const baseScope = {
    guildId,
    teamId,
  ...
  for (const tier of tiers) {
    ...
    if (matched) {
      if (shouldLogDebug) (0, _loggerB5KlacON.t)(`[routing] match: matchedBy=${tier.matchedBy} agentId=${matched.binding.agentId}`);
      return choose(matched.binding.agentId, tier.matchedBy, matched.binding.session);
    }
  }
  return choose((0, _agentScopeQAnuLFjf.x)(input.cfg), "default");
}
//#endregion /* v9-45bb665a348ae80a */
