...
const FileSecretRefSchema = _zod.z.object({
  ...
  provider: _zod.z.string().regex(SECRET_PROVIDER_ALIAS_PATTERN, "Secret reference provider must match /^[a-z][a-z0-9_-]{0,63}$/ (example: \"default\")."),
  id: _zod.z.string().refine(_refContractCsOjNPCR.s, "File secret reference id must be an absolute JSON pointer (example: \"/providers/openai/apiKey\"), or \"value\" for singleValue mode.")
}).strict();
const ExecSecretRefSchema = _zod.z.object({
  source: _zod.z.literal("exec"),
  provider: _zod.z.string().regex(SECRET_PROVIDER_ALIAS_PATTERN, "Secret reference provider must match /^[a-z][a-z0-9_-]{0,63}$/ (example: \"default\")."),
  id: _zod.z.string().refine(_refContractCsOjNPCR.o, (0, _refContractCsOjNPCR.a)())
}).strict();
...
const SecretsExecProviderSchema = _zod.z.object({
  source: _zod.z.literal("exec"),
  command: _zod.z.string().min(1).refine((value) => (0, _execSafetyBxxyVvE.t)(value), "secrets.providers.*.command is unsafe.").refine((value) => isAbsolutePath(value), "secrets.providers.*.command must be an absolute path."),
...
const ConfiguredProviderRequestTlsSchema = _zod.z.object({
  ca: SecretInputSchema.optional().register(_zodSchemaSensitiveB92jMOck.t),
  cert: SecretInputSchema.optional().register(_zodSchemaSensitiveB92jMOck.t),
  key: SecretInputSchema.optional().register(_zodSchemaSensitiveB92jMOck.t),
  passphrase: SecretInputSchema.optional().register(_zodSchemaSensitiveB92jMOck.t),
...
_zod.z.object({
  ...
  token: SecretInputSchema.register(_zodSchemaSensitiveB92jMOck.t)
}).strict(),
_zod.z.object({
  mode: _zod.z.literal("header"),
  headerName: _zod.z.string().min(1),
  value: SecretInputSchema.register(_zodSchemaSensitiveB92jMOck.t),
...
const ConfiguredProviderRequestFields = {
  headers: _zod.z.record(_zod.z.string(), SecretInputSchema.register(_zodSchemaSensitiveB92jMOck.t)).optional(),
...
const ModelProviderSchema = _zod.z.object({
  ...
  apiKey: SecretInputSchema.optional().register(_zodSchemaSensitiveB92jMOck.t),
  auth: _zod.z.union([
...
const TtsProviderConfigSchema = _zod.z.object({ apiKey: SecretInputSchema.optional().register(_zodSchemaSensitiveB92jMOck.t) }).catchall(_zod.z.union([
_zod.z.string(),
_zod.z.number(),
_zod.z.boolean(),
_zod.z.null(),
...
const normalizeAllowFrom = (values) => (0, _stringNormalization5F0OqbT.s)(values);
...
const TranscribeAudioSchema = exports.k = _zod.z.object({
  command: _zod.z.array(_zod.z.string()).superRefine((value, ctx) => {
    ...
    if (!(0, _execSafetyBxxyVvE.t)(executable)) ctx.addIssue({
      code: _zod.z.ZodIssueCode.custom,
      path: [0],
...
const ExecutableTokenSchema = exports.s = _zod.z.string().refine(_execSafetyBxxyVvE.t, "expected safe executable name or path");
