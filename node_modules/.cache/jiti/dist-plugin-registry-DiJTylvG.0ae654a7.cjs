...
function createPluginRegistryIdNormalizer(index, options = {}) {
  ...
  const registry = options.lookUpTable?.manifestRegistry ?? options.manifestRegistry ?? (0, _manifestRegistryInstalledDFLMxxdp.t)({
    index,
...
function hasMissingPersistedPluginSource(index) {
  return index.plugins.some((plugin) => {
    ...
    return !_nodeFs.default.existsSync(plugin.rootDir) || !(0, _installedPluginIndexStoreWXOsBZRd.m)(plugin) && !_nodeFs.default.existsSync(plugin.manifestPath) || (plugin.source ? !_nodeFs.default.existsSync(plugin.source) : false) || (plugin.setupSource ? !_nodeFs.default.existsSync(plugin.setupSource) : false);
  });
}
...
function hasMismatchedPersistedBundledPluginRoot(index, env) {
  const bundledPluginsDir = (0, _bundledDirM7NsR8Gx.n)(env);
...
function hasStalePersistedPluginMetadata(index) {
  return index.plugins.some((plugin) => {
    if (!(0, _installedPluginIndexStoreWXOsBZRd.m)(plugin)) if ((0, _installedPluginIndexStoreWXOsBZRd._)(plugin.manifestPath, plugin.manifestFile) === false) {
    ...
    if ((0, _installedPluginIndexStoreWXOsBZRd._)(packageJsonPath, plugin.packageJson.fileSignature) === false) return hashExistingFile(packageJsonPath) !== plugin.packageJson.hash;
...
function loadPluginRegistrySnapshotWithMetadata(params = {}) {
  ...
  if (persistedInstallRecordReadsEnabled) {
    persistedIndex = (0, _installedPluginIndexStoreWXOsBZRd.r)(params);
    if (persistedReadsEnabled && persistedIndex) {if (params.config && persistedIndex.policyHash !== (0, _installedPluginIndexStoreWXOsBZRd.g)(params.config)) diagnostics.push({
        level: "warn",
  ...
  return {
    snapshot: (0, _installedPluginIndexStoreWXOsBZRd.f)({
      ...params,
      installRecords: params.installRecords ?? (0, _installedPluginIndexStoreWXOsBZRd.h)(persistedIndex)
    }),
    source: "derived",
    diagnostics
  };
...
function getPluginRecord(params) {
  return (0, _installedPluginIndexStoreWXOsBZRd.u)(resolveSnapshot(params), params.pluginId);
}
function isPluginEnabled(params) {
  return (0, _installedPluginIndexStoreWXOsBZRd.d)(resolveSnapshot(params), params.pluginId, params.config);
}
function inspectPluginRegistry(params = {}) {
  return (0, _installedPluginIndexStoreWXOsBZRd.t)(params);
}
function refreshPluginRegistry(params) {
  return (0, _installedPluginIndexStoreWXOsBZRd.i)(params);
}
//#endregion
...
function resolveContributionPluginIds(params) {
  ...
  return params.index.plugins.filter((plugin) => (0, _installedPluginIndexStoreWXOsBZRd.d)(params.index, plugin.pluginId, params.config)).map((plugin) => plugin.pluginId);
}
function loadContributionManifestRegistry(params) {
  return (0, _manifestRegistryInstalledDFLMxxdp.t)({
    index: params.index,
...
function normalizePluginsConfigWithRegistry(config, index, options = {}) {
  return (0, _configNormalizationSharedBkjbUp.i)(config, createPluginRegistryIdNormalizer(index, options));
}
...
function resolveProviderOwners(params) {
  const providerId = (0, _providerIdDd60CFPS.r)(params.providerId);
  if (!providerId) return [];
  if (params.lookUpTable) {
    const index = params.lookUpTable.index;
    const owners = [];
    for (const [contributionId, ownerIds] of params.lookUpTable.owners.providers.entries()) if ((0, _providerIdDd60CFPS.r)(contributionId) === providerId) owners.push(...ownerIds);
  ...
  return resolvePluginContributionOwners({
    ...
    matches: (contributionId) => (0, _providerIdDd60CFPS.r)(contributionId) === providerId
  });
}
