"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveInboundLastRouteSessionKey;exports.i = resolveAgentRoute;exports.n = deriveLastRoutePolicy;exports.r = pickFirstExistingAgentId;exports.t = buildAgentSessionKey;var _stringCoerceC1IzJjqi = require("./string-coerce-C1IzJjqi.js");
var _globalsDeRFSEIV = require("./globals-DeRFSEIV.js");
var _loggerBvytzgs = require("./logger-bvytzgs1.js");
...
var _chatTypeDAU8eOlz = require("./chat-type-DAU8eOlz.js");
var _bindingScopeAmGt2T8S = require("./binding-scope-amGt2T8S.js");
var _bindingsDSOcFD4j = require("./bindings-DSOcFD4j.js");
var _peerKindMatchFF6fKm3k = require("./peer-kind-match-FF6fKm3k.js");
...
function normalizeToken(value) {
  return (0, _stringCoerceC1IzJjqi.a)(value);
}
function normalizeId(value) {
  return (0, _bindingScopeAmGt2T8S.n)(value);
}
function buildAgentSessionKey(params) {
  const channel = normalizeToken(params.channel) || "unknown";
  const peer = params.peer;
  return (0, _sessionKeyEpIbK3Oz.i)({
    agentId: params.agentId,
    mainKey: _sessionKeyEpIbK3Oz.n,
    channel,
...
function resolveAgentLookupCache(cfg) {
  ...
  for (const agent of listAgents(cfg)) {
    ...
    if (!rawId) continue;
    byNormalizedId.set((0, _sessionKeyEpIbK3Oz.c)(rawId), (0, _sessionKeyEpIbK3Oz.f)(rawId));
  }
  const next = {
    agentsRef,
    byNormalizedId,
    fallbackDefaultAgentId: (0, _sessionKeyEpIbK3Oz.f)((0, _agentScope_6dFncNS.x)(cfg))
  };
...
function pickFirstExistingAgentId(cfg, agentId) {
  ...
  const normalized = (0, _sessionKeyEpIbK3Oz.c)(trimmed);
  if (lookup.byNormalizedId.size === 0) return (0, _sessionKeyEpIbK3Oz.f)(trimmed);
...
function resolveAccountPatternKey(accountPattern) {
  if (!accountPattern.trim()) return _accountIdC3j_3_su.t;
  return (0, _accountIdC3j_3_su.n)(accountPattern);
}
function buildEvaluatedBindingsByChannel(cfg) {
  const byChannel = /* @__PURE__ */new Map();
  let order = 0;
  for (const binding of (0, _bindingsDSOcFD4j.n)(cfg)) {
...
function normalizePeerConstraint(peer) {
  ...
  const kind = (0, _chatTypeDAU8eOlz.t)(peer.kind);
...
function normalizeBindingMatch(match) {
  ...
  return {
    ...
    roles: (0, _bindingScopeAmGt2T8S.r)(rawRoles)
  };
}
...
function matchesBindingScope(match, scope) {
  ...
  if (match.peer.state === "valid") {
    if (!scope.peer || !(0, _peerKindMatchFF6fKm3k.t)(match.peer.kind, scope.peer.kind) || scope.peer.id !== match.peer.id) return false;
  }
  if (match.peer.state === "wildcard-kind") {
    if (!scope.peer || !(0, _peerKindMatchFF6fKm3k.t)(match.peer.kind, scope.peer.kind)) return false;
  }
  return (0, _bindingScopeAmGt2T8S.a)(match, scope);
}
function resolveAgentRoute(input) {
  const channel = normalizeToken(input.channel);
  const accountId = (0, _accountIdC3j_3_su.n)(input.accountId);
  const peer = input.peer ? {
    kind: (0, _chatTypeDAU8eOlz.t)(input.peer.kind) ?? input.peer.kind,
  ...
  const shouldLogDebug = (0, _globalsDeRFSEIV.a)();
  const parentPeer = input.parentPeer ? {
    kind: (0, _chatTypeDAU8eOlz.t)(input.parentPeer.kind) ?? input.parentPeer.kind,
  ...
  const choose = (agentId, matchedBy) => {
    ...
    const sessionKey = (0, _stringCoerceC1IzJjqi.a)(buildAgentSessionKey({
    ...
    const mainSessionKey = (0, _stringCoerceC1IzJjqi.a)((0, _sessionKeyEpIbK3Oz.r)({
      agentId: resolvedAgentId,
      mainKey: _sessionKeyEpIbK3Oz.n
    }));
    const route = {
  ...
  if (shouldLogDebug) {
    (0, _loggerBvytzgs.t)(`[routing] resolveAgentRoute: channel=${channel} accountId=${accountId} peer=${formatPeer(peer)} guildId=${guildId || "none"} teamId=${teamId || "none"} bindings=${bindings.length}`);
    for (const entry of bindings) (0, _loggerBvytzgs.t)(`[routing] binding: agentId=${entry.binding.agentId} accountPattern=${entry.match.accountPattern || "default"} peer=${formatNormalizedPeer(entry.match.peer)} guildId=${entry.match.guildId ?? "none"} teamId=${entry.match.teamId ?? "none"} roles=${entry.match.roles?.length ?? 0}`);
  }
  const baseScope = {
    guildId,
    teamId,
  ...
  for (const tier of tiers) {
    ...
    if (matched) {
      if (shouldLogDebug) (0, _loggerBvytzgs.t)(`[routing] match: matchedBy=${tier.matchedBy} agentId=${matched.binding.agentId}`);
      return choose(matched.binding.agentId, tier.matchedBy);
    }
  }
  return choose((0, _agentScope_6dFncNS.x)(input.cfg), "default");
}
//#endregion /* v9-633ddb2b3504456f */
