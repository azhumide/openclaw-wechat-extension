"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveChannelDefaultAccountId;exports.n = formatPairingApproveHint;exports.r = parseOptionalDelimitedEntries;exports.t = buildAccountScopedDmSecurityPolicy;var _commandFormatLSnUCVVF = require("./command-format-LSnUCVVF.js");
...
function formatPairingApproveHint(channelId) {
  return `Approve via: ${(0, _commandFormatLSnUCVVF.t)(`openclaw pairing list ${channelId}`)} / ${(0, _commandFormatLSnUCVVF.t)(`openclaw pairing approve ${channelId} <code>`)}`;
}
...
function buildAccountScopedDmSecurityPolicy(params) {
  ...
  const defaultBasePath = `channels.${params.channelKey}.accounts.${_accountIdBM1T.t}.`;
  const accountConfig = channelConfig?.accounts?.[resolvedAccountId];
  const defaultAccountConfig = params.inheritSharedDefaultsFromDefaultAccount && resolvedAccountId !== "default" ? channelConfig?.accounts?.[_accountIdBM1T.t] : void 0;
