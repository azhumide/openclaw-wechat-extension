...
function resolveDefaultLogDir() {
  return canUseNodeFs() ? (0, _tmpOpenclawDirCraDYfRT.n)() : _tmpOpenclawDirCraDYfRT.t;
}
function resolveDefaultLogFile(defaultLogDir) {
  return canUseNodeFs() ? _nodePath.default.join(defaultLogDir, "openclaw.log") : `${_tmpOpenclawDirCraDYfRT.t}/openclaw.log`;
}
...
function sanitizeDiagnosticLogText(value, maxChars) {
  return clampDiagnosticLogText((0, _redactDVSkTguP.r)(clampDiagnosticLogText(value, maxChars)), maxChars);
}
...
function assignDiagnosticLogAttribute(attributes, state, key, value) {
  ...
  if ((0, _prototypeKeysB9rjoUHr.t)(normalizedKey)) return;
  if ((0, _redactDVSkTguP.r)(normalizedKey) !== normalizedKey) return;
...
function normalizeTraceContext(value) {
  ...
  if (!(0, _diagnosticEventsBwpOTDQH._)(candidate.traceId)) return;
  if (candidate.spanId !== void 0 && !(0, _diagnosticEventsBwpOTDQH.h)(candidate.spanId)) return;
  if (candidate.parentSpanId !== void 0 && !(0, _diagnosticEventsBwpOTDQH.h)(candidate.parentSpanId)) return;
  if (candidate.traceFlags !== void 0 && !(0, _diagnosticEventsBwpOTDQH.g)(candidate.traceFlags)) return;
  return {
...
function buildTraceFileLogFields(logObj) {
  ...
  const trace = findLogTraceContext(bindings, args) ?? (0, _diagnosticEventsBwpOTDQH.m)();
  if (!trace) return;
  return {
...
function buildDiagnosticLogRecord(logObj) {
  ...
  const trace = findLogTraceContext(bindings, numericArgs) ?? (0, _diagnosticEventsBwpOTDQH.m)();
...
function attachDiagnosticEventTransport(logger) {
  logger.attachTransport((logObj) => {
    try {
      (0, _diagnosticEventsBwpOTDQH.n)(buildDiagnosticLogRecord(logObj));
    } catch {}
  });
}
...
function resolveSettings() {
  ...
  const cfg = loggingState.overrideSettings ?? (0, _redactDVSkTguP.s)();
...
function buildLogger(settings) {
  ...
  logger.attachTransport((logObj) => {
    try {
      ...
      const payload = `${(0, _redactDVSkTguP.r)(JSON.stringify({
        ...logObj,
        time,
        ...structuredFields,
        ...traceFields
...
const __test__ = exports.r = { shouldSkipMutatingLoggingConfigRead: _redactDVSkTguP.c };
