"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolvePackageExtensionEntries;exports.c = planManifestModelCatalogSuppressions;exports.i = loadPluginManifest;exports.l = loadOpenClawProviderIndex;exports.n = void 0;exports.o = planProviderIndexModelCatalogRows;exports.r = getPackageManifestMetadata;exports.s = planManifestModelCatalogRows;exports.t = void 0;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _utilsDvkbxKCZ = require("./utils-DvkbxKCZ.js");
var _boundaryFileRead6rU9DotD = require("./boundary-file-read-6rU9DotD.js");
var _stringNormalizationBgTtzLL = require("./string-normalization-BgTtzLL7.js");
var _legacyNamesPaIj6vU = require("./legacy-names-PaIj6vU5.js");
var _prototypeKeysB9rjoUHr = require("./prototype-keys-B9rjoUHr.js");
var _normalizeCtLIxVHe = require("./normalize-CtLIxVHe.js");
var _npmRegistrySpecBaUygPdz = require("./npm-registry-spec-BaUygPdz.js");
var _parseJsonCompatCwuU8jsj = require("./parse-json-compat-CwuU8jsj.js");
var _manifestCommandAliasesBJmsGUoA = require("./manifest-command-aliases-BJmsGUoA.js");
...
function normalizeSafeKey(value) {
  ...
  return key && !(0, _prototypeKeysB9rjoUHr.t)(key) ? key : "";
}
function normalizeInstall(value) {
  if (!(0, _utilsDvkbxKCZ.c)(value)) return;
  const npmSpec = (0, _stringCoerceBje8XVt.c)(value.npmSpec);
  const parsed = npmSpec ? (0, _npmRegistrySpecBaUygPdz.r)(npmSpec) : null;
  if (!parsed) return;
...
function normalizeCategories(value) {
  return [...new Set((0, _stringNormalizationBgTtzLL.l)(value))];
}
function normalizePreviewCatalog(params) {
  const provider = (0, _normalizeCtLIxVHe.t)({ providers: { [params.providerId]: params.value } }, { ownedProviders: new Set([params.providerId]) })?.providers?.[params.providerId];
  if (!provider) return;
  for (const model of provider.models) model.status ??= "preview";
  return provider;
}
function normalizeOnboardingScopes(value) {
  const scopes = (0, _stringNormalizationBgTtzLL.l)(value).filter((scope) => scope === "text-inference" || scope === "image-generation");
...
function normalizeAuthChoice(params) {
  if (!(0, _utilsDvkbxKCZ.c)(params.value)) return;
...
function normalizeProvider(rawProviderId, value) {
  if (!(0, _utilsDvkbxKCZ.c)(value)) return;
  const providerId = (0, _normalizeCtLIxVHe.i)(rawProviderId);
  if (!providerId) return;
  const id = (0, _normalizeCtLIxVHe.i)((0, _stringCoerceBje8XVt.c)(value.id) ?? "");
...
function normalizeOpenClawProviderIndex(value) {
  if (!(0, _utilsDvkbxKCZ.c)(value) || value.version !== OPENCLAW_PROVIDER_INDEX_VERSION) return;
  if (!(0, _utilsDvkbxKCZ.c)(value.providers)) return;
  const providers = {};
  for (const [rawProviderId, rawProvider] of Object.entries(value.providers)) {
    const providerId = (0, _normalizeCtLIxVHe.i)(rawProviderId);
    if (!providerId || (0, _prototypeKeysB9rjoUHr.t)(providerId)) continue;
...
function planManifestModelCatalogRows(params) {
  const providerFilter = params.providerFilter ? (0, _normalizeCtLIxVHe.i)(params.providerFilter) : void 0;
  const entries = [];
...
function planManifestModelCatalogPluginEntries(params) {
  ...
  return Object.entries(providers).flatMap(([provider, providerCatalog]) => {
    const normalizedProvider = (0, _normalizeCtLIxVHe.i)(provider);
    ...
    return plannedProviders.flatMap((plannedProvider) => {
      const rows = (0, _normalizeCtLIxVHe.n)({
...
function buildOwnedProviderSet(plugin) {
  return new Set((plugin.providers ?? []).map(_normalizeCtLIxVHe.i).filter(Boolean));
}
function buildModelCatalogProviderAliasTargets(plugin) {
  ...
  for (const [rawAlias, alias] of Object.entries(plugin.modelCatalog?.aliases ?? {})) {
    const aliasProvider = (0, _normalizeCtLIxVHe.i)(rawAlias);
    const targetProvider = (0, _normalizeCtLIxVHe.i)(alias.provider);
...
function planManifestModelCatalogSuppressions(params) {
  ...
  for (const plugin of params.registry.plugins) {
    ...
    for (const suppression of plugin.modelCatalog?.suppressions ?? []) {
      const provider = (0, _normalizeCtLIxVHe.i)(suppression.provider);
      ...
      suppressions.push({
        ...
        provider,
        model,
        mergeKey: (0, _normalizeCtLIxVHe.r)(provider, model),
...
function planProviderIndexModelCatalogRows(params) {
  ...
  for (const [providerId, provider] of Object.entries(params.index.providers)) {
    const normalizedProvider = (0, _normalizeCtLIxVHe.i)(providerId);
...
function normalizeStringListRecord(value) {
  ...
  for (const [key, rawValues] of Object.entries(value)) {
    ...
    if (!providerId || (0, _prototypeKeysB9rjoUHr.t)(providerId)) continue;
    const values = (0, _stringNormalizationBgTtzLL.l)(rawValues);
...
function normalizeStringRecord(value) {
  ...
  for (const [rawKey, rawValue] of Object.entries(value)) {
    ...
    if (!key || (0, _prototypeKeysB9rjoUHr.t)(key) || !value) continue;
...
function normalizeMediaUnderstandingCapabilities(value) {
  const values = (0, _stringNormalizationBgTtzLL.l)(value).filter((entry) => MEDIA_UNDERSTANDING_CAPABILITIES.has(entry));
  return values.length > 0 ? values : void 0;
}
function normalizeMediaUnderstandingNativeDocumentInputs(value) {
  const values = (0, _stringNormalizationBgTtzLL.l)(value).filter((entry) => entry === "pdf");
  return values.length > 0 ? values : void 0;
}
function normalizeMediaUnderstandingProviderMetadata(value) {
  ...
  for (const [rawProviderId, rawMetadata] of Object.entries(value)) {
    ...
    if (!providerId || (0, _prototypeKeysB9rjoUHr.t)(providerId) || !(0, _utilsDvkbxKCZ.c)(rawMetadata)) continue;
...
function normalizeManifestContracts(value) {
  if (!(0, _utilsDvkbxKCZ.c)(value)) return;
  const embeddedExtensionFactories = (0, _stringNormalizationBgTtzLL.l)(value.embeddedExtensionFactories);
  const agentToolResultMiddleware = (0, _stringNormalizationBgTtzLL.l)(value.agentToolResultMiddleware);
  const externalAuthProviders = (0, _stringNormalizationBgTtzLL.l)(value.externalAuthProviders);
  const memoryEmbeddingProviders = (0, _stringNormalizationBgTtzLL.l)(value.memoryEmbeddingProviders);
  const speechProviders = (0, _stringNormalizationBgTtzLL.l)(value.speechProviders);
  const realtimeTranscriptionProviders = (0, _stringNormalizationBgTtzLL.l)(value.realtimeTranscriptionProviders);
  const realtimeVoiceProviders = (0, _stringNormalizationBgTtzLL.l)(value.realtimeVoiceProviders);
  const mediaUnderstandingProviders = (0, _stringNormalizationBgTtzLL.l)(value.mediaUnderstandingProviders);
  const documentExtractors = (0, _stringNormalizationBgTtzLL.l)(value.documentExtractors);
  const imageGenerationProviders = (0, _stringNormalizationBgTtzLL.l)(value.imageGenerationProviders);
  const videoGenerationProviders = (0, _stringNormalizationBgTtzLL.l)(value.videoGenerationProviders);
  const musicGenerationProviders = (0, _stringNormalizationBgTtzLL.l)(value.musicGenerationProviders);
  const webContentExtractors = (0, _stringNormalizationBgTtzLL.l)(value.webContentExtractors);
  const webFetchProviders = (0, _stringNormalizationBgTtzLL.l)(value.webFetchProviders);
  const webSearchProviders = (0, _stringNormalizationBgTtzLL.l)(value.webSearchProviders);
  const migrationProviders = (0, _stringNormalizationBgTtzLL.l)(value.migrationProviders);
  const tools = (0, _stringNormalizationBgTtzLL.l)(value.tools);
  const contracts = {
...
function normalizeManifestConfigContracts(value) {
  if (!(0, _utilsDvkbxKCZ.c)(value)) return;
  const compatibilityMigrationPaths = (0, _stringNormalizationBgTtzLL.l)(value.compatibilityMigrationPaths);
  const compatibilityRuntimePaths = (0, _stringNormalizationBgTtzLL.l)(value.compatibilityRuntimePaths);
  const rawSecretInputs = (0, _utilsDvkbxKCZ.c)(value.secretInputs) ? value.secretInputs : void 0;
...
function normalizeManifestModelSupport(value) {
  if (!(0, _utilsDvkbxKCZ.c)(value)) return;
  const modelPrefixes = (0, _stringNormalizationBgTtzLL.l)(value.modelPrefixes);
  const modelPatterns = (0, _stringNormalizationBgTtzLL.l)(value.modelPatterns);
  const modelSupport = {
...
function normalizeManifestModelPricingSource(value) {
  ...
  if (!(0, _utilsDvkbxKCZ.c)(value)) return;
  const provider = (0, _normalizeCtLIxVHe.i)((0, _stringCoerceBje8XVt.c)(value.provider) ?? "");
  const modelIdTransforms = (0, _stringNormalizationBgTtzLL.l)(value.modelIdTransforms).filter((entry) => entry === "version-dots");
  const source = {
    ...(provider ? { provider } : {}),
...
function normalizeManifestModelPricing(value, params) {
  if (!(0, _utilsDvkbxKCZ.c)(value) || !(0, _utilsDvkbxKCZ.c)(value.providers)) return;
  const ownedProviders = new Set([...params.ownedProviders].map((provider) => (0, _normalizeCtLIxVHe.i)(provider)).filter(Boolean));
  const providers = {};
...
function normalizeManifestModelIdPrefixRules(value) {
  ...
  for (const rawRule of value) {
    if (!(0, _utilsDvkbxKCZ.c)(rawRule)) continue;
...
function normalizeManifestModelIdNormalizationProvider(value) {
  if (!(0, _utilsDvkbxKCZ.c)(value)) return;
  const aliases = {};
  if ((0, _utilsDvkbxKCZ.c)(value.aliases)) for (const [rawAlias, rawCanonical] of Object.entries(value.aliases)) {
    const alias = (0, _normalizeCtLIxVHe.i)(rawAlias);
    const canonical = (0, _stringCoerceBje8XVt.c)(rawCanonical);
    if (alias && canonical) aliases[alias] = canonical;
  }
  const stripPrefixes = (0, _stringNormalizationBgTtzLL.l)(value.stripPrefixes);
...
function normalizeManifestProviderEndpoints(value) {
  ...
  for (const rawEndpoint of value) {
    if (!(0, _utilsDvkbxKCZ.c)(rawEndpoint)) continue;
    const endpointClass = (0, _stringCoerceBje8XVt.c)(rawEndpoint.endpointClass);
    if (!endpointClass) continue;
    const hosts = (0, _stringNormalizationBgTtzLL.l)(rawEndpoint.hosts).map((host) => host.toLowerCase());
    const hostSuffixes = (0, _stringNormalizationBgTtzLL.l)(rawEndpoint.hostSuffixes).map((host) => host.toLowerCase());
    const baseUrls = (0, _stringNormalizationBgTtzLL.l)(rawEndpoint.baseUrls);
...
function normalizeManifestProviderRequestProvider(value) {
  ...
  const supportsStreamingUsage = (0, _utilsDvkbxKCZ.c)(value.openAICompletions) ? value.openAICompletions.supportsStreamingUsage : void 0;
...
function normalizeManifestActivation(value) {
  if (!(0, _utilsDvkbxKCZ.c)(value)) return;
  const onProviders = (0, _stringNormalizationBgTtzLL.l)(value.onProviders);
  const onAgentHarnesses = (0, _stringNormalizationBgTtzLL.l)(value.onAgentHarnesses);
  const onCommands = (0, _stringNormalizationBgTtzLL.l)(value.onCommands);
  const onChannels = (0, _stringNormalizationBgTtzLL.l)(value.onChannels);
  const onRoutes = (0, _stringNormalizationBgTtzLL.l)(value.onRoutes);
  const onConfigPaths = (0, _stringNormalizationBgTtzLL.l)(value.onConfigPaths);
  const onStartup = typeof value.onStartup === "boolean" ? value.onStartup : void 0;
  const onCapabilities = (0, _stringNormalizationBgTtzLL.l)(value.onCapabilities).filter((capability) => capability === "provider" || capability === "channel" || capability === "tool" || capability === "hook");
  const activation = {
...
function normalizeManifestSetupProviders(value) {
  ...
  for (const entry of value) {
    ...
    if (!id) continue;
    const authMethods = (0, _stringNormalizationBgTtzLL.l)(entry.authMethods);
    const envVars = (0, _stringNormalizationBgTtzLL.l)(entry.envVars);
    normalized.push({
      id,
...
function normalizeManifestSetup(value) {
  ...
  const cliBackends = (0, _stringNormalizationBgTtzLL.l)(value.cliBackends);
  const configMigrations = (0, _stringNormalizationBgTtzLL.l)(value.configMigrations);
...
function normalizeProviderAuthChoices(value) {
  ...
  for (const entry of value) {
    ...
    const deprecatedChoiceIds = (0, _stringNormalizationBgTtzLL.l)(entry.deprecatedChoiceIds);
    ...
    const onboardingScopes = (0, _stringNormalizationBgTtzLL.l)(entry.onboardingScopes).filter((scope) => scope === "text-inference" || scope === "image-generation");
    normalized.push({
      provider,
      method,
      choiceId,
...
function normalizeChannelConfigs(value) {
  ...
  for (const [key, rawEntry] of Object.entries(value)) {
    ...
    if (!channelId || (0, _prototypeKeysB9rjoUHr.t)(channelId) || !(0, _utilsDvkbxKCZ.c)(rawEntry)) continue;
    const schema = (0, _utilsDvkbxKCZ.c)(rawEntry.schema) ? rawEntry.schema : null;
    if (!schema) continue;
    const uiHints = (0, _utilsDvkbxKCZ.c)(rawEntry.uiHints) ? rawEntry.uiHints : void 0;
    const runtime = (0, _utilsDvkbxKCZ.c)(rawEntry.runtime) && typeof rawEntry.runtime.safeParse === "function" ? rawEntry.runtime : void 0;
    const label = (0, _stringCoerceBje8XVt.c)(rawEntry.label) ?? "";
    const description = (0, _stringCoerceBje8XVt.c)(rawEntry.description) ?? "";
    const preferOver = (0, _stringNormalizationBgTtzLL.l)(rawEntry.preferOver);
...
function loadPluginManifest(rootDir, rejectHardlinks = true, rootRealPath) {
  ...
  if (!opened.ok) return (0, _boundaryFileRead6rU9DotD.n)(opened, {
    path: () => ({
      ok: false,
  ...
  let raw;
  try {
    raw = (0, _parseJsonCompatCwuU8jsj.t)(_nodeFs.default.readFileSync(opened.fd, "utf-8"));
  } catch (err) {
    return {
      ok: false,
  ...
  if (!(0, _utilsDvkbxKCZ.c)(raw)) return {
    ok: false,
  ...
  const configSchema = (0, _utilsDvkbxKCZ.c)(raw.configSchema) ? raw.configSchema : null;
  ...
  const legacyPluginIds = (0, _stringNormalizationBgTtzLL.l)(raw.legacyPluginIds);
  const autoEnableWhenConfiguredProviders = (0, _stringNormalizationBgTtzLL.l)(raw.autoEnableWhenConfiguredProviders);
  ...
  const channels = (0, _stringNormalizationBgTtzLL.l)(raw.channels);
  const providers = (0, _stringNormalizationBgTtzLL.l)(raw.providers);
  const providerDiscoveryEntry = (0, _stringCoerceBje8XVt.c)(raw.providerDiscoveryEntry);
  const modelSupport = normalizeManifestModelSupport(raw.modelSupport);
  const modelCatalog = (0, _normalizeCtLIxVHe.t)(raw.modelCatalog, { ownedProviders: new Set(providers) });
  ...
  const cliBackends = (0, _stringNormalizationBgTtzLL.l)(raw.cliBackends);
  const syntheticAuthRefs = (0, _stringNormalizationBgTtzLL.l)(raw.syntheticAuthRefs);
  const nonSecretAuthMarkers = (0, _stringNormalizationBgTtzLL.l)(raw.nonSecretAuthMarkers);
  const commandAliases = (0, _manifestCommandAliasesBJmsGUoA.t)(raw.commandAliases);
  ...
  const skills = (0, _stringNormalizationBgTtzLL.l)(raw.skills);
  ...
  let uiHints;
  if ((0, _utilsDvkbxKCZ.c)(raw.uiHints)) uiHints = raw.uiHints;
  return {
    ok: true,
    manifest: {
      id,
...
function getPackageManifestMetadata(manifest) {
  if (!manifest) return;
  return manifest[_legacyNamesPaIj6vU.n];
}
