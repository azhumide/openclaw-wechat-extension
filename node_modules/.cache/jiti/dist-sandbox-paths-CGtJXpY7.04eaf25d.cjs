"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveSandboxedMediaSource;exports.i = resolveSandboxPath;exports.n = assertSandboxPath;exports.r = resolveSandboxInputPath;exports.t = assertMediaNotDataUrl;var _tmpOpenclawDirCWQcmOLf = require("./tmp-openclaw-dir-CWQcmOLf.js");
var _boundaryPathBSBSMt0u = require("./boundary-path-BSBSMt0u.js");
var _pathAliasGuardsCnakGAD = require("./path-alias-guards-CnakGAD2.js");
var _archivePathSJ6MaCv = require("./archive-path-SJ6-MaCv.js");
var _localFileAccessBPC1MUhk = require("./local-file-access-BPC1MUhk.js");
var _mediaSourceUrlCVvUCxg = require("./media-source-url-CVv-uCxg.js");
...
function hostPathLooksAbsolute(expanded) {
  return _nodePath.default.isAbsolute(expanded) || (0, _archivePathSJ6MaCv.t)(expanded);
}
function resolveToCwd(filePath, cwd) {
  const expanded = expandPath(filePath);
  if ((0, _archivePathSJ6MaCv.t)(expanded)) return _nodePath.default.win32.normalize(expanded);
...
function resolveSandboxPath(params) {
  ...
  if (relative.startsWith("..") || _nodePath.default.isAbsolute(relative) || (0, _archivePathSJ6MaCv.t)(relative)) throw new Error(`Path escapes sandbox root (${shortPath(rootResolved)}): ${params.filePath}`);
  return {
    resolved,
    relative
  };
...
async function assertSandboxPath(params) {
  ...
  await (0, _pathAliasGuardsCnakGAD.n)({
...
async function resolveSandboxedMediaSource(params) {
  ...
  if (!raw) return raw;
  if ((0, _mediaSourceUrlCVvUCxg.t)(raw)) return raw;
  let candidate = raw;
  if (/^file:\/\//i.test(candidate)) {
    ...
    try {
      candidate = (0, _localFileAccessBPC1MUhk.a)(candidate);
    } catch (err) {
  ...
  (0, _localFileAccessBPC1MUhk.t)(candidate, "Sandbox media path");
...
function mapContainerWorkspaceFileUrl(params) {
  ...
  if ((0, _localFileAccessBPC1MUhk.r)(parsed.pathname)) return;
...
async function resolveAllowedTmpMediaPath(params) {
  ...
  const openClawTmpDir = _nodePath.default.resolve((0, _tmpOpenclawDirCWQcmOLf.n)());
  if (!(0, _boundaryPathBSBSMt0u.s)(openClawTmpDir, resolved)) return;
