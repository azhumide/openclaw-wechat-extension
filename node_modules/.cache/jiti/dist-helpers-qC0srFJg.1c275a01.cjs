"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveChannelDefaultAccountId;exports.n = formatPairingApproveHint;exports.r = parseOptionalDelimitedEntries;exports.t = buildAccountScopedDmSecurityPolicy;var _commandFormatDXo6xcsW = require("./command-format-DXo6xcsW.js");
...
function formatPairingApproveHint(channelId) {
  return `Approve via: ${(0, _commandFormatDXo6xcsW.t)(`openclaw pairing list ${channelId}`)} / ${(0, _commandFormatDXo6xcsW.t)(`openclaw pairing approve ${channelId} <code>`)}`;
}
...
function buildAccountScopedDmSecurityPolicy(params) {
  ...
  const defaultBasePath = `channels.${params.channelKey}.accounts.${_accountIdCKs_R3Vn.t}.`;
  const accountConfig = channelConfig?.accounts?.[resolvedAccountId];
  const defaultAccountConfig = params.inheritSharedDefaultsFromDefaultAccount && resolvedAccountId !== "default" ? channelConfig?.accounts?.[_accountIdCKs_R3Vn.t] : void 0;
