...
var _workspaceDefaultBsU85q1D = require("./workspace-default-BsU85q1D.js");
var _modelInputUXpHvROZ = require("./model-input-uXpHvROZ.js");
var _agentFilterDpnVatGR = require("./agent-filter-DpnVatGR.js");
...
function warnMultipleDefaultAgents() {
  Promise.resolve().then(() => jitiImport("./subsystem-Cyh-zhf1.js").then((m) => _interopRequireWildcard(m))).then(({ createSubsystemLogger }) => {
...
function listAgentIds(cfg) {
  ...
  if (agents.length === 0) return [_sessionKeyBSlq04Dw.t];
  const seen = /* @__PURE__ */new Set();
  const ids = [];
  for (const entry of agents) {
    const id = (0, _sessionKeyBSlq04Dw.c)(entry?.id);
    if (seen.has(id)) continue;
    seen.add(id);
    ids.push(id);
  }
  return ids.length > 0 ? ids : [_sessionKeyBSlq04Dw.t];
}
function resolveDefaultAgentId(cfg) {
  ...
  return (0, _sessionKeyBSlq04Dw.c)(chosen || "main");
}
function resolveAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyBSlq04Dw.c)(agentId);
  return listAgentEntries(cfg).find((entry) => (0, _sessionKeyBSlq04Dw.c)(entry.id) === id);
}
function resolveAgentConfig(cfg, agentId) {
  const entry = resolveAgentEntry(cfg, (0, _sessionKeyBSlq04Dw.c)(agentId));
  if (!entry) return;
...
function resolveAgentWorkspaceDir(cfg, agentId, env = process.env) {
  ...
  if (configured) return stripNullBytes$1((0, _utilsCCskKJVV.p)(configured, env));
  const defaultAgentId = resolveDefaultAgentId(cfg);
  const fallback = cfg.agents?.defaults?.workspace?.trim();
  if (id === defaultAgentId) {
    if (fallback) return stripNullBytes$1((0, _utilsCCskKJVV.p)(fallback, env));
    return stripNullBytes$1((0, _workspaceDefaultBsU85q1D.n)(env));
  }
  if (fallback) return stripNullBytes$1(_nodePath.default.join((0, _utilsCCskKJVV.p)(fallback, env), id));
...
function resolveAgentDir(cfg, agentId, env = process.env) {
  ...
  if (configured) return (0, _utilsCCskKJVV.p)(configured, env);
...
function resolveSessionAgentIds(params) {
  ...
  const explicitAgentId = explicitAgentIdRaw ? (0, _sessionKeyBSlq04Dw.c)(explicitAgentIdRaw) : null;
  const sessionKey = params.sessionKey?.trim();
  const normalizedSessionKey = sessionKey ? (0, _stringCoerceBje8XVt.a)(sessionKey) : void 0;
  const parsed = normalizedSessionKey ? (0, _sessionKeyUtilsB7QL53SS.o)(normalizedSessionKey) : null;
  return {
    defaultAgentId,
    sessionAgentId: explicitAgentId ?? (parsed?.agentId ? (0, _sessionKeyBSlq04Dw.c)(parsed.agentId) : defaultAgentId)
  };
}
...
function resolveAgentSkillsFilter(cfg, agentId) {
  return (0, _agentFilterDpnVatGR.t)(cfg, agentId);
}
...
function findMutableAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyBSlq04Dw.c)(agentId);
  return cfg.agents?.list?.find((entry) => (0, _sessionKeyBSlq04Dw.c)(entry?.id) === id);
}
...
function resolveFallbackAgentId(params) {
  ...
  if (explicitAgentId) return (0, _sessionKeyBSlq04Dw.c)(explicitAgentId);
  return (0, _sessionKeyBSlq04Dw.u)(params.sessionKey);
}
...
function hasConfiguredModelFallbacks(params) {
  ...
  const defaultFallbacks = (0, _modelInputUXpHvROZ.t)(params.cfg?.agents?.defaults?.model);
...
function normalizePathForComparison(input) {
  const resolved = _nodePath.default.resolve(stripNullBytes((0, _utilsCCskKJVV.p)(input)));
