"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = resolveBundledPluginsDir;var _stringCoerceCjxCKZ6B = require("./string-coerce-CjxCKZ6B.js");
...
function bundledPluginsDisabled(env) {
  const raw = (0, _stringCoerceCjxCKZ6B.s)(env.OPENCLAW_DISABLE_BUNDLED_PLUGINS);
...
function resolveBundledPluginsDir(env = process.env) {
  ...
  if (override) {
    const resolvedOverride = (0, _utilsCB8xp0O.m)(override, env);
    if (_nodeFs.default.existsSync(resolvedOverride)) return resolvedOverride;
    try {
      const argvPackageRoot = (0, _openclawRootGhlHRwjz.n)({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
    (0, _openclawRootGhlHRwjz.n)({ argv1: process.argv[1] }),
    (0, _openclawRootGhlHRwjz.n)({ cwd: process.cwd() }),
    (0, _openclawRootGhlHRwjz.n)({ moduleUrl: "file:///home/rs/openclaw/dist/bundled-dir-U738ay4K.js" })].
  ...
  try {
    let cursor = _nodePath.default.dirname((0, _nodeUrl.fileURLToPath)("file:///home/rs/openclaw/dist/bundled-dir-U738ay4K.js"));
    for (let i = 0; i < 6; i += 1) {
