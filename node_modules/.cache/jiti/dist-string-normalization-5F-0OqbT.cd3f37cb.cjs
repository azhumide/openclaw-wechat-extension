...
function normalizeStringEntries(list) {
  return (list ?? []).map((entry) => (0, _stringCoerceCjxCKZ6B.c)(String(entry)) ?? "").filter(Boolean);
}
function normalizeStringEntriesLower(list) {
  return normalizeStringEntries(list).map((entry) => (0, _stringCoerceCjxCKZ6B.s)(entry) ?? "");
}
function normalizeTrimmedStringList(value) {
  if (!Array.isArray(value)) return [];
  return value.flatMap((entry) => {
    const normalized = (0, _stringCoerceCjxCKZ6B.c)(entry);
...
function normalizeSingleOrTrimmedStringList(value) {
  ...
  const normalized = (0, _stringCoerceCjxCKZ6B.c)(value);
  return normalized ? [normalized] : [];
}
...
function normalizeHyphenSlug(raw) {
  const trimmed = (0, _stringCoerceCjxCKZ6B.s)(raw) ?? "";
  if (!trimmed) return "";
