"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveBundledRuntimeDependencyInstallRoot;exports.c = scanBundledPluginRuntimeDeps;exports.i = repairBundledRuntimeDepsInstallRoot;exports.n = installBundledRuntimeDeps;exports.o = resolveBundledRuntimeDependencyPackageInstallRoot;exports.r = registerBundledRuntimeDependencyNodePath;exports.s = resolveBundledRuntimeDependencyPackageRoot;exports.t = ensureBundledPluginRuntimeDeps;var _stringCoerceC1IzJjqi = require("./string-coerce-C1IzJjqi.js");
var _homeDirCfZ08GnU = require("./home-dir-CfZ08GnU.js");
var _pathsBG0ad0P = require("./paths-BG0ad0P6.js");
var _configStateUvzNc3si = require("./config-state-UvzNc3si.js");
...
const semver = (0, _nodeModule.createRequire)("file:///home/rs/openclaw/dist/bundled-runtime-deps-BdEAdjwi.js")("semver");
...
function readRuntimeDepsLockOwner(lockDir) {
  const owner = readJsonObject(_nodePath.default.join(lockDir, BUNDLED_RUNTIME_DEPS_LOCK_OWNER_FILE));
  return {
...
function resolveBundledRuntimeDepsExternalBaseDir(env) {
  ...
  if (explicit) return (0, _homeDirCfZ08GnU.r)(explicit, {
    env,
  ...
  return _nodePath.default.join((0, _pathsBG0ad0P._)(env, _nodeOs.default.homedir), "plugin-runtime-deps");
}
...
function restoreSourceCheckoutRuntimeDepsFromCache(params) {
  ...
  try {
    replaceNodeModulesDir(_nodePath.default.join(params.installRoot, "node_modules"), cachedNodeModulesDir);
    return true;
  } catch {
    return false;
  }
...
function resolveBundledRuntimeDepsNpmRunner(params) {
  ...
  const rawNpmExecPath = (0, _stringCoerceC1IzJjqi.s)(env.npm_execpath) ? env.npm_execpath : void 0;
  const npmCliPath = [
...
function readBundledPluginChannels(pluginDir) {
  const channels = readJsonObject(_nodePath.default.join(pluginDir, "openclaw.plugin.json"))?.channels;
  if (!Array.isArray(channels)) return [];
  return channels.filter((entry) => typeof entry === "string" && entry !== "");
}
function readBundledPluginEnabledByDefault(pluginDir) {
  return readJsonObject(_nodePath.default.join(pluginDir, "openclaw.plugin.json"))?.enabledByDefault === true;
}
function isBundledPluginConfiguredForRuntimeDeps(params) {
  const plugins = (0, _configStateUvzNc3si.o)(params.config.plugins);
  ...
  for (const channelId of readBundledPluginChannels(params.pluginDir)) {
    const normalizedChannelId = (0, _stringCoerceC1IzJjqi.s)(channelId);
...
function normalizePluginIdSet(pluginIds) {
  if (!pluginIds) return;
  const normalized = pluginIds.map((entry) => (0, _stringCoerceC1IzJjqi.s)(entry)).filter((entry) => Boolean(entry));
...
function installBundledRuntimeDeps(params) {
  ...
  try {
    ...
    if (isolatedExecutionRoot) _nodeFs.default.writeFileSync(_nodePath.default.join(installExecutionRoot, "package.json"), `${JSON.stringify({
    ...
    if (isolatedExecutionRoot) {
      ...
      replaceNodeModulesDir(_nodePath.default.join(params.installRoot, "node_modules"), stagedNodeModulesDir);
    }
  } finally {
...
function ensureBundledPluginRuntimeDeps(params) {
  ...
  return withBundledRuntimeDepsInstallRootLock(installRoot, () => {
    ...
    const installExecutionRoot = (cacheDir && isPluginRootInstall && resolveSourceCheckoutBundledPluginPackageRoot(params.pluginRoot) ? cacheDir : void 0) ?? (isPluginRootInstall ? _nodePath.default.join(installRoot, PLUGIN_ROOT_INSTALL_STAGE_DIR) : void 0);
