"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = withNormalizedTimestamp;exports.i = resolveUserTimezone;exports.n = normalizeTimestamp;exports.r = resolveUserTimeFormat;exports.t = formatUserTime;var _nodeChild_process = require("node:child_process");
...
function normalizeTimestamp(raw) {
  if (raw == null) return;
  let timestampMs;
  if (raw instanceof Date) timestampMs = raw.getTime();else
  if (typeof raw === "number" && Number.isFinite(raw)) timestampMs = raw < 0xe8d4a51000 ? Math.round(raw * 1e3) : Math.round(raw);else
  if (typeof raw === "string") {
    const trimmed = raw.trim();
    if (!trimmed) return;
    if (/^\d+(\.\d+)?$/.test(trimmed)) {
      const num = Number(trimmed);
      if (Number.isFinite(num)) if (trimmed.includes(".")) timestampMs = Math.round(num * 1e3);else
      if (trimmed.length >= 13) timestampMs = Math.round(num);else
...
function detectSystemTimeFormat() {
  if (process.platform === "darwin") try {
    const result = (0, _nodeChild_process.execFileSync)("defaults", [
    "read",
    "-g",
  ...
  if (process.platform === "win32") try {
    const result = (0, _nodeChild_process.execFileSync)("powershell", ["-Command", "(Get-Culture).DateTimeFormat.ShortTimePattern"], {
      encoding: "utf8",
      timeout: 1e3
    }).trim();
