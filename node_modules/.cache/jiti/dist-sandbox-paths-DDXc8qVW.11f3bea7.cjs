...
var _archivePathBUzT4xLE = require("./archive-path-BUzT4xLE.js");
var _pathAliasGuardsDu_7Bd2f = require("./path-alias-guards-Du_7Bd2f.js");
var _localFileAccessBfFCIMq = require("./local-file-access-Bf-fCIMq.js");
var _mediaSourceUrlBVFSCIa = require("./media-source-url-BVFSCIa2.js");
...
function hostPathLooksAbsolute(expanded) {
  return _nodePath.default.isAbsolute(expanded) || (0, _archivePathBUzT4xLE.t)(expanded);
}
function resolveToCwd(filePath, cwd) {
  const expanded = expandPath(filePath);
  if ((0, _archivePathBUzT4xLE.t)(expanded)) return _nodePath.default.win32.normalize(expanded);
...
function resolveSandboxPath(params) {
  ...
  if (relative.startsWith("..") || _nodePath.default.isAbsolute(relative) || (0, _archivePathBUzT4xLE.t)(relative)) throw new Error(`Path escapes sandbox root (${shortPath(rootResolved)}): ${params.filePath}`);
  return {
    resolved,
    relative
  };
...
async function assertSandboxPath(params) {
  ...
  await (0, _pathAliasGuardsDu_7Bd2f.n)({
...
async function resolveSandboxedMediaSource(params) {
  ...
  if (!raw) return raw;
  if ((0, _mediaSourceUrlBVFSCIa.t)(raw)) return raw;
  let candidate = raw;
  if (/^file:\/\//i.test(candidate)) {
    ...
    try {
      candidate = (0, _localFileAccessBfFCIMq.a)(candidate);
    } catch (err) {
  ...
  (0, _localFileAccessBfFCIMq.t)(candidate, "Sandbox media path");
...
function mapContainerWorkspaceFileUrl(params) {
  ...
  if ((0, _localFileAccessBfFCIMq.r)(parsed.pathname)) return;
...
async function resolveAllowedTmpMediaPath(params) {
  ...
  const openClawTmpDir = _nodePath.default.resolve((0, _tmpOpenclawDirCLT34C.n)());
