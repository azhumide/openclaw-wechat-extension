...
const MODEL_CATALOG_APIS = new Set(_typesModelsBAlR6uU.t);
...
function normalizeSafeRecordKey(value) {
  ...
  return key && !(0, _prototypeKeysXqIkmvix.t)(key) ? key : "";
}
...
function normalizeModelCatalogInputs(value) {
  const inputs = (0, _stringNormalizationBrdBBNdu.l)(value).filter((input) => MODEL_CATALOG_INPUTS.has(input));
...
function normalizeModelCatalogTieredCost(value) {
  ...
  for (const entry of value) {
    if (!(0, _utilsDvkbxKCZ.c)(entry) || !Array.isArray(entry.range)) continue;
...
function normalizeModelCatalogCompat(value) {
  ...
  {
    const normalized = (0, _stringNormalizationBrdBBNdu.l)(value[field]);
    if (normalized.length > 0) compat[field] = normalized;
  }
  if ((0, _utilsDvkbxKCZ.c)(value.reasoningEffortMap)) {
...
function normalizeModelCatalogModel(value) {
  ...
  const replaces = (0, _stringNormalizationBrdBBNdu.l)(value.replaces);
  const replacedBy = (0, _stringCoerceBje8XVt.c)(value.replacedBy) ?? "";
  const tags = (0, _stringNormalizationBrdBBNdu.l)(value.tags);
  return {
    id,
    ...(name ? { name } : {}),
    ...(api ? { api } : {}),
...
function normalizeModelCatalogAliases(value, ownedProviders) {
  ...
  for (const [rawAlias, rawTarget] of Object.entries(value)) {
    ...
    if (!alias || !(0, _utilsDvkbxKCZ.c)(rawTarget)) continue;
...
function normalizeModelCatalogSuppressions(value) {
  ...
  for (const entry of value) {
    ...
    const rawWhen = (0, _utilsDvkbxKCZ.c)(entry.when) ? entry.when : void 0;
    const baseUrlHosts = (0, _stringNormalizationBrdBBNdu.l)(rawWhen?.baseUrlHosts).map((host) => host.toLowerCase());
    const providerConfigApiIn = (0, _stringNormalizationBrdBBNdu.l)(rawWhen?.providerConfigApiIn).map((api) => api.toLowerCase());
...
function normalizeStringList(value) {
  const normalized = (0, _stringNormalizationBrdBBNdu.l)(value);
  return normalized.length > 0 ? normalized : void 0;
}
