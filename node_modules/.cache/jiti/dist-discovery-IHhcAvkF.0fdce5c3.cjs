...
var _bundledDirJ2GlQTG = require("./bundled-dir-J2GlQTG6.js");
var _boundaryFileReadCgZlf7kr = require("./boundary-file-read-cgZlf7kr.js");
var _manifestC0Wa8IEc = require("./manifest-C0Wa8IEc.js");
var _pathSafetyCwoi_Anq = require("./path-safety-Cwoi_Anq.js");
var _packageEntryResolutionCunNEYlm = require("./package-entry-resolution-CunNEYlm.js");
...
function isSameOrInside(baseDir, targetPath) {
  ...
  return target === base || (0, _pathSafetyCwoi_Anq.n)(base, target);
}
...
function resolvePluginSourceRoots(params) {
  ...
  const workspaceRoot = params.workspaceDir ? (0, _utilsBYAfe1GS.p)(params.workspaceDir, env) : void 0;
  return {
    stock: (0, _bundledDirJ2GlQTG.t)(env),
    global: _nodePath.default.join((0, _utilsBYAfe1GS.d)(env), "extensions"),
...
function resolvePluginCacheInputs(params) {
  ...
  return {
    ...
    loadPaths: (params.loadPaths ?? []).filter((entry) => typeof entry === "string").map((entry) => entry.trim()).filter(Boolean).map((entry) => (0, _utilsBYAfe1GS.p)(entry, env))
  };
}
//#endregion
...
function checkSourceEscapesRoot(params) {
  const sourceRealPath = (0, _pathSafetyCwoi_Anq.r)(params.source);
  const rootRealPath = (0, _pathSafetyCwoi_Anq.r)(params.rootDir);
  if (!sourceRealPath || !rootRealPath) return null;
  if ((0, _pathSafetyCwoi_Anq.n)(rootRealPath, sourceRealPath)) return null;
  return {
...
function checkPathStatAndPermissions(params) {
  ...
  for (const targetPath of pathsToCheck) {
    ...
    seen.add(normalized);
    let stat = (0, _pathSafetyCwoi_Anq.i)(targetPath);
    if (!stat) return {
    ...
    if ((modeBits & 2) !== 0 && params.origin === "bundled") try {
      ...
      const repairedStat = (0, _pathSafetyCwoi_Anq.i)(targetPath);
...
function formatCandidateBlockMessage(issue) {
  ...
  if (issue.reason === "path_world_writable") return `blocked plugin candidate: world-writable path (${issue.targetPath}, mode=${(0, _pathSafetyCwoi_Anq.t)(issue.modeBits ?? 0)})`;
...
function resolvesToSameDirectory(left, right) {
  ...
  const leftRealPath = (0, _pathSafetyCwoi_Anq.r)(left);
  const rightRealPath = (0, _pathSafetyCwoi_Anq.r)(right);
...
function resolveIdHintManifestId(rootDir, rejectHardlinks) {
  const manifest = (0, _manifestC0Wa8IEc.i)(rootDir, rejectHardlinks);
...
function addCandidate(params) {
  ...
  const resolvedRoot = (0, _pathSafetyCwoi_Anq.r)(params.rootDir) ?? _nodePath.default.resolve(params.rootDir);
  ...
  params.candidates.push({
    ...
    packageManifest: (0, _manifestC0Wa8IEc.r)(manifest ?? void 0),
    bundledManifest: params.bundledManifest,
    bundledManifestPath: params.bundledManifestPath
  });
}
function discoverBundleInRoot(params) {
  const bundleFormat = (0, _pathSafetyCwoi_Anq.c)(params.rootDir);
  if (!bundleFormat) return "none";
  const bundleManifest = (0, _pathSafetyCwoi_Anq.l)({
    rootDir: params.rootDir,
    bundleFormat,
...
function discoverInDirectory(params) {
  ...
  const resolvedDir = (0, _pathSafetyCwoi_Anq.r)(params.dir) ?? _nodePath.default.resolve(params.dir);
  ...
  for (const entry of entries) {
    ...
    const extensionResolution = (0, _manifestC0Wa8IEc.a)(manifest ?? void 0);
    const extensions = extensionResolution.status === "ok" ? extensionResolution.entries : [];
    const manifestId = resolveIdHintManifestId(fullPath, rejectHardlinks);
    const setupSource = (0, _packageEntryResolutionCunNEYlm.n)({
      packageDir: fullPath,
      manifest,
      origin: params.origin,
    ...
    if (extensions.length > 0) {
      const resolvedRuntimeSources = (0, _packageEntryResolutionCunNEYlm.t)({
        packageDir: fullPath,
        manifest,
        extensions,
        origin: params.origin,
    ...
    const indexFile = [..._manifestC0Wa8IEc.t].map((candidate) => _nodePath.default.join(fullPath, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
...
function discoverFromPath(params) {
  const resolved = (0, _utilsBYAfe1GS.p)(params.rawPath, params.env);
  ...
  if (stat.isDirectory()) {
    ...
    const indexFile = [..._manifestC0Wa8IEc.t].map((candidate) => _nodePath.default.join(resolved, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
    if (indexFile && isExtensionFile(indexFile)) {
      addCandidate({
        candidates: params.candidates,
        diagnostics: params.diagnostics,
...
function discoverOpenClawPlugins(params) {
  ...
  const workspaceRoot = workspaceDir ? (0, _utilsBYAfe1GS.p)(workspaceDir, env) : void 0;
    ...
    load: () => {
      ...
      for (const extraPath of extra) {
        ...
        const bundledAlias = resolvePackagedBundledLoadPathAlias({
          ...
          loadPath: (0, _utilsBYAfe1GS.p)(trimmed, env)
        });
        if (bundledAlias) {
