...
function normalizePathList(value) {
  ...
  return value.map((entry) => (0, _stringCoerceCjxCKZ6B.c)(entry)).filter((entry) => Boolean(entry));
}
...
function hasInlineCapabilityValue(value) {
  ...
  if ((0, _utilsCB8xp0O.l)(value)) return Object.keys(value).length > 0;
  return value === true;
}
function slugifyPluginId(raw, rootDir) {
  const fallback = _nodePath.default.basename(rootDir);
  return ((0, _stringCoerceCjxCKZ6B.a)(raw) || (0, _stringCoerceCjxCKZ6B.a)(fallback)).replace(/[^a-z0-9]+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "") || "bundle-plugin";
}
function loadBundleManifestFile(params) {
  ...
  try {
    ...
    if (!(0, _utilsCB8xp0O.l)(raw)) return {
      ok: false,
...
function loadBundleManifest(params) {
  ...
  const interfaceRecord = (0, _utilsCB8xp0O.l)(raw.interface) ? raw.interface : void 0;
  const name = (0, _stringCoerceCjxCKZ6B.c)(raw.name);
  const description = (0, _stringCoerceCjxCKZ6B.c)(raw.description) ?? (0, _stringCoerceCjxCKZ6B.c)(raw.shortDescription) ?? (0, _stringCoerceCjxCKZ6B.c)(interfaceRecord?.shortDescription);
  const version = (0, _stringCoerceCjxCKZ6B.c)(raw.version);
...
function detectBundleManifestFormat(rootDir) {
  ...
  if (_manifestDgyExZJE.t.some((candidate) => _nodeFs.default.existsSync(_nodePath.default.join(rootDir, candidate)))) return null;
  if ([
...
function isPathInside(baseDir, targetPath) {
  return (0, _boundaryPathBSBSMt0u.s)(baseDir, targetPath);
}
