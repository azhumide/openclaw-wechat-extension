"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.detectZip = detectZip;var Token = _interopRequireWildcard(require("token-types"));
...
async function readZipDataDescriptorEntryWithLimit(zipHandler, { shouldBuffer, maximumLength = maximumZipEntrySizeInBytes } = {}) {
  ...
  if (!(0, _parser.hasUnknownFileSize)(zipHandler.tokenizer)) {
...
function getRemainingZipScanBudget(zipHandler, startOffset) {
  if ((0, _parser.hasUnknownFileSize)(zipHandler.tokenizer)) {
    return Math.max(0, _parser.maximumUntrustedSkipSizeInBytes - (zipHandler.tokenizer.position - startOffset));
  }
...
async function readZipEntryData(zipHandler, zipHeader, { shouldBuffer, maximumDescriptorLength = maximumZipEntrySizeInBytes } = {}) {
  ...
  if (!shouldBuffer) {
    await (0, _parser.safeIgnore)(zipHandler.tokenizer, zipHeader.compressedSize, {
      maximumLength: (0, _parser.hasUnknownFileSize)(zipHandler.tokenizer) ? maximumZipEntrySizeInBytes : zipHandler.tokenizer.fileInfo.size,
...
_inflate.ZipHandler.prototype.inflate = async function (zipHeader, fileData, callback) {
...
_inflate.ZipHandler.prototype.unzip = async function (fileCallback) {
  let stop = false;
  let zipEntryCount = 0;
  const zipScanStart = this.tokenizer.position;
  this.knownSizeDescriptorScannedBytes = 0;
  do {
    if ((0, _parser.hasExceededUnknownSizeScanBudget)(this.tokenizer, zipScanStart, _parser.maximumUntrustedSkipSizeInBytes)) {
      throw new _parser.ParserHardLimitError(`ZIP stream probing exceeds ${_parser.maximumUntrustedSkipSizeInBytes} bytes`);
    }
...
async function detectZip(tokenizer) {
  ...
  const iWorkFileType = (0, _parser.hasUnknownFileSize)(tokenizer) &&
