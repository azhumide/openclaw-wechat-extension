...
function hasInlineCapabilityValue(value) {
  ...
  if ((0, _utilsDvkbxKCZ.c)(value)) return Object.keys(value).length > 0;
  return value === true;
}
...
function loadBundleManifestFile(params) {
  ...
  if (!opened.ok) return (0, _boundaryFileReadC8TEdk8a.n)(opened, {
    path: () => {
...
function loadBundleManifest(params) {
  ...
  const interfaceRecord = (0, _utilsDvkbxKCZ.c)(raw.interface) ? raw.interface : void 0;
...
function detectBundleManifestFormat(rootDir) {
  ...
  if (_manifestDkU_xlZi.t.some((candidate) => _nodeFs.default.existsSync(_nodePath.default.join(rootDir, candidate)))) return null;
  if ([
...
function isPathInside(baseDir, targetPath) {
  return (0, _boundaryPath6aPPz8oV.s)(baseDir, targetPath);
}
