...
function normalizePathList(value) {
  ...
  return value.map((entry) => (0, _stringCoerceC1IzJjqi.c)(entry)).filter((entry) => Boolean(entry));
}
...
function hasInlineCapabilityValue(value) {
  ...
  if ((0, _utilsBMRcljdi.l)(value)) return Object.keys(value).length > 0;
  return value === true;
}
function slugifyPluginId(raw, rootDir) {
  const fallback = _nodePath.default.basename(rootDir);
  return ((0, _stringCoerceC1IzJjqi.a)(raw) || (0, _stringCoerceC1IzJjqi.a)(fallback)).replace(/[^a-z0-9]+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "") || "bundle-plugin";
}
...
function loadBundleManifest(params) {
  ...
  const interfaceRecord = (0, _utilsBMRcljdi.l)(raw.interface) ? raw.interface : void 0;
  const name = (0, _stringCoerceC1IzJjqi.c)(raw.name);
  const description = (0, _stringCoerceC1IzJjqi.c)(raw.description) ?? (0, _stringCoerceC1IzJjqi.c)(raw.shortDescription) ?? (0, _stringCoerceC1IzJjqi.c)(interfaceRecord?.shortDescription);
...
function detectBundleManifestFormat(rootDir) {
  ...
  if (_manifestUasFm2SQ.t.some((candidate) => _nodeFs.default.existsSync(_nodePath.default.join(rootDir, candidate)))) return null;
  if ([
...
function isPathInside(baseDir, targetPath) {
  return (0, _boundaryPathC2iMx2xR.s)(baseDir, targetPath);
}
