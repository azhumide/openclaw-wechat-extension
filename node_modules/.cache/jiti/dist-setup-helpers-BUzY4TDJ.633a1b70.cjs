"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = createSetupInputPresenceValidator;exports.c = moveSingleAccountChannelSectionToDefaultAccount;exports.i = createPatchedAccountSetupAdapter;exports.l = patchScopedAccountConfig;exports.n = applySetupAccountConfigPatch;exports.o = createZodSetupInputValidator;exports.r = createEnvPatchedAccountSetupAdapter;exports.s = migrateBaseNameToDefaultAccount;exports.t = applyAccountNameToChannelSection;exports.u = prepareScopedSetupConfig;var _accountIdCKs_R3Vn = require("./account-id-CKs_R3Vn.js");
...
function applyAccountNameToChannelSection(params) {
  ...
  const accountId = (0, _accountIdCKs_R3Vn.n)(params.accountId);
...
function migrateBaseNameToDefaultAccount(params) {
  ...
  if (!defaultAccount.name) accounts[_accountIdCKs_R3Vn.t] = {
    ...defaultAccount,
    name: baseName
  };
...
function prepareScopedSetupConfig(params) {
  ...
  if (!params.migrateBaseName || (0, _accountIdCKs_R3Vn.n)(params.accountId) === "default") return namedConfig;
...
function createPatchedAccountSetupAdapter(params) {
  return {
    resolveAccountId: ({ accountId }) => (0, _accountIdCKs_R3Vn.n)(accountId),
...
const GenericSetupInputSchema = _zod.z.object({ useEnv: _zod.z.boolean().optional() }).passthrough();
...
function resolveExistingAccountKey(accounts, targetAccountId) {
  for (const existingKey of Object.keys(accounts)) if ((0, _accountIdCKs_R3Vn.n)(existingKey) === targetAccountId) return existingKey;
...
function resolveSingleAccountPromotionTarget(params) {
  ...
  const normalizedDefaultAccount = typeof params.channel.defaultAccount === "string" && params.channel.defaultAccount.trim() ? (0, _accountIdCKs_R3Vn.n)(params.channel.defaultAccount) : void 0;
  if (normalizedDefaultAccount) return Object.keys(accounts).find((accountId) => (0, _accountIdCKs_R3Vn.n)(accountId) === normalizedDefaultAccount) ?? "default";
  const namedAccounts = Object.keys(accounts).filter(Boolean);
  return namedAccounts.length === 1 ? namedAccounts[0] : _accountIdCKs_R3Vn.t;
}
function moveSingleAccountChannelSectionToDefaultAccount(params) {
  ...
  return moveSingleAccountKeysIntoAccount({
    ...
    accounts,
    keysToMove,
    targetAccountId: _accountIdCKs_R3Vn.t
  });
}
//#endregion /* v9-81afa171da2214d1 */
