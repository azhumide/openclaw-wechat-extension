...
function normalizeSlotValue(value) {
  ...
  if (!trimmed) return;
  if ((0, _stringCoerceBje8XVt.s)(trimmed) === "none") return null;
  return trimmed;
}
function normalizePluginEntries(entries, normalizePluginId) {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    const subagent = subagentRaw && typeof subagentRaw === "object" && !Array.isArray(subagentRaw) ? {
      ...
      allowedModels: Array.isArray(subagentRaw.allowedModels) ? subagentRaw.allowedModels.map((model) => (0, _stringCoerceBje8XVt.c)(model)).filter((model) => Boolean(model)) : void 0
    } : void 0;
...
function normalizePluginsConfigWithResolver(config, normalizePluginId = identityNormalizePluginId) {
  ...
  return {
    ...
    slots: {
      memory: memorySlot === void 0 ? (0, _slotsLigfY10h.n)("memory") : memorySlot,
...
function isBundledChannelEnabledByChannelConfig(cfg, pluginId) {
  if (!cfg) return false;
  const channelId = (0, _idsBNV8Hqfu.r)(pluginId);
  if (!channelId) return false;
  const entry = cfg.channels?.[channelId];
  if (!entry || typeof entry !== "object" || Array.isArray(entry)) return false;
  return entry.enabled === true;
}
//#endregion /* v9-6ac7e9306e08a328 */
