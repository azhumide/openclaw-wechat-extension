...
function resolveDefaultLogDir() {
  return canUseNodeFs() ? (0, _tmpOpenclawDirCLT34C.n)() : _tmpOpenclawDirCLT34C.t;
}
function resolveDefaultLogFile(defaultLogDir) {
  return canUseNodeFs() ? _nodePath.default.join(defaultLogDir, "openclaw.log") : `${_tmpOpenclawDirCLT34C.t}/openclaw.log`;
}
...
function sanitizeDiagnosticLogText(value, maxChars) {
  return clampDiagnosticLogText((0, _redactVz8FmIJK.r)(clampDiagnosticLogText(value, maxChars)), maxChars);
}
...
function assignDiagnosticLogAttribute(attributes, state, key, value) {
  ...
  if ((0, _prototypeKeysXqIkmvix.t)(normalizedKey)) return;
  if ((0, _redactVz8FmIJK.r)(normalizedKey) !== normalizedKey) return;
...
function normalizeTraceContext(value) {
  ...
  if (!(0, _diagnosticEventsBrUJFNQ._)(candidate.traceId)) return;
  if (candidate.spanId !== void 0 && !(0, _diagnosticEventsBrUJFNQ.h)(candidate.spanId)) return;
  if (candidate.parentSpanId !== void 0 && !(0, _diagnosticEventsBrUJFNQ.h)(candidate.parentSpanId)) return;
  if (candidate.traceFlags !== void 0 && !(0, _diagnosticEventsBrUJFNQ.g)(candidate.traceFlags)) return;
  return {
...
function buildTraceFileLogFields(logObj) {
  ...
  const trace = findLogTraceContext(bindings, args) ?? (0, _diagnosticEventsBrUJFNQ.m)();
  if (!trace) return;
  return {
...
function buildDiagnosticLogRecord(logObj) {
  ...
  const trace = findLogTraceContext(bindings, numericArgs) ?? (0, _diagnosticEventsBrUJFNQ.m)();
...
function attachDiagnosticEventTransport(logger) {
  logger.attachTransport((logObj) => {
    try {
      (0, _diagnosticEventsBrUJFNQ.n)(buildDiagnosticLogRecord(logObj));
    } catch {}
  });
}
...
function resolveSettings() {
  ...
  const cfg = loggingState.overrideSettings ?? (0, _redactVz8FmIJK.s)();
...
function buildLogger(settings) {
  ...
  logger.attachTransport((logObj) => {
    try {
      ...
      const payload = `${(0, _redactVz8FmIJK.r)(JSON.stringify({
        ...logObj,
        time,
        ...structuredFields,
        ...traceFields
...
const __test__ = exports.r = { shouldSkipMutatingLoggingConfigRead: _redactVz8FmIJK.c };
