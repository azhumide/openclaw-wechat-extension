...
var _tmpOpenclawDirWEYPFjsW = require("./tmp-openclaw-dir-WEYPFjsW.js");
var _boundaryPath6aPPz8oV = require("./boundary-path-6aPPz8oV.js");
var _archivePathCUvFJL = require("./archive-path-CUvFJ-L4.js");
var _pathAliasGuardsDaWdU8nv = require("./path-alias-guards-DaWdU8nv.js");
var _localFileAccessRBYenfpu = require("./local-file-access-rBYenfpu.js");
var _mediaSourceUrlHV4xofh = require("./media-source-url-HV4xofh3.js");
...
function hostPathLooksAbsolute(expanded) {
  return _nodePath.default.isAbsolute(expanded) || (0, _archivePathCUvFJL.t)(expanded);
}
function resolveToCwd(filePath, cwd) {
  const expanded = expandPath(filePath);
  if ((0, _archivePathCUvFJL.t)(expanded)) return _nodePath.default.win32.normalize(expanded);
...
function resolveSandboxPath(params) {
  ...
  if (relative.startsWith("..") || _nodePath.default.isAbsolute(relative) || (0, _archivePathCUvFJL.t)(relative)) throw new Error(`Path escapes sandbox root (${shortPath(rootResolved)}): ${params.filePath}`);
  return {
    resolved,
    relative
  };
...
async function assertSandboxPath(params) {
  ...
  await (0, _pathAliasGuardsDaWdU8nv.n)({
...
async function resolveSandboxedMediaSource(params) {
  ...
  if (!raw) return raw;
  if ((0, _mediaSourceUrlHV4xofh.t)(raw)) return raw;
  let candidate = raw;
  if (/^file:\/\//i.test(candidate)) {
    ...
    try {
      candidate = (0, _localFileAccessRBYenfpu.a)(candidate);
    } catch (err) {
  ...
  (0, _localFileAccessRBYenfpu.t)(candidate, "Sandbox media path");
...
function mapContainerWorkspaceFileUrl(params) {
  ...
  if ((0, _localFileAccessRBYenfpu.r)(parsed.pathname)) return;
...
async function resolveAllowedTmpMediaPath(params) {
  ...
  const openClawTmpDir = _nodePath.default.resolve((0, _tmpOpenclawDirWEYPFjsW.n)());
  if (!(0, _boundaryPath6aPPz8oV.s)(openClawTmpDir, resolved)) return;
