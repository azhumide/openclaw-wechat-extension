...
var _workspaceDefaultClDnTm = require("./workspace-default-ClDnTm-0.js");
var _modelInputBac2nsYA = require("./model-input-Bac2nsYA.js");
var _agentFilterMsVN_KR = require("./agent-filter-msVN_-kR.js");
...
function warnMultipleDefaultAgents() {
  Promise.resolve().then(() => jitiImport("./subsystem-BPbPvZzl.js").then((m) => _interopRequireWildcard(m))).then(({ createSubsystemLogger }) => {
...
function listAgentIds(cfg) {
  ...
  if (agents.length === 0) return [_sessionKeyHxP9B3Or.t];
  const seen = /* @__PURE__ */new Set();
  const ids = [];
  for (const entry of agents) {
    const id = (0, _sessionKeyHxP9B3Or.c)(entry?.id);
    if (seen.has(id)) continue;
    seen.add(id);
    ids.push(id);
  }
  return ids.length > 0 ? ids : [_sessionKeyHxP9B3Or.t];
}
function resolveDefaultAgentId(cfg) {
  ...
  return (0, _sessionKeyHxP9B3Or.c)(chosen || "main");
}
function resolveAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyHxP9B3Or.c)(agentId);
  return listAgentEntries(cfg).find((entry) => (0, _sessionKeyHxP9B3Or.c)(entry.id) === id);
}
function resolveAgentConfig(cfg, agentId) {
  const entry = resolveAgentEntry(cfg, (0, _sessionKeyHxP9B3Or.c)(agentId));
  if (!entry) return;
...
function resolveAgentWorkspaceDir(cfg, agentId, env = process.env) {
  ...
  if (id === defaultAgentId) {
    ...
    return stripNullBytes$1((0, _workspaceDefaultClDnTm.n)(env));
  }
...
function resolveSessionAgentIds(params) {
  ...
  const explicitAgentId = explicitAgentIdRaw ? (0, _sessionKeyHxP9B3Or.c)(explicitAgentIdRaw) : null;
  const sessionKey = params.sessionKey?.trim();
  const normalizedSessionKey = sessionKey ? (0, _stringCoerceBje8XVt.a)(sessionKey) : void 0;
  const parsed = normalizedSessionKey ? (0, _sessionKeyUtilsNaHBWFyS.o)(normalizedSessionKey) : null;
  return {
    defaultAgentId,
    sessionAgentId: explicitAgentId ?? (parsed?.agentId ? (0, _sessionKeyHxP9B3Or.c)(parsed.agentId) : defaultAgentId)
  };
}
...
function resolveAgentSkillsFilter(cfg, agentId) {
  return (0, _agentFilterMsVN_KR.t)(cfg, agentId);
}
...
function findMutableAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyHxP9B3Or.c)(agentId);
  return cfg.agents?.list?.find((entry) => (0, _sessionKeyHxP9B3Or.c)(entry?.id) === id);
}
...
function resolveFallbackAgentId(params) {
  ...
  if (explicitAgentId) return (0, _sessionKeyHxP9B3Or.c)(explicitAgentId);
  return (0, _sessionKeyHxP9B3Or.u)(params.sessionKey);
}
...
function hasConfiguredModelFallbacks(params) {
  ...
  const defaultFallbacks = (0, _modelInputBac2nsYA.t)(params.cfg?.agents?.defaults?.model);
