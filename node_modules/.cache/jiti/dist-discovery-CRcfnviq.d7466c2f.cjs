"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = buildBundledPluginLoadPathAliases;exports.i = resolvePluginSourceRoots;exports.n = discoverOpenClawPlugins;exports.o = normalizeBundledLookupPath;exports.r = resolvePluginCacheInputs;exports.t = clearPluginDiscoveryCache;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _utilsDvkbxKCZ = require("./utils-DvkbxKCZ.js");
var _bundledDirDygrJuRr = require("./bundled-dir-DygrJuRr.js");
var _boundaryFileReadC8TEdk8a = require("./boundary-file-read-C8TEdk8a.js");
var _manifestDkU_xlZi = require("./manifest-DkU_xlZi.js");
var _pathSafetyCrnL8Kkz = require("./path-safety-CrnL8Kkz.js");
var _packageEntryResolutionBgEPolxj = require("./package-entry-resolution-BgEPolxj.js");
...
function isSameOrInside(baseDir, targetPath) {
  ...
  return target === base || (0, _pathSafetyCrnL8Kkz.n)(base, target);
}
...
function resolvePluginSourceRoots(params) {
  ...
  const workspaceRoot = params.workspaceDir ? (0, _utilsDvkbxKCZ.p)(params.workspaceDir, env) : void 0;
  return {
    stock: (0, _bundledDirDygrJuRr.t)(env),
    global: _nodePath.default.join((0, _utilsDvkbxKCZ.d)(env), "extensions"),
...
function resolvePluginCacheInputs(params) {
  ...
  return {
    ...
    loadPaths: (params.loadPaths ?? []).filter((entry) => typeof entry === "string").map((entry) => entry.trim()).filter(Boolean).map((entry) => (0, _utilsDvkbxKCZ.p)(entry, env))
  };
}
//#endregion
...
function checkSourceEscapesRoot(params) {
  const sourceRealPath = (0, _pathSafetyCrnL8Kkz.r)(params.source, params.realpathCache);
  const rootRealPath = (0, _pathSafetyCrnL8Kkz.r)(params.rootDir, params.realpathCache);
  if (!sourceRealPath || !rootRealPath) return null;
  if ((0, _pathSafetyCrnL8Kkz.n)(rootRealPath, sourceRealPath)) return null;
  return {
...
function checkPathStatAndPermissions(params) {
  ...
  for (const targetPath of pathsToCheck) {
    ...
    seen.add(normalized);
    let stat = (0, _pathSafetyCrnL8Kkz.i)(targetPath);
    if (!stat) return {
    ...
    if ((modeBits & 2) !== 0 && params.origin === "bundled") try {
      ...
      const repairedStat = (0, _pathSafetyCrnL8Kkz.i)(targetPath);
...
function formatCandidateBlockMessage(issue) {
  ...
  if (issue.reason === "path_world_writable") return `blocked plugin candidate: world-writable path (${issue.targetPath}, mode=${(0, _pathSafetyCrnL8Kkz.t)(issue.modeBits ?? 0)})`;
...
function resolveScannedEntryType(entry, fullPath) {
  ...
  const stat = (0, _pathSafetyCrnL8Kkz.i)(fullPath);
  if (!stat) return null;
...
function resolvesToSameDirectory(left, right, realpathCache) {
  ...
  const leftRealPath = (0, _pathSafetyCrnL8Kkz.r)(left, realpathCache);
  const rightRealPath = (0, _pathSafetyCrnL8Kkz.r)(right, realpathCache);
...
function readPackageManifest(dir, rejectHardlinks = true, rootRealPath) {
  const opened = (0, _boundaryFileReadC8TEdk8a.i)({
...
function resolveIdHintManifestId(rootDir, rejectHardlinks, rootRealPath) {
  const manifest = (0, _manifestDkU_xlZi.i)(rootDir, rejectHardlinks, rootRealPath);
...
function addCandidate(params) {
  ...
  const resolvedRoot = (0, _pathSafetyCrnL8Kkz.r)(params.rootDir, params.realpathCache) ?? _nodePath.default.resolve(params.rootDir);
  ...
  params.candidates.push({
    ...
    packageManifest: (0, _manifestDkU_xlZi.r)(manifest ?? void 0),
    bundledManifest: params.bundledManifest,
    bundledManifestPath: params.bundledManifestPath
  });
}
function discoverBundleInRoot(params) {
  const bundleFormat = (0, _pathSafetyCrnL8Kkz.c)(params.rootDir);
  if (!bundleFormat) return "none";
  const rootRealPath = (0, _pathSafetyCrnL8Kkz.r)(params.rootDir, params.realpathCache) ?? void 0;
  const bundleManifest = (0, _pathSafetyCrnL8Kkz.l)({
    rootDir: params.rootDir,
    ...(rootRealPath !== void 0 ? { rootRealPath } : {}),
    bundleFormat,
...
function discoverInDirectory(params) {
  ...
  const resolvedDir = (0, _pathSafetyCrnL8Kkz.r)(params.dir, params.realpathCache) ?? _nodePath.default.resolve(params.dir);
  ...
  for (const entry of entries) {
    ...
    const fullPathRealPath = (0, _pathSafetyCrnL8Kkz.r)(fullPath, params.realpathCache) ?? void 0;
    const manifest = readPackageManifest(fullPath, rejectHardlinks, fullPathRealPath);
    const extensionResolution = (0, _manifestDkU_xlZi.a)(manifest ?? void 0);
    const extensions = extensionResolution.status === "ok" ? extensionResolution.entries : [];
    const manifestId = resolveIdHintManifestId(fullPath, rejectHardlinks, fullPathRealPath);
    const setupSource = (0, _packageEntryResolutionBgEPolxj.n)({
      packageDir: fullPath,
    ...
    if (extensions.length > 0) {
      const resolvedRuntimeSources = (0, _packageEntryResolutionBgEPolxj.t)({
        packageDir: fullPath,
        ...(fullPathRealPath !== void 0 ? { packageRootRealPath: fullPathRealPath } : {}),
        manifest,
        extensions,
    ...
    const indexFile = [..._manifestDkU_xlZi.t].map((candidate) => _nodePath.default.join(fullPath, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
...
function discoverFromPath(params) {
  const resolved = (0, _utilsDvkbxKCZ.p)(params.rawPath, params.env);
  ...
  if (stat.isDirectory()) {
    const rejectHardlinks = params.origin !== "bundled";
    const resolvedRealPath = (0, _pathSafetyCrnL8Kkz.r)(resolved, params.realpathCache) ?? void 0;
    ...
    const indexFile = [..._manifestDkU_xlZi.t].map((candidate) => _nodePath.default.join(resolved, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
    if (indexFile && isExtensionFile(indexFile)) {
      addCandidate({
        candidates: params.candidates,
        diagnostics: params.diagnostics,
...
function discoverOpenClawPlugins(params) {
  ...
  const workspaceRoot = workspaceDir ? (0, _utilsDvkbxKCZ.p)(workspaceDir, env) : void 0;
    ...
    load: () => {
      ...
      for (const extraPath of extra) {
        ...
        const bundledAlias = resolvePackagedBundledLoadPathAlias({
          ...
          loadPath: (0, _utilsDvkbxKCZ.p)(trimmed, env)
        });
        if (bundledAlias) {
