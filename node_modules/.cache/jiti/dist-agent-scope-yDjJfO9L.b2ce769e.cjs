...
var _modelInputCDnCe1Q = require("./model-input-CDn-ce1Q.js");
var _workspaceDefault_Jg7EPWS = require("./workspace-default-_Jg7EPWS.js");
var _agentFilterC6Aja0hq = require("./agent-filter-C6Aja0hq.js");
...
function warnMultipleDefaultAgents() {
  Promise.resolve().then(() => jitiImport("./subsystem-OEnHHmEI.js").then((m) => _interopRequireWildcard(m))).then(({ createSubsystemLogger }) => {
...
function listAgentIds(cfg) {
  ...
  if (agents.length === 0) return [_sessionKey7OXmevzf.t];
  const seen = /* @__PURE__ */new Set();
  const ids = [];
  for (const entry of agents) {
    const id = (0, _sessionKey7OXmevzf.c)(entry?.id);
    if (seen.has(id)) continue;
    seen.add(id);
    ids.push(id);
  }
  return ids.length > 0 ? ids : [_sessionKey7OXmevzf.t];
}
function resolveDefaultAgentId(cfg) {
  ...
  return (0, _sessionKey7OXmevzf.c)(chosen || "main");
}
function resolveAgentEntry(cfg, agentId) {
  const id = (0, _sessionKey7OXmevzf.c)(agentId);
  return listAgentEntries(cfg).find((entry) => (0, _sessionKey7OXmevzf.c)(entry.id) === id);
}
function resolveAgentConfig(cfg, agentId) {
  const entry = resolveAgentEntry(cfg, (0, _sessionKey7OXmevzf.c)(agentId));
  if (!entry) return;
...
function resolveAgentWorkspaceDir(cfg, agentId, env = process.env) {
  ...
  if (configured) return stripNullBytes$1((0, _utilsBYAfe1GS.p)(configured, env));
  const defaultAgentId = resolveDefaultAgentId(cfg);
  const fallback = cfg.agents?.defaults?.workspace?.trim();
  if (id === defaultAgentId) {
    if (fallback) return stripNullBytes$1((0, _utilsBYAfe1GS.p)(fallback, env));
    return stripNullBytes$1((0, _workspaceDefault_Jg7EPWS.n)(env));
  }
  if (fallback) return stripNullBytes$1(_nodePath.default.join((0, _utilsBYAfe1GS.p)(fallback, env), id));
  const stateDir = (0, _pathsBFfL0I8G._)(env);
...
function resolveAgentDir(cfg, agentId, env = process.env) {
  ...
  if (configured) return (0, _utilsBYAfe1GS.p)(configured, env);
  const root = (0, _pathsBFfL0I8G._)(env);
...
function resolveSessionAgentIds(params) {
  ...
  const explicitAgentId = explicitAgentIdRaw ? (0, _sessionKey7OXmevzf.c)(explicitAgentIdRaw) : null;
  const sessionKey = params.sessionKey?.trim();
  const normalizedSessionKey = sessionKey ? (0, _stringCoerceBje8XVt.a)(sessionKey) : void 0;
  const parsed = normalizedSessionKey ? (0, _sessionKeyUtilsB9qLc2A.o)(normalizedSessionKey) : null;
  return {
    defaultAgentId,
    sessionAgentId: explicitAgentId ?? (parsed?.agentId ? (0, _sessionKey7OXmevzf.c)(parsed.agentId) : defaultAgentId)
  };
}
...
function resolveAgentSkillsFilter(cfg, agentId) {
  return (0, _agentFilterC6Aja0hq.t)(cfg, agentId);
}
...
function resolveFallbackAgentId(params) {
  ...
  if (explicitAgentId) return (0, _sessionKey7OXmevzf.c)(explicitAgentId);
  return (0, _sessionKey7OXmevzf.u)(params.sessionKey);
}
...
function hasConfiguredModelFallbacks(params) {
  ...
  const defaultFallbacks = (0, _modelInputCDnCe1Q.t)(params.cfg?.agents?.defaults?.model);
...
function normalizePathForComparison(input) {
  const resolved = _nodePath.default.resolve(stripNullBytes((0, _utilsBYAfe1GS.p)(input)));
