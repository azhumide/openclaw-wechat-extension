...
function hasInlineCapabilityValue(value) {
  ...
  if ((0, _utilsBYAfe1GS.c)(value)) return Object.keys(value).length > 0;
  return value === true;
}
...
function loadBundleManifest(params) {
  ...
  const interfaceRecord = (0, _utilsBYAfe1GS.c)(raw.interface) ? raw.interface : void 0;
...
function detectBundleManifestFormat(rootDir) {
  ...
  if (_manifestC0Wa8IEc.t.some((candidate) => _nodeFs.default.existsSync(_nodePath.default.join(rootDir, candidate)))) return null;
  if ([
...
function isPathInside(baseDir, targetPath) {
  return (0, _boundaryPathBUtehO_x.s)(baseDir, targetPath);
}
