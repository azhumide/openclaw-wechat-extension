"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = stripRedundantSubsystemPrefixForConsole;exports.n = createSubsystemRuntime;exports.r = runtimeForLogger;exports.t = createSubsystemLogger;var _redactVz8FmIJK = require("./redact-vz8FmIJK.js");
var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _progressLineBLlwPNs = require("./progress-line-BLlwPNs4.js");
var _idsNy_qj_av = require("./ids-ny_qj_av.js");
var _runtimeCChwgwyg = require("./runtime-CChwgwyg.js");
var _loggerR45Bhn_ = require("./logger-r45-Bhn_.js");
var _consoleDHOcTkM = require("./console-DHOcTk-M.js");
...
function shouldLogToConsole(level, settings) {
  ...
  return (0, _loggerR45Bhn_._)(level) >= (0, _loggerR45Bhn_._)(settings.level);
}
const inspectValue = (() => {
...
function isChannelSubsystemPrefix(value) {
  ...
  return (0, _idsNy_qj_av.r)(normalized) === normalized || normalized === "webchat";
}
...
function formatConsoleLine(opts) {
  ...
  if (opts.style === "json") return (0, _redactVz8FmIJK.r)(JSON.stringify({
    time: (0, _consoleDHOcTkM.n)("json"),
    level: opts.level,
  ...
  const displayMessage = stripRedundantSubsystemPrefixForConsole((0, _redactVz8FmIJK.r)(opts.message), displaySubsystem);
  return `${[(() => {
    if (opts.style === "pretty") return color.gray((0, _consoleDHOcTkM.n)("pretty"));
    if (_loggerR45Bhn_.h.consoleTimestampPrefix) return color.gray((0, _consoleDHOcTkM.n)(opts.style));
    return "";
...
function writeConsoleLine(level, line, opts = {}) {
  ...
  const redacted = opts.redacted ? sanitized : (0, _redactVz8FmIJK.r)(sanitized);
  const sink = _loggerR45Bhn_.h.rawConsole ?? console;
  if (_loggerR45Bhn_.h.forceConsoleToStderr || level === "error" || level === "fatal") (sink.error ?? console.error)(redacted);else
...
function shouldSuppressProbeConsoleLine(params) {
  if ((0, _loggerR45Bhn_.b)()) return false;
...
function createSubsystemLogger(subsystem) {
  ...
  const emitLog = (level, message, meta) => {
    const consoleSettings = (0, _consoleDHOcTkM.r)();
    const consoleEnabled = shouldLogToConsole(level, { level: consoleSettings.level }) && (0, _consoleDHOcTkM.l)(resolvedSubsystem);
    const fileEnabled = (0, _loggerR45Bhn_.s)(level);
    ...
    if (fileEnabled) logToFile((0, _loggerR45Bhn_.i)({ subsystem: resolvedSubsystem }), level, message, fileMeta);
  ...
  return {
    ...
    isEnabled(level, target = "any") {
      const isConsoleEnabled = shouldLogToConsole(level, { level: (0, _consoleDHOcTkM.r)().level }) && (0, _consoleDHOcTkM.l)(resolvedSubsystem);
      const isFileEnabled = (0, _loggerR45Bhn_.s)(level);
    ...
    raw(message) {
      if ((0, _loggerR45Bhn_.s)("info")) logToFile((0, _loggerR45Bhn_.i)({ subsystem: resolvedSubsystem }), "info", message, { raw: true });
      if (shouldLogToConsole("info", { level: (0, _consoleDHOcTkM.r)().level }) && (0, _consoleDHOcTkM.l)(resolvedSubsystem)) {
        if (shouldSuppressProbeConsoleLine({
          level: "info",
          subsystem: resolvedSubsystem,
          message
...
function runtimeForLogger(logger, exit = _runtimeCChwgwyg.n.exit) {
  return {
    log(...args) {
...
function createSubsystemRuntime(subsystem, exit = _runtimeCChwgwyg.n.exit) {
  return runtimeForLogger(createSubsystemLogger(subsystem), exit);
}
//#endregion /* v9-ae3d4f37aa2e149f */
