...
var _registryCWPwZ76z = require("./registry-CWPwZ76z.js");
require("./plugins-C2gQv6Dl.js");
var _bindingsCdLgqvD = require("./bindings-CdLgqv-d.js");
var _resolveRoute4JrCKFrU = require("./resolve-route-4JrCKFrU.js");
...
function buildConfiguredAcpSessionKey(spec) {
  ...
  return `agent:${(0, _sessionKeyBd0xquXF.f)(spec.agentId)}:acp:binding:${spec.channel}:${spec.accountId}:${hash}`;
}
...
function parseConfiguredAcpSessionKey(sessionKey) {
  ...
  return {
    channel,
    accountId: (0, _accountIdVYgQddVH.n)(tokens[3] ?? "default")
  };
}
function resolveConfiguredAcpBindingSpecFromRecord(record) {
  ...
  const agentId = (0, _stringCoerceBje8XVt.c)(record.metadata?.agentId) ?? (0, _sessionKeyBd0xquXF.u)(record.targetSessionKey);
  if (!agentId) return null;
  return {
    channel: record.conversation.channel,
    accountId: (0, _accountIdVYgQddVH.n)(record.conversation.accountId),
    conversationId,
...
function resolveConfiguredBindingWorkspaceCwd(params) {
  if ((0, _stringCoerceBje8XVt.c)((0, _agentScopeDf_s1jDI.v)(params.cfg, params.agentId)?.workspace)) return (0, _agentScopeDf_s1jDI.x)(params.cfg, params.agentId);
  if (params.agentId === (0, _agentScopeDf_s1jDI.S)(params.cfg)) {
    if ((0, _stringCoerceBje8XVt.c)(params.cfg.agents?.defaults?.workspace)) return (0, _agentScopeDf_s1jDI.x)(params.cfg, params.agentId);
  }
}
...
function resolveLoadedChannelPlugin(channel) {
  ...
  return (0, _registryCWPwZ76z.t)(normalized);
}
...
function compileConfiguredBindingRule(params) {
  const agentId = (0, _resolveRoute4JrCKFrU.r)(params.cfg, params.binding.agentId ?? "main");
...
function compileConfiguredBindingRegistry(params) {
  ...
  for (const binding of (0, _bindingsCdLgqvD.r)(params.cfg)) {
...
function resolveAccountMatchPriority(match, actual) {
  ...
  if (trimmed === "*") return 1;
  return (0, _accountIdVYgQddVH.n)(trimmed) === actual ? 2 : 0;
}
...
function toConfiguredBindingConversationRef(conversation) {
  ...
  return {
    channel,
    accountId: (0, _accountIdVYgQddVH.n)(conversation.accountId),
    conversationId,
...
function materializeConfiguredBindingRecord(params) {
  return params.rule.targetFactory.materialize({
    accountId: (0, _accountIdVYgQddVH.n)(params.accountId),
