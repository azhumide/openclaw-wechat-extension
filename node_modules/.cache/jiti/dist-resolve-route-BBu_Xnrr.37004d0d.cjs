...
var _chatTypeBsQ6o51R = require("./chat-type-BsQ6o51R.js");
var _bindingScopeQI0ueMbG = require("./binding-scope-QI0ueMbG.js");
var _bindingsBymomZU = require("./bindings-BymomZU5.js");
var _peerKindMatchGdEbQOlA = require("./peer-kind-match-gdEbQOlA.js");
...
function normalizeId(value) {
  return (0, _bindingScopeQI0ueMbG.n)(value);
}
...
function buildEvaluatedBindingsByChannel(cfg) {
  ...
  let order = 0;
  for (const binding of (0, _bindingsBymomZU.n)(cfg)) {
...
function normalizePeerConstraint(peer) {
  ...
  const kind = (0, _chatTypeBsQ6o51R.t)(peer.kind);
...
function normalizeBindingMatch(match) {
  ...
  return {
    ...
    roles: (0, _bindingScopeQI0ueMbG.r)(rawRoles)
  };
}
...
function matchesBindingScope(match, scope) {
  ...
  if (match.peer.state === "valid") {
    if (!scope.peer || !(0, _peerKindMatchGdEbQOlA.t)(match.peer.kind, scope.peer.kind) || scope.peer.id !== match.peer.id) return false;
  }
  if (match.peer.state === "wildcard-kind") {
    if (!scope.peer || !(0, _peerKindMatchGdEbQOlA.t)(match.peer.kind, scope.peer.kind)) return false;
  }
  return (0, _bindingScopeQI0ueMbG.a)(match, scope);
}
function resolveAgentRoute(input) {
  const channel = normalizeToken(input.channel);
  const accountId = (0, _accountIdC3j_3_su.n)(input.accountId);
  const peer = input.peer ? {
    kind: (0, _chatTypeBsQ6o51R.t)(input.peer.kind) ?? input.peer.kind,
  ...
  const parentPeer = input.parentPeer ? {
    kind: (0, _chatTypeBsQ6o51R.t)(input.parentPeer.kind) ?? input.parentPeer.kind,
