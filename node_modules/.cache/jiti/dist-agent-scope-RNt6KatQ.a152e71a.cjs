...
var _workspaceDefaultCrWuRmeo = require("./workspace-default-CrWuRmeo.js");
var _modelInputBa1eTv7s = require("./model-input-Ba1eTv7s.js");
var _agentFilterWwHqftPQ = require("./agent-filter-wwHqftPQ.js");
...
function warnMultipleDefaultAgents() {
  Promise.resolve().then(() => jitiImport("./subsystem-Bn9SOdPE.js").then((m) => _interopRequireWildcard(m))).then(({ createSubsystemLogger }) => {
...
function listAgentIds(cfg) {
  ...
  if (agents.length === 0) return [_sessionKeyC6FNnIG.t];
  const seen = /* @__PURE__ */new Set();
  const ids = [];
  for (const entry of agents) {
    const id = (0, _sessionKeyC6FNnIG.c)(entry?.id);
    if (seen.has(id)) continue;
    seen.add(id);
    ids.push(id);
  }
  return ids.length > 0 ? ids : [_sessionKeyC6FNnIG.t];
}
function resolveDefaultAgentId(cfg) {
  ...
  return (0, _sessionKeyC6FNnIG.c)(chosen || "main");
}
function resolveAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyC6FNnIG.c)(agentId);
  return listAgentEntries(cfg).find((entry) => (0, _sessionKeyC6FNnIG.c)(entry.id) === id);
}
function resolveAgentConfig(cfg, agentId) {
  const entry = resolveAgentEntry(cfg, (0, _sessionKeyC6FNnIG.c)(agentId));
  if (!entry) return;
...
function resolveAgentWorkspaceDir(cfg, agentId, env = process.env) {
  ...
  if (id === defaultAgentId) {
    ...
    return stripNullBytes$1((0, _workspaceDefaultCrWuRmeo.n)(env));
  }
...
function resolveSessionAgentIds(params) {
  ...
  const explicitAgentId = explicitAgentIdRaw ? (0, _sessionKeyC6FNnIG.c)(explicitAgentIdRaw) : null;
  ...
  return {
    defaultAgentId,
    sessionAgentId: explicitAgentId ?? (parsed?.agentId ? (0, _sessionKeyC6FNnIG.c)(parsed.agentId) : defaultAgentId)
  };
}
...
function resolveAgentSkillsFilter(cfg, agentId) {
  return (0, _agentFilterWwHqftPQ.t)(cfg, agentId);
}
...
function findMutableAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyC6FNnIG.c)(agentId);
  return cfg.agents?.list?.find((entry) => (0, _sessionKeyC6FNnIG.c)(entry?.id) === id);
}
...
function resolveFallbackAgentId(params) {
  ...
  if (explicitAgentId) return (0, _sessionKeyC6FNnIG.c)(explicitAgentId);
  return (0, _sessionKeyC6FNnIG.u)(params.sessionKey);
}
...
function hasConfiguredModelFallbacks(params) {
  ...
  const defaultFallbacks = (0, _modelInputBa1eTv7s.t)(params.cfg?.agents?.defaults?.model);
