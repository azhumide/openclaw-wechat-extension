...
async function sniffMime(buffer) {
  if (!buffer) return;
  try {
    fileTypeModulePromise ??= Promise.resolve().then(() => jitiImport("file-type").then((m) => _interopRequireWildcard(m)));
    const { fileTypeFromBuffer } = await fileTypeModulePromise;
    return (await fileTypeFromBuffer(sliceMimeSniffBuffer(buffer)))?.mime ?? void 0;
  } catch {
    return;
  }
}
