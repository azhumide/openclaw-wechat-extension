...
var _channelCatalogRegistryC9YLpU = require("./channel-catalog-registry-C9-YLpU9.js");
...
function listPackageRoots() {
  return [(0, _openclawRootCRSCIPqz.n)({ cwd: process.cwd() }), (0, _openclawRootCRSCIPqz.n)({ moduleUrl: "file:///home/rs/openclaw/dist/ids-DUzZX4p8.js" })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index);
}
function readBundledExtensionCatalogEntriesSync() {
  try {
    return (0, _channelCatalogRegistryC9YLpU.t)({ origin: "bundled" }).map((entry) => entry.channel);
  } catch {
    return [];
  }
}
