...
var _registryDXGKA3mU = require("./registry-dXGKA3mU.js");
var _runtimeDH5Ph4Z = require("./runtime-DH5Ph4Z1.js");
require("./plugins-Bc9yusIY.js");
var _bindingsCR3ZsOC = require("./bindings-CR3ZsOC9.js");
var _resolveRouteBBcZvIS = require("./resolve-route-B-BcZvIS.js");
...
function buildConfiguredAcpSessionKey(spec) {
  ...
  return `agent:${(0, _sessionKeyC6FNnIG.f)(spec.agentId)}:acp:binding:${spec.channel}:${spec.accountId}:${hash}`;
}
...
function parseConfiguredAcpSessionKey(sessionKey) {
  ...
  return {
    channel,
    accountId: (0, _accountIdCRE2SEcy.n)(tokens[3] ?? "default")
  };
}
function resolveConfiguredAcpBindingSpecFromRecord(record) {
  ...
  const agentId = (0, _stringCoerceBje8XVt.c)(record.metadata?.agentId) ?? (0, _sessionKeyC6FNnIG.u)(record.targetSessionKey);
  if (!agentId) return null;
  return {
    channel: record.conversation.channel,
    accountId: (0, _accountIdCRE2SEcy.n)(record.conversation.accountId),
    conversationId,
...
function resolveConfiguredBindingWorkspaceCwd(params) {
  if ((0, _stringCoerceBje8XVt.c)((0, _agentScopeRNt6KatQ.v)(params.cfg, params.agentId)?.workspace)) return (0, _agentScopeRNt6KatQ.x)(params.cfg, params.agentId);
  if (params.agentId === (0, _agentScopeRNt6KatQ.S)(params.cfg)) {
    if ((0, _stringCoerceBje8XVt.c)(params.cfg.agents?.defaults?.workspace)) return (0, _agentScopeRNt6KatQ.x)(params.cfg, params.agentId);
  }
}
...
function resolveLoadedChannelPlugin(channel) {
  ...
  return (0, _registryDXGKA3mU.t)(normalized);
}
...
function compileConfiguredBindingRule(params) {
  const agentId = (0, _resolveRouteBBcZvIS.r)(params.cfg, params.binding.agentId ?? "main");
...
function compileConfiguredBindingRegistry(params) {
  ...
  for (const binding of (0, _bindingsCR3ZsOC.r)(params.cfg)) {
...
function resolveCompiledBindingRegistry(cfg) {
  const activeRegistry = (0, _runtimeDH5Ph4Z.g)();
  const registryVersion = (0, _runtimeDH5Ph4Z.n)();
...
function primeCompiledBindingRegistry(cfg) {
  ...
  compiledRegistryCache.set(cfg, {
    registryRef: activeRegistry,
    registryVersion: (0, _runtimeDH5Ph4Z.n)(),
    registry
  });
  return registry;
}
...
function resolveAccountMatchPriority(match, actual) {
  ...
  if (trimmed === "*") return 1;
  return (0, _accountIdCRE2SEcy.n)(trimmed) === actual ? 2 : 0;
}
...
function toConfiguredBindingConversationRef(conversation) {
  ...
  return {
    channel,
    accountId: (0, _accountIdCRE2SEcy.n)(conversation.accountId),
    conversationId,
...
function materializeConfiguredBindingRecord(params) {
  return params.rule.targetFactory.materialize({
    accountId: (0, _accountIdCRE2SEcy.n)(params.accountId),
