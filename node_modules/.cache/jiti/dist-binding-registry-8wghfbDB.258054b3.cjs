"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveConfiguredBindingRecordForConversation;exports.c = resolveConfiguredAcpBindingSpecFromRecord;exports.i = resolveConfiguredBindingRecordBySessionKey;exports.l = toResolvedConfiguredAcpBinding;exports.n = resolveConfiguredBinding;exports.o = buildConfiguredAcpSessionKey;exports.r = resolveConfiguredBindingRecord;exports.s = normalizeBindingConfig;exports.t = primeConfiguredBindingRegistry;var _stringCoerceC1IzJjqi = require("./string-coerce-C1IzJjqi.js");
var _accountIdC3j_3_su = require("./account-id-C3j_3_su.js");
var _sessionKeyEpIbK3Oz = require("./session-key-EpIbK3Oz.js");
var _registryCi9ATKEN = require("./registry-Ci9ATKEN.js");
var _agentScope_6dFncNS = require("./agent-scope-_6dFncNS.js");
require("./plugins-CjU2NdbZ.js");
var _runtimeBFywV6BM = require("./runtime-BFywV6BM.js");
var _bindingsCPyZiwzQ = require("./bindings-CPyZiwzQ.js");
var _resolveRouteDWtJo8cH = require("./resolve-route-DWtJo8cH.js");
var _nodeCrypto = require("node:crypto");
//#region src/acp/persistent-bindings.types.ts
function normalizeMode(value) {
  return (0, _stringCoerceC1IzJjqi.s)(value) === "oneshot" ? "oneshot" : "persistent";
}
function normalizeBindingConfig(raw) {
  if (!raw || typeof raw !== "object") return {};
  const shape = raw;
  const mode = (0, _stringCoerceC1IzJjqi.c)(shape.mode);
  return {
    mode: mode ? normalizeMode(mode) : void 0,
    cwd: (0, _stringCoerceC1IzJjqi.c)(shape.cwd),
    backend: (0, _stringCoerceC1IzJjqi.c)(shape.backend),
    label: (0, _stringCoerceC1IzJjqi.c)(shape.label)
  };
}
...
function buildConfiguredAcpSessionKey(spec) {
  ...
  return `agent:${(0, _sessionKeyEpIbK3Oz.f)(spec.agentId)}:acp:binding:${spec.channel}:${spec.accountId}:${hash}`;
}
...
function parseConfiguredAcpSessionKey(sessionKey) {
  ...
  const channel = (0, _stringCoerceC1IzJjqi.s)(tokens[2]);
  if (!channel) return null;
  return {
    channel,
    accountId: (0, _accountIdC3j_3_su.n)(tokens[3] ?? "default")
  };
}
function resolveConfiguredAcpBindingSpecFromRecord(record) {
  ...
  const agentId = (0, _stringCoerceC1IzJjqi.c)(record.metadata?.agentId) ?? (0, _sessionKeyEpIbK3Oz.u)(record.targetSessionKey);
  if (!agentId) return null;
  return {
    channel: record.conversation.channel,
    accountId: (0, _accountIdC3j_3_su.n)(record.conversation.accountId),
    conversationId,
    parentConversationId: (0, _stringCoerceC1IzJjqi.c)(record.conversation.parentConversationId),
    agentId,
    acpAgentId: (0, _stringCoerceC1IzJjqi.c)(record.metadata?.acpAgentId),
    mode: normalizeMode(record.metadata?.mode),
    cwd: (0, _stringCoerceC1IzJjqi.c)(record.metadata?.cwd),
    backend: (0, _stringCoerceC1IzJjqi.c)(record.metadata?.backend),
    label: (0, _stringCoerceC1IzJjqi.c)(record.metadata?.label)
  };
}
...
function resolveAgentRuntimeAcpDefaults(params) {
  const ownerAgentId = (0, _stringCoerceC1IzJjqi.a)(params.ownerAgentId);
  const agent = params.cfg.agents?.list?.find((entry) => (0, _stringCoerceC1IzJjqi.s)(entry.id) === ownerAgentId);
  if (!agent || agent.runtime?.type !== "acp") return {};
  return {
    acpAgentId: (0, _stringCoerceC1IzJjqi.c)(agent.runtime.acp?.agent),
    mode: (0, _stringCoerceC1IzJjqi.c)(agent.runtime.acp?.mode),
    cwd: (0, _stringCoerceC1IzJjqi.c)(agent.runtime.acp?.cwd),
    backend: (0, _stringCoerceC1IzJjqi.c)(agent.runtime.acp?.backend)
  };
}
function resolveConfiguredBindingWorkspaceCwd(params) {
  if ((0, _stringCoerceC1IzJjqi.c)((0, _agentScope_6dFncNS._)(params.cfg, params.agentId)?.workspace)) return (0, _agentScope_6dFncNS.b)(params.cfg, params.agentId);
  if (params.agentId === (0, _agentScope_6dFncNS.x)(params.cfg)) {
    if ((0, _stringCoerceC1IzJjqi.c)(params.cfg.agents?.defaults?.workspace)) return (0, _agentScope_6dFncNS.b)(params.cfg, params.agentId);
  }
}
...
function buildAcpTargetFactory(params) {
  ...
  const acpAgentId = (0, _stringCoerceC1IzJjqi.c)(runtimeDefaults.acpAgentId);
  return {
    driverId: "acp",
...
function resolveLoadedChannelPlugin(channel) {
  const normalized = (0, _stringCoerceC1IzJjqi.s)(channel);
  if (!normalized) return;
  return (0, _registryCi9ATKEN.t)(normalized);
}
...
function resolveBindingConversationId(binding) {
  return (0, _stringCoerceC1IzJjqi.c)(binding.match?.peer?.id) ?? null;
}
...
function compileConfiguredBindingRule(params) {
  const agentId = (0, _resolveRouteDWtJo8cH.r)(params.cfg, params.binding.agentId ?? "main");
  ...
  return {
    channel: params.channel,
    accountPattern: (0, _stringCoerceC1IzJjqi.c)(params.binding.match.accountId),
    binding: params.binding,
    bindingConversationId: params.bindingConversationId,
    target: params.target,
    agentId,
...
function compileConfiguredBindingRegistry(params) {
  ...
  for (const binding of (0, _bindingsCPyZiwzQ.r)(params.cfg)) {
...
function resolveCompiledBindingRegistry(cfg) {
  const activeRegistry = (0, _runtimeBFywV6BM.g)();
  const registryVersion = (0, _runtimeBFywV6BM.n)();
...
function primeCompiledBindingRegistry(cfg) {
  ...
  compiledRegistryCache.set(cfg, {
    registryRef: activeRegistry,
    registryVersion: (0, _runtimeBFywV6BM.n)(),
    registry
  });
  return registry;
}
...
function resolveAccountMatchPriority(match, actual) {
  ...
  if (trimmed === "*") return 1;
  return (0, _accountIdC3j_3_su.n)(trimmed) === actual ? 2 : 0;
}
...
function resolveCompiledBindingChannel(raw) {
  const normalized = (0, _stringCoerceC1IzJjqi.s)(raw);
...
function toConfiguredBindingConversationRef(conversation) {
  ...
  return {
    channel,
    accountId: (0, _accountIdC3j_3_su.n)(conversation.accountId),
    conversationId,
    parentConversationId: (0, _stringCoerceC1IzJjqi.c)(conversation.parentConversationId)
  };
}
function materializeConfiguredBindingRecord(params) {
  return params.rule.targetFactory.materialize({
    accountId: (0, _accountIdC3j_3_su.n)(params.accountId),
