...
var _manifestBRvO4Lcw = require("./manifest-BRvO4Lcw.js");
var _bundleManifestBwHr1gNY = require("./bundle-manifest-BwHr1gNY.js");
var _packageEntryResolutionBysVvIJY = require("./package-entry-resolution-BysVvIJY.js");
...
function isSameOrInside(baseDir, targetPath) {
  ...
  return target === base || (0, _bundleManifestBwHr1gNY.l)(base, target);
}
...
function resolvePluginSourceRoots(params) {
  ...
  return {
    stock: (0, _bundledDirBdVWKJP.n)(env),
...
function checkSourceEscapesRoot(params) {
  const sourceRealPath = (0, _bundleManifestBwHr1gNY.u)(params.source, params.realpathCache);
  const rootRealPath = (0, _bundleManifestBwHr1gNY.u)(params.rootDir, params.realpathCache);
  if (!sourceRealPath || !rootRealPath) return null;
  if ((0, _bundleManifestBwHr1gNY.l)(rootRealPath, sourceRealPath)) return null;
  return {
...
function checkPathStatAndPermissions(params) {
  ...
  for (const targetPath of pathsToCheck) {
    ...
    seen.add(normalized);
    let stat = (0, _bundleManifestBwHr1gNY.d)(targetPath);
    if (!stat) return {
    ...
    if ((modeBits & 2) !== 0 && params.origin === "bundled") try {
      ...
      const repairedStat = (0, _bundleManifestBwHr1gNY.d)(targetPath);
...
function formatCandidateBlockMessage(issue) {
  ...
  if (issue.reason === "path_world_writable") return `blocked plugin candidate: world-writable path (${issue.targetPath}, mode=${(0, _bundleManifestBwHr1gNY.c)(issue.modeBits ?? 0)})`;
...
function resolveScannedEntryType(entry, fullPath) {
  ...
  const stat = (0, _bundleManifestBwHr1gNY.d)(fullPath);
  if (!stat) return null;
...
function resolvesToSameDirectory(left, right, realpathCache) {
  ...
  const leftRealPath = (0, _bundleManifestBwHr1gNY.u)(left, realpathCache);
  const rightRealPath = (0, _bundleManifestBwHr1gNY.u)(right, realpathCache);
...
function resolveIdHintManifestId(rootDir, rejectHardlinks, rootRealPath) {
  const manifest = (0, _manifestBRvO4Lcw.i)(rootDir, rejectHardlinks, rootRealPath);
...
function addCandidate(params) {
  ...
  const resolvedRoot = (0, _bundleManifestBwHr1gNY.u)(params.rootDir, params.realpathCache) ?? _nodePath.default.resolve(params.rootDir);
  ...
  params.candidates.push({
    ...
    packageManifest: (0, _manifestBRvO4Lcw.r)(manifest ?? void 0),
    bundledManifest: params.bundledManifest,
    bundledManifestPath: params.bundledManifestPath
  });
}
function discoverBundleInRoot(params) {
  const bundleFormat = (0, _bundleManifestBwHr1gNY.i)(params.rootDir);
  if (!bundleFormat) return "none";
  const rootRealPath = (0, _bundleManifestBwHr1gNY.u)(params.rootDir, params.realpathCache) ?? void 0;
  const bundleManifest = (0, _bundleManifestBwHr1gNY.a)({
    rootDir: params.rootDir,
    ...(rootRealPath !== void 0 ? { rootRealPath } : {}),
    bundleFormat,
...
function discoverInDirectory(params) {
  ...
  const resolvedDir = (0, _bundleManifestBwHr1gNY.u)(params.dir, params.realpathCache) ?? _nodePath.default.resolve(params.dir);
  ...
  for (const entry of entries) {
    ...
    const fullPathRealPath = (0, _bundleManifestBwHr1gNY.u)(fullPath, params.realpathCache) ?? void 0;
    const manifest = readPackageManifest(fullPath, rejectHardlinks, fullPathRealPath);
    const extensionResolution = (0, _manifestBRvO4Lcw.a)(manifest ?? void 0);
    const extensions = extensionResolution.status === "ok" ? extensionResolution.entries : [];
    const manifestId = resolveIdHintManifestId(fullPath, rejectHardlinks, fullPathRealPath);
    const setupSource = (0, _packageEntryResolutionBysVvIJY.n)({
      packageDir: fullPath,
    ...
    if (extensions.length > 0) {
      const resolvedRuntimeSources = (0, _packageEntryResolutionBysVvIJY.t)({
        packageDir: fullPath,
        ...(fullPathRealPath !== void 0 ? { packageRootRealPath: fullPathRealPath } : {}),
        manifest,
        extensions,
    ...
    const indexFile = [..._manifestBRvO4Lcw.t].map((candidate) => _nodePath.default.join(fullPath, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
...
function discoverFromPath(params) {
  ...
  if (stat.isDirectory()) {
    const rejectHardlinks = params.origin !== "bundled";
    const resolvedRealPath = (0, _bundleManifestBwHr1gNY.u)(resolved, params.realpathCache) ?? void 0;
    ...
    const indexFile = [..._manifestBRvO4Lcw.t].map((candidate) => _nodePath.default.join(resolved, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
    if (indexFile && isExtensionFile(indexFile)) {
      addCandidate({
        candidates: params.candidates,
        diagnostics: params.diagnostics,
