...
var _globalsCJu56k = require("./globals-CJu56k75.js");
var _loggerMz_TsqXj = require("./logger-mz_TsqXj.js");
var _accountIdDhSD7lhD = require("./account-id-DhSD7lhD.js");
var _sessionKeyHxP9B3Or = require("./session-key-hxP9B3Or.js");
var _agentScopeI10se9ty = require("./agent-scope-i10se9ty.js");
var _chatTypeBrFPcj8_ = require("./chat-type-BrFPcj8_.js");
var _bindingScopeBBMk6Cch = require("./binding-scope-BBMk6Cch.js");
var _bindingsC4DZxyB = require("./bindings-C4DZxyB0.js");
var _peerKindMatchOa5DS = require("./peer-kind-match--oa5DS17.js");
...
function normalizeId(value) {
  return (0, _bindingScopeBBMk6Cch.n)(value);
}
function buildAgentSessionKey(params) {
  const channel = normalizeToken(params.channel) || "unknown";
  const peer = params.peer;
  return (0, _sessionKeyHxP9B3Or.i)({
    agentId: params.agentId,
    mainKey: _sessionKeyHxP9B3Or.n,
    channel,
...
function resolveAgentLookupCache(cfg) {
  ...
  for (const agent of listAgents(cfg)) {
    ...
    if (!rawId) continue;
    byNormalizedId.set((0, _sessionKeyHxP9B3Or.c)(rawId), (0, _sessionKeyHxP9B3Or.f)(rawId));
  }
  const next = {
    agentsRef,
    byNormalizedId,
    fallbackDefaultAgentId: (0, _sessionKeyHxP9B3Or.f)((0, _agentScopeI10se9ty.S)(cfg))
  };
...
function pickFirstExistingAgentId(cfg, agentId) {
  ...
  const normalized = (0, _sessionKeyHxP9B3Or.c)(trimmed);
  if (lookup.byNormalizedId.size === 0) return (0, _sessionKeyHxP9B3Or.f)(trimmed);
...
function resolveAccountPatternKey(accountPattern) {
  if (!accountPattern.trim()) return _accountIdDhSD7lhD.t;
  return (0, _accountIdDhSD7lhD.n)(accountPattern);
}
function buildEvaluatedBindingsByChannel(cfg) {
  const byChannel = /* @__PURE__ */new Map();
  let order = 0;
  for (const binding of (0, _bindingsC4DZxyB.n)(cfg)) {
...
function normalizePeerConstraint(peer) {
  ...
  const kind = (0, _chatTypeBrFPcj8_.t)(peer.kind);
...
function normalizeBindingMatch(match) {
  ...
  return {
    ...
    roles: (0, _bindingScopeBBMk6Cch.r)(rawRoles)
  };
}
...
function matchesBindingScope(match, scope) {
  ...
  if (match.peer.state === "valid") {
    if (!scope.peer || !(0, _peerKindMatchOa5DS.t)(match.peer.kind, scope.peer.kind) || scope.peer.id !== match.peer.id) return false;
  }
  if (match.peer.state === "wildcard-kind") {
    if (!scope.peer || !(0, _peerKindMatchOa5DS.t)(match.peer.kind, scope.peer.kind)) return false;
  }
  return (0, _bindingScopeBBMk6Cch.a)(match, scope);
}
function resolveAgentRoute(input) {
  const channel = normalizeToken(input.channel);
  const accountId = (0, _accountIdDhSD7lhD.n)(input.accountId);
  const peer = input.peer ? {
    kind: (0, _chatTypeBrFPcj8_.t)(input.peer.kind) ?? input.peer.kind,
  ...
  const shouldLogDebug = (0, _globalsCJu56k.a)();
  const parentPeer = input.parentPeer ? {
    kind: (0, _chatTypeBrFPcj8_.t)(input.parentPeer.kind) ?? input.parentPeer.kind,
  ...
  const choose = (agentId, matchedBy, sessionOverride) => {
    ...
    const mainSessionKey = (0, _stringCoerceBje8XVt.a)((0, _sessionKeyHxP9B3Or.r)({
      agentId: resolvedAgentId,
      mainKey: _sessionKeyHxP9B3Or.n
    }));
    const route = {
  ...
  if (shouldLogDebug) {
    (0, _loggerMz_TsqXj.t)(`[routing] resolveAgentRoute: channel=${channel} accountId=${accountId} peer=${formatPeer(peer)} guildId=${guildId || "none"} teamId=${teamId || "none"} bindings=${bindings.length}`);
    for (const entry of bindings) (0, _loggerMz_TsqXj.t)(`[routing] binding: agentId=${entry.binding.agentId} accountPattern=${entry.match.accountPattern || "default"} peer=${formatNormalizedPeer(entry.match.peer)} guildId=${entry.match.guildId ?? "none"} teamId=${entry.match.teamId ?? "none"} roles=${entry.match.roles?.length ?? 0}`);
  }
  const baseScope = {
    guildId,
    teamId,
  ...
  for (const tier of tiers) {
    ...
    if (matched) {
      if (shouldLogDebug) (0, _loggerMz_TsqXj.t)(`[routing] match: matchedBy=${tier.matchedBy} agentId=${matched.binding.agentId}`);
      return choose(matched.binding.agentId, tier.matchedBy, matched.binding.session);
    }
  }
  return choose((0, _agentScopeI10se9ty.S)(input.cfg), "default");
}
//#endregion /* v9-9ec62532fe1ca915 */
