"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = formatPrereleaseResolutionError;exports.C = getPackageManifestMetadata;exports.D = planManifestModelCatalogRows;exports.E = planProviderIndexModelCatalogRows;exports.M = isPrereleaseSemverVersion;exports.N = parseRegistryNpmSpec;exports.O = planManifestModelCatalogSuppressions;exports.P = validateRegistryNpmSpec;exports.S = void 0;exports.T = resolvePackageExtensionEntries;exports._ = detectBundleManifestFormat;exports.a = resolvePluginSourceRoots;exports.b = normalizeBundlePathList;exports.c = validatePackageExtensionEntriesForInstall;exports.d = isPathInside;exports.f = safeRealpathSync;exports.h = exports.g = void 0;exports.i = resolvePluginCacheInputs;exports.j = isPrereleaseResolutionAllowed;exports.k = loadOpenClawProviderIndex;exports.l = buildBundledPluginLoadPathAliases;exports.m = void 0;exports.n = buildPluginDependencyStatus;exports.o = tracePluginLifecyclePhase;exports.p = safeStatSync;exports.r = normalizePluginDependencySpecs;exports.s = tracePluginLifecyclePhaseAsync;exports.t = discoverOpenClawPlugins;exports.u = normalizeBundledLookupPath;exports.v = loadBundleManifest;exports.w = loadPluginManifest;exports.x = void 0;exports.y = mergeBundlePathLists;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _utilsCCskKJVV = require("./utils-CCskKJVV.js");
var _boundaryFileRead7ScTVYIc = require("./boundary-file-read-7ScTVYIc.js");
var _bundledDirM7NsR8Gx = require("./bundled-dir-m7NsR8Gx.js");
var _pluginCachePrimitivesBXH3UUqE = require("./plugin-cache-primitives-BXH3UUqE.js");
var _legacyNamesFmU0VYG = require("./legacy-names-FmU0-VYG.js");
var _prototypeKeysRBElZn9Q = require("./prototype-keys-RBElZn9Q.js");
var _normalizeDGCH8jIH = require("./normalize-DGCH8jIH.js");
var _stringNormalizationRJ4BOsjW = require("./string-normalization-rJ4BOsjW.js");
var _clawhubSpecTcp25s = require("./clawhub-spec-tcp25s--.js");
var _parseJsonCompatPvyAOq9e = require("./parse-json-compat-pvyAOq9e.js");
var _manifestCommandAliasesS5HPuXVA = require("./manifest-command-aliases-S5HPuXVA.js");
...
function normalizeSafeKey(value) {
  ...
  return key && !(0, _prototypeKeysRBElZn9Q.t)(key) ? key : "";
}
function normalizeInstall(value) {
  if (!(0, _utilsCCskKJVV.c)(value)) return;
  const clawhubSpec = (0, _stringCoerceBje8XVt.c)(value.clawhubSpec);
  const parsedClawHub = clawhubSpec ? (0, _clawhubSpecTcp25s.t)(clawhubSpec) : null;
...
function normalizeCategories(value) {
  return [...new Set((0, _stringNormalizationRJ4BOsjW.l)(value))];
}
function normalizePreviewCatalog(params) {
  const provider = (0, _normalizeDGCH8jIH.t)({ providers: { [params.providerId]: params.value } }, { ownedProviders: new Set([params.providerId]) })?.providers?.[params.providerId];
  if (!provider) return;
  for (const model of provider.models) model.status ??= "preview";
  return provider;
}
function normalizeOnboardingScopes(value) {
  const scopes = (0, _stringNormalizationRJ4BOsjW.l)(value).filter((scope) => scope === "text-inference" || scope === "image-generation");
...
function normalizeAuthChoice(params) {
  if (!(0, _utilsCCskKJVV.c)(params.value)) return;
...
function normalizeProvider(rawProviderId, value) {
  if (!(0, _utilsCCskKJVV.c)(value)) return;
  const providerId = (0, _normalizeDGCH8jIH.i)(rawProviderId);
  if (!providerId) return;
  const id = (0, _normalizeDGCH8jIH.i)((0, _stringCoerceBje8XVt.c)(value.id) ?? "");
...
function normalizeOpenClawProviderIndex(value) {
  if (!(0, _utilsCCskKJVV.c)(value) || value.version !== OPENCLAW_PROVIDER_INDEX_VERSION) return;
  if (!(0, _utilsCCskKJVV.c)(value.providers)) return;
  const providers = {};
  for (const [rawProviderId, rawProvider] of Object.entries(value.providers)) {
    const providerId = (0, _normalizeDGCH8jIH.i)(rawProviderId);
    if (!providerId || (0, _prototypeKeysRBElZn9Q.t)(providerId)) continue;
...
function planManifestModelCatalogRows(params) {
  const providerFilter = params.providerFilter ? (0, _normalizeDGCH8jIH.i)(params.providerFilter) : void 0;
  const entries = [];
...
function planManifestModelCatalogPluginEntries(params) {
  ...
  return Object.entries(providers).flatMap(([provider, providerCatalog]) => {
    const normalizedProvider = (0, _normalizeDGCH8jIH.i)(provider);
    ...
    return plannedProviders.flatMap((plannedProvider) => {
      const rows = (0, _normalizeDGCH8jIH.n)({
...
function buildOwnedProviderSet(plugin) {
  return new Set((plugin.providers ?? []).map(_normalizeDGCH8jIH.i).filter(Boolean));
}
function buildModelCatalogProviderAliasTargets(plugin) {
  ...
  for (const [rawAlias, alias] of Object.entries(plugin.modelCatalog?.aliases ?? {})) {
    const aliasProvider = (0, _normalizeDGCH8jIH.i)(rawAlias);
    const targetProvider = (0, _normalizeDGCH8jIH.i)(alias.provider);
...
function planManifestModelCatalogSuppressions(params) {
  ...
  for (const plugin of params.registry.plugins) {
    ...
    for (const suppression of plugin.modelCatalog?.suppressions ?? []) {
      const provider = (0, _normalizeDGCH8jIH.i)(suppression.provider);
      ...
      suppressions.push({
        ...
        provider,
        model,
        mergeKey: (0, _normalizeDGCH8jIH.r)(provider, model),
...
function planProviderIndexModelCatalogRows(params) {
  ...
  for (const [providerId, provider] of Object.entries(params.index.providers)) {
    const normalizedProvider = (0, _normalizeDGCH8jIH.i)(providerId);
...
const pluginManifestLoadCache = new _pluginCachePrimitivesBXH3UUqE.t(512);
function normalizeStringListRecord(value) {
  ...
  for (const [key, rawValues] of Object.entries(value)) {
    ...
    if (!providerId || (0, _prototypeKeysRBElZn9Q.t)(providerId)) continue;
    const values = (0, _stringNormalizationRJ4BOsjW.l)(rawValues);
...
function normalizeStringRecord(value) {
  ...
  for (const [rawKey, rawValue] of Object.entries(value)) {
    ...
    if (!key || (0, _prototypeKeysRBElZn9Q.t)(key) || !value) continue;
...
function normalizeMediaUnderstandingCapabilities(value) {
  const values = (0, _stringNormalizationRJ4BOsjW.l)(value).filter((entry) => MEDIA_UNDERSTANDING_CAPABILITIES.has(entry));
  return values.length > 0 ? values : void 0;
}
function normalizeMediaUnderstandingNativeDocumentInputs(value) {
  const values = (0, _stringNormalizationRJ4BOsjW.l)(value).filter((entry) => entry === "pdf");
  return values.length > 0 ? values : void 0;
}
function normalizeMediaUnderstandingProviderMetadata(value) {
  ...
  for (const [rawProviderId, rawMetadata] of Object.entries(value)) {
    ...
    if (!providerId || (0, _prototypeKeysRBElZn9Q.t)(providerId) || !(0, _utilsCCskKJVV.c)(rawMetadata)) continue;
...
function normalizeProviderBaseUrlGuard(value) {
  ...
  const allowedBaseUrls = (0, _stringNormalizationRJ4BOsjW.l)(value.allowedBaseUrls);
...
function normalizeCapabilityProviderAuthSignals(value) {
  ...
  for (const rawSignal of value) {
    if (!(0, _utilsCCskKJVV.c)(rawSignal)) continue;
...
function normalizeCapabilityProviderModeConfigSignal(value) {
  ...
  const allowed = (0, _stringNormalizationRJ4BOsjW.l)(value.allowed);
  const disallowed = (0, _stringNormalizationRJ4BOsjW.l)(value.disallowed);
  const signal = {
    ...(path ? { path } : {}),
...
function normalizeCapabilityProviderConfigSignals(value) {
  ...
  for (const rawSignal of value) {
    ...
    const required = (0, _stringNormalizationRJ4BOsjW.l)(rawSignal.required);
    const requiredAny = (0, _stringNormalizationRJ4BOsjW.l)(rawSignal.requiredAny);
...
function normalizeCapabilityProviderMetadata(value) {
  ...
  for (const [rawProviderId, rawMetadata] of Object.entries(value)) {
    const providerId = (0, _stringCoerceBje8XVt.c)(rawProviderId) ?? "";
    if (!providerId || (0, _prototypeKeysRBElZn9Q.t)(providerId) || !(0, _utilsCCskKJVV.c)(rawMetadata)) continue;
    const aliases = (0, _stringNormalizationRJ4BOsjW.l)(rawMetadata.aliases);
    const authProviders = (0, _stringNormalizationRJ4BOsjW.l)(rawMetadata.authProviders);
...
function normalizeManifestContracts(value) {
  if (!(0, _utilsCCskKJVV.c)(value)) return;
  const embeddedExtensionFactories = (0, _stringNormalizationRJ4BOsjW.l)(value.embeddedExtensionFactories);
  const agentToolResultMiddleware = (0, _stringNormalizationRJ4BOsjW.l)(value.agentToolResultMiddleware);
  const externalAuthProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.externalAuthProviders);
  const memoryEmbeddingProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.memoryEmbeddingProviders);
  const speechProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.speechProviders);
  const realtimeTranscriptionProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.realtimeTranscriptionProviders);
  const realtimeVoiceProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.realtimeVoiceProviders);
  const mediaUnderstandingProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.mediaUnderstandingProviders);
  const documentExtractors = (0, _stringNormalizationRJ4BOsjW.l)(value.documentExtractors);
  const imageGenerationProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.imageGenerationProviders);
  const videoGenerationProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.videoGenerationProviders);
  const musicGenerationProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.musicGenerationProviders);
  const webContentExtractors = (0, _stringNormalizationRJ4BOsjW.l)(value.webContentExtractors);
  const webFetchProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.webFetchProviders);
  const webSearchProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.webSearchProviders);
  const migrationProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.migrationProviders);
  const tools = (0, _stringNormalizationRJ4BOsjW.l)(value.tools);
  const contracts = {
...
function normalizeManifestDangerousConfigFlags(value) {
  ...
  for (const entry of value) {
    if (!(0, _utilsCCskKJVV.c)(entry)) continue;
...
function normalizeManifestConfigContracts(value) {
  if (!(0, _utilsCCskKJVV.c)(value)) return;
  const compatibilityMigrationPaths = (0, _stringNormalizationRJ4BOsjW.l)(value.compatibilityMigrationPaths);
  const compatibilityRuntimePaths = (0, _stringNormalizationRJ4BOsjW.l)(value.compatibilityRuntimePaths);
  const rawSecretInputs = (0, _utilsCCskKJVV.c)(value.secretInputs) ? value.secretInputs : void 0;
...
function normalizeManifestModelSupport(value) {
  if (!(0, _utilsCCskKJVV.c)(value)) return;
  const modelPrefixes = (0, _stringNormalizationRJ4BOsjW.l)(value.modelPrefixes);
  const modelPatterns = (0, _stringNormalizationRJ4BOsjW.l)(value.modelPatterns);
  const modelSupport = {
...
function normalizeManifestModelPricingSource(value) {
  ...
  if (!(0, _utilsCCskKJVV.c)(value)) return;
  const provider = (0, _normalizeDGCH8jIH.i)((0, _stringCoerceBje8XVt.c)(value.provider) ?? "");
  const modelIdTransforms = (0, _stringNormalizationRJ4BOsjW.l)(value.modelIdTransforms).filter((entry) => entry === "version-dots");
  const source = {
    ...(provider ? { provider } : {}),
...
function normalizeManifestModelPricing(value, params) {
  if (!(0, _utilsCCskKJVV.c)(value) || !(0, _utilsCCskKJVV.c)(value.providers)) return;
  const ownedProviders = new Set([...params.ownedProviders].map((provider) => (0, _normalizeDGCH8jIH.i)(provider)).filter(Boolean));
  const providers = {};
...
function normalizeManifestModelIdPrefixRules(value) {
  ...
  for (const rawRule of value) {
    if (!(0, _utilsCCskKJVV.c)(rawRule)) continue;
...
function normalizeManifestModelIdNormalizationProvider(value) {
  if (!(0, _utilsCCskKJVV.c)(value)) return;
  const aliases = {};
  if ((0, _utilsCCskKJVV.c)(value.aliases)) for (const [rawAlias, rawCanonical] of Object.entries(value.aliases)) {
    const alias = (0, _normalizeDGCH8jIH.i)(rawAlias);
    const canonical = (0, _stringCoerceBje8XVt.c)(rawCanonical);
    if (alias && canonical) aliases[alias] = canonical;
  }
  const stripPrefixes = (0, _stringNormalizationRJ4BOsjW.l)(value.stripPrefixes);
...
function normalizeManifestProviderEndpoints(value) {
  ...
  for (const rawEndpoint of value) {
    if (!(0, _utilsCCskKJVV.c)(rawEndpoint)) continue;
    const endpointClass = (0, _stringCoerceBje8XVt.c)(rawEndpoint.endpointClass);
    if (!endpointClass) continue;
    const hosts = (0, _stringNormalizationRJ4BOsjW.l)(rawEndpoint.hosts).map((host) => host.toLowerCase());
    const hostSuffixes = (0, _stringNormalizationRJ4BOsjW.l)(rawEndpoint.hostSuffixes).map((host) => host.toLowerCase());
    const baseUrls = (0, _stringNormalizationRJ4BOsjW.l)(rawEndpoint.baseUrls);
...
function normalizeManifestProviderRequestProvider(value) {
  ...
  const supportsStreamingUsage = (0, _utilsCCskKJVV.c)(value.openAICompletions) ? value.openAICompletions.supportsStreamingUsage : void 0;
...
function normalizeManifestActivation(value) {
  if (!(0, _utilsCCskKJVV.c)(value)) return;
  const onProviders = (0, _stringNormalizationRJ4BOsjW.l)(value.onProviders);
  const onAgentHarnesses = (0, _stringNormalizationRJ4BOsjW.l)(value.onAgentHarnesses);
  const onCommands = (0, _stringNormalizationRJ4BOsjW.l)(value.onCommands);
  const onChannels = (0, _stringNormalizationRJ4BOsjW.l)(value.onChannels);
  const onRoutes = (0, _stringNormalizationRJ4BOsjW.l)(value.onRoutes);
  const onConfigPaths = (0, _stringNormalizationRJ4BOsjW.l)(value.onConfigPaths);
  const onStartup = typeof value.onStartup === "boolean" ? value.onStartup : void 0;
  const onCapabilities = (0, _stringNormalizationRJ4BOsjW.l)(value.onCapabilities).filter((capability) => capability === "provider" || capability === "channel" || capability === "tool" || capability === "hook");
  const activation = {
...
function normalizeManifestSetupProviders(value) {
  ...
  for (const entry of value) {
    ...
    if (!id) continue;
    const authMethods = (0, _stringNormalizationRJ4BOsjW.l)(entry.authMethods);
    const envVars = (0, _stringNormalizationRJ4BOsjW.l)(entry.envVars);
...
function normalizeManifestSetupProviderAuthEvidence(value) {
  ...
  const normalized = [];
  for (const entry of value) {
    if (!(0, _utilsCCskKJVV.c)(entry) || entry.type !== "local-file-with-env") continue;
    ...
    const fallbackPaths = (0, _stringNormalizationRJ4BOsjW.l)(entry.fallbackPaths);
    if (!fileEnvVar && fallbackPaths.length === 0) continue;
    const requiresAnyEnv = (0, _stringNormalizationRJ4BOsjW.l)(entry.requiresAnyEnv);
    const requiresAllEnv = (0, _stringNormalizationRJ4BOsjW.l)(entry.requiresAllEnv);
...
function normalizeManifestSetup(value) {
  ...
  const cliBackends = (0, _stringNormalizationRJ4BOsjW.l)(value.cliBackends);
  const configMigrations = (0, _stringNormalizationRJ4BOsjW.l)(value.configMigrations);
...
function normalizeProviderAuthChoices(value) {
  ...
  for (const entry of value) {
    ...
    const deprecatedChoiceIds = (0, _stringNormalizationRJ4BOsjW.l)(entry.deprecatedChoiceIds);
    ...
    const onboardingScopes = (0, _stringNormalizationRJ4BOsjW.l)(entry.onboardingScopes).filter((scope) => scope === "text-inference" || scope === "image-generation");
    normalized.push({
      provider,
      method,
      choiceId,
...
function normalizeChannelConfigs(value) {
  ...
  for (const [key, rawEntry] of Object.entries(value)) {
    ...
    if (!channelId || (0, _prototypeKeysRBElZn9Q.t)(channelId) || !(0, _utilsCCskKJVV.c)(rawEntry)) continue;
    const schema = (0, _utilsCCskKJVV.c)(rawEntry.schema) ? rawEntry.schema : null;
    if (!schema) continue;
    const uiHints = (0, _utilsCCskKJVV.c)(rawEntry.uiHints) ? rawEntry.uiHints : void 0;
    const runtime = (0, _utilsCCskKJVV.c)(rawEntry.runtime) && typeof rawEntry.runtime.safeParse === "function" ? rawEntry.runtime : void 0;
    const label = (0, _stringCoerceBje8XVt.c)(rawEntry.label) ?? "";
    const description = (0, _stringCoerceBje8XVt.c)(rawEntry.description) ?? "";
    const preferOver = (0, _stringNormalizationRJ4BOsjW.l)(rawEntry.preferOver);
...
function buildPluginManifestLoadCacheKey(params) {
  return (0, _pluginCachePrimitivesBXH3UUqE.r)([
  [
...
function loadPluginManifest(rootDir, rejectHardlinks = true, rootRealPath) {
  ...
  const opened = (0, _boundaryFileRead7ScTVYIc.i)({
  ...
  if (!opened.ok) return (0, _boundaryFileRead7ScTVYIc.n)(opened, {
    path: () => ({
      ok: false,
  ...
  let raw;
  try {
    raw = (0, _parseJsonCompatPvyAOq9e.t)(_nodeFs.default.readFileSync(opened.fd, "utf-8"));
  } catch (err) {
    return cacheResult({
      ok: false,
  ...
  if (!(0, _utilsCCskKJVV.c)(raw)) return cacheResult({
    ok: false,
  ...
  const configSchema = (0, _utilsCCskKJVV.c)(raw.configSchema) ? raw.configSchema : null;
  ...
  const legacyPluginIds = (0, _stringNormalizationRJ4BOsjW.l)(raw.legacyPluginIds);
  const autoEnableWhenConfiguredProviders = (0, _stringNormalizationRJ4BOsjW.l)(raw.autoEnableWhenConfiguredProviders);
  ...
  const channels = (0, _stringNormalizationRJ4BOsjW.l)(raw.channels);
  const providers = (0, _stringNormalizationRJ4BOsjW.l)(raw.providers);
  const providerDiscoveryEntry = (0, _stringCoerceBje8XVt.c)(raw.providerDiscoveryEntry);
  const modelSupport = normalizeManifestModelSupport(raw.modelSupport);
  const modelCatalog = (0, _normalizeDGCH8jIH.t)(raw.modelCatalog, { ownedProviders: new Set(providers) });
  ...
  const cliBackends = (0, _stringNormalizationRJ4BOsjW.l)(raw.cliBackends);
  const syntheticAuthRefs = (0, _stringNormalizationRJ4BOsjW.l)(raw.syntheticAuthRefs);
  const nonSecretAuthMarkers = (0, _stringNormalizationRJ4BOsjW.l)(raw.nonSecretAuthMarkers);
  const commandAliases = (0, _manifestCommandAliasesS5HPuXVA.t)(raw.commandAliases);
  ...
  const skills = (0, _stringNormalizationRJ4BOsjW.l)(raw.skills);
  ...
  let uiHints;
  if ((0, _utilsCCskKJVV.c)(raw.uiHints)) uiHints = raw.uiHints;
  return cacheResult({
    ok: true,
    manifest: {
      id,
...
function getPackageManifestMetadata(manifest) {
  if (!manifest) return;
  return manifest[_legacyNamesFmU0VYG.n];
}
...
function hasInlineCapabilityValue(value) {
  ...
  if ((0, _utilsCCskKJVV.c)(value)) return Object.keys(value).length > 0;
  return value === true;
}
...
function loadBundleManifestFile(params) {
  ...
  try {
    ...
    if (!(0, _utilsCCskKJVV.c)(raw)) return {
      ok: false,
      error: "plugin manifest must be an object",
      manifestPath
    };
...
function loadBundleManifest(params) {
  ...
  const interfaceRecord = (0, _utilsCCskKJVV.c)(raw.interface) ? raw.interface : void 0;
  const name = (0, _stringCoerceBje8XVt.c)(raw.name);
...
function isPathInside(baseDir, targetPath) {
  return (0, _boundaryFileRead7ScTVYIc.d)(baseDir, targetPath);
}
...
function normalizePackageManifestStringList(value) {
  if (!Array.isArray(value)) return [];
  return value.map((entry) => (0, _stringCoerceBje8XVt.c)(entry) ?? "").filter(Boolean);
}
...
async function validatePackageExtensionEntry(params) {
  ...
  try {
    if (!(await (0, _boundaryFileRead7ScTVYIc.o)({
      absolutePath,
  ...
  const opened = await (0, _boundaryFileRead7ScTVYIc.r)({
    absolutePath,
    rootPath: params.packageDir,
    boundaryLabel: "plugin package directory"
  });
...
async function validatePackageExtensionEntriesForInstall(params) {
  ...
  if (setupEntry) {
    ...
    if (!foundBuiltSetupEntry) return {
      ok: false,
...
function resolveExistingPackageEntrySource(params) {
  const source = _nodePath.default.resolve(params.packageDir, params.entryPath);
  if (!_nodeFs.default.existsSync(source)) return null;
...
function resolvePluginSourceRoots(params) {
  ...
  const workspaceRoot = params.workspaceDir ? (0, _utilsCCskKJVV.p)(params.workspaceDir, env) : void 0;
  return {
    stock: (0, _bundledDirM7NsR8Gx.n)(env),
    global: _nodePath.default.join((0, _utilsCCskKJVV.d)(env), "extensions"),
...
function resolvePluginCacheInputs(params) {
  ...
  return {
    ...
    loadPaths: (params.loadPaths ?? []).filter((entry) => typeof entry === "string").map((entry) => entry.trim()).filter(Boolean).map((entry) => (0, _utilsCCskKJVV.p)(entry, env))
  };
}
//#endregion
...
function collectInstalledPluginRecordPaths(installRecords, env) {
  ...
  for (const record of Object.values(installRecords ?? {})) {
    ...
    if (!rawPath) continue;
    const resolved = (0, _utilsCCskKJVV.p)(rawPath, env);
...
function discoverFromPath(params) {
  const resolved = (0, _utilsCCskKJVV.p)(params.rawPath, params.env);
...
function discoverOpenClawPlugins(params) {
  ...
  const workspaceRoot = workspaceDir ? (0, _utilsCCskKJVV.p)(workspaceDir, env) : void 0;
  ...
  const scopedResult = tracePluginLifecyclePhase("discovery scan", () => {
    ...
    for (const extraPath of extra) {
      ...
      const bundledAlias = resolvePackagedBundledLoadPathAlias({
        ...
        loadPath: (0, _utilsCCskKJVV.p)(trimmed, env)
      });
      if (bundledAlias) {
  ...
  const sharedResult = tracePluginLifecyclePhase("discovery scan", () => {
    ...
    const sourceCheckoutDependencyDiagnostic = (0, _bundledDirM7NsR8Gx.r)(env);
