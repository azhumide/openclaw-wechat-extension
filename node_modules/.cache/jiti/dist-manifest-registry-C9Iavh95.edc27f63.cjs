"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveMemorySlotDecision;exports.c = readPersistedInstalledPluginIndexInstallRecords;exports.i = resolveEffectivePluginActivationState;exports.l = readPersistedInstalledPluginIndexInstallRecordsSync;exports.n = void 0;exports.o = loadInstalledPluginIndexInstallRecords;exports.r = normalizePluginsConfigWithResolver;exports.s = loadInstalledPluginIndexInstallRecordsSync;exports.t = loadPluginManifestRegistry;exports.u = resolveInstalledPluginIndexStorePath;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
...
var _versionEw9InAD = require("./version-Ew9InAD4.js");
var _configNormalizationSharedBkjbUp = require("./config-normalization-shared-BkjbUp88.js");
...
var _jsonFilesBvzDldMA = require("./json-files-BvzDldMA.js");
var _minHostVersionDBJHE8w = require("./min-host-version-DBJ-HE8w.js");
...
function resolveInstalledPluginIndexStorePath(options = {}) {
  ...
  const stateDir = options.stateDir ?? (0, _pathsC1_Y0cDn.v)(env);
...
async function readPersistedInstalledPluginIndexInstallRecords(options = {}) {
  return extractPluginInstallRecordsFromPersistedInstalledPluginIndex(await (0, _jsonFilesBvzDldMA.i)(resolveInstalledPluginIndexStorePath(options)));
}
function readPersistedInstalledPluginIndexInstallRecordsSync(options = {}) {
  return extractPluginInstallRecordsFromPersistedInstalledPluginIndex((0, _jsonFilesBvzDldMA.a)(resolveInstalledPluginIndexStorePath(options)));
}
...
function normalizePluginsConfigWithResolver(config, normalizePluginId = _configNormalizationSharedBkjbUp.n) {
  return (0, _configNormalizationSharedBkjbUp.i)(config, normalizePluginId);
}
function resolvePluginActivationState(params) {
  return (0, _configNormalizationSharedBkjbUp.l)((0, _configNormalizationSharedBkjbUp.c)({
    ...params,
    activationSource: {
...
const hasExplicitPluginConfig = exports.n = _configNormalizationSharedBkjbUp.t;
const isBundledChannelEnabledByChannelConfig = _configNormalizationSharedBkjbUp.r;
function resolveEffectivePluginActivationState(params) {
  return resolvePluginActivationState(params);
}
function resolveMemorySlotDecision(params) {
  return (0, _configNormalizationSharedBkjbUp.s)(params);
}
//#endregion
...
function normalizePreferredPluginIds(raw) {
  return (0, _stringNormalizationRJ4BOsjW.a)(raw);
}
...
function mergePackageChannelMetaIntoChannelConfigs(params) {
  ...
  if (!channelId || (0, _prototypeKeysRBElZn9Q.t)(channelId) || !params.channelConfigs || !Object.prototype.hasOwnProperty.call(params.channelConfigs, channelId)) return params.channelConfigs;
  const existing = params.channelConfigs[channelId];
  if (!existing) return params.channelConfigs;
  const label = existing.label ?? (0, _stringCoerceBje8XVt.c)(params.packageChannel?.label) ?? "";
  const description = existing.description ?? (0, _stringCoerceBje8XVt.c)(params.packageChannel?.blurb) ?? "";
  ...
  for (const [key, value] of Object.entries(params.channelConfigs)) if (!(0, _prototypeKeysRBElZn9Q.t)(key)) merged[key] = value;
  merged[channelId] = {
    ...existing,
    ...(label ? { label } : {}),
...
function buildRecord(params) {
  ...
  return {
    ...
    name: (0, _stringCoerceBje8XVt.c)(params.manifest.name) ?? params.candidate.packageName,
    description: (0, _stringCoerceBje8XVt.c)(params.manifest.description) ?? params.candidate.packageDescription,
    version: (0, _stringCoerceBje8XVt.c)(params.manifest.version) ?? params.candidate.packageVersion,
...
function buildBundleRecord(params) {
  return {
    id: params.manifest.id,
    name: (0, _stringCoerceBje8XVt.c)(params.manifest.name) ?? params.candidate.idHint,
    description: (0, _stringCoerceBje8XVt.c)(params.manifest.description),
    version: (0, _stringCoerceBje8XVt.c)(params.manifest.version),
    packageName: params.candidate.packageName,
    packageVersion: params.candidate.packageVersion,
    packageDescription: params.candidate.packageDescription,
    packageManifest: params.candidate.packageManifest,
...
function pushProviderAuthEnvVarsCompatDiagnostic(params) {
  ...
  params.diagnostics.push({
    level: "warn",
    pluginId: (0, _ansiDqm1lzVL.t)(params.record.id),
    source: (0, _ansiDqm1lzVL.t)(params.record.manifestPath),
    message: `providerAuthEnvVars is deprecated compatibility metadata for provider env-var lookup; mirror ${providerIds.map(_ansiDqm1lzVL.t).join(", ")} env vars to setup.providers[].envVars before the deprecation window closes`
  });
}
function pushNonBundledChannelConfigDescriptorDiagnostic(params) {
  ...
  const safeMissingChannels = missingChannels.map(_ansiDqm1lzVL.t);
  params.diagnostics.push({
    level: "warn",
    pluginId: (0, _ansiDqm1lzVL.t)(params.record.id),
    source: (0, _ansiDqm1lzVL.t)(params.record.manifestPath),
...
function matchesInstalledPluginRecord(params) {
  ...
  const candidateSource = (0, _utilsCCskKJVV.p)(params.candidate.source, params.env);
  const trackedPaths = [record.installPath, record.sourcePath].filter((entry) => typeof entry === "string" && entry.trim().length > 0).map((entry) => (0, _utilsCCskKJVV.p)(entry, params.env));
  if (trackedPaths.length === 0) return false;
  return trackedPaths.some((trackedPath) => {
    return candidateSource === trackedPath || (0, _discoveryB19Xdk1_.d)(trackedPath, candidateSource);
  });
}
...
function loadPluginManifestRegistry(params = {}) {
  ...
  const currentHostVersion = (0, _versionEw9InAD.o)(env);
  for (const candidate of candidates) {
    ...
    } : isBundleRecord && candidate.bundleFormat ? (0, _discoveryB19Xdk1_.v)({
      rootDir: candidate.rootDir,
      bundleFormat: candidate.bundleFormat,
      rejectHardlinks
    }) : (0, _discoveryB19Xdk1_.w)(candidate.rootDir, rejectHardlinks);
    if (!manifestRes.ok) {
      diagnostics.push({
        level: "error",
    ...
    if (candidate.origin !== "bundled") {
      ...
      const minHostVersionCheck = (0, _minHostVersionDBJHE8w.t)({
    ...
    if (existing) {
      ...
      if ((() => {
        ...
        const existingReal = (0, _discoveryB19Xdk1_.f)(existing.candidate.rootDir, realpathCache);
        const candidateReal = (0, _discoveryB19Xdk1_.f)(candidate.rootDir, realpathCache);
