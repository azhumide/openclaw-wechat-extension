...
var _workspaceDefaultBLwEksn = require("./workspace-default-B-lwEksn.js");
var _modelInputBhAB75wV = require("./model-input-BhAB75wV.js");
var _agentFilterDYKqj5SF = require("./agent-filter-DYKqj5SF.js");
...
function warnMultipleDefaultAgents() {
  Promise.resolve().then(() => jitiImport("./subsystem-CH23unOR.js").then((m) => _interopRequireWildcard(m))).then(({ createSubsystemLogger }) => {
...
function listAgentIds(cfg) {
  ...
  if (agents.length === 0) return [_sessionKeyBd0xquXF.t];
  const seen = /* @__PURE__ */new Set();
  const ids = [];
  for (const entry of agents) {
    const id = (0, _sessionKeyBd0xquXF.c)(entry?.id);
    if (seen.has(id)) continue;
    seen.add(id);
    ids.push(id);
  }
  return ids.length > 0 ? ids : [_sessionKeyBd0xquXF.t];
}
function resolveDefaultAgentId(cfg) {
  ...
  return (0, _sessionKeyBd0xquXF.c)(chosen || "main");
}
function resolveAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyBd0xquXF.c)(agentId);
  return listAgentEntries(cfg).find((entry) => (0, _sessionKeyBd0xquXF.c)(entry.id) === id);
}
function resolveAgentConfig(cfg, agentId) {
  const entry = resolveAgentEntry(cfg, (0, _sessionKeyBd0xquXF.c)(agentId));
  if (!entry) return;
...
function resolveAgentWorkspaceDir(cfg, agentId, env = process.env) {
  ...
  if (configured) return stripNullBytes$1((0, _utilsDvkbxKCZ.p)(configured, env));
  const defaultAgentId = resolveDefaultAgentId(cfg);
  const fallback = cfg.agents?.defaults?.workspace?.trim();
  if (id === defaultAgentId) {
    if (fallback) return stripNullBytes$1((0, _utilsDvkbxKCZ.p)(fallback, env));
    return stripNullBytes$1((0, _workspaceDefaultBLwEksn.n)(env));
  }
  if (fallback) return stripNullBytes$1(_nodePath.default.join((0, _utilsDvkbxKCZ.p)(fallback, env), id));
  const stateDir = (0, _pathsB2cMKWd._)(env);
...
function resolveAgentDir(cfg, agentId, env = process.env) {
  ...
  if (configured) return (0, _utilsDvkbxKCZ.p)(configured, env);
  const root = (0, _pathsB2cMKWd._)(env);
...
function resolveSessionAgentIds(params) {
  ...
  const explicitAgentId = explicitAgentIdRaw ? (0, _sessionKeyBd0xquXF.c)(explicitAgentIdRaw) : null;
  const sessionKey = params.sessionKey?.trim();
  const normalizedSessionKey = sessionKey ? (0, _stringCoerceBje8XVt.a)(sessionKey) : void 0;
  const parsed = normalizedSessionKey ? (0, _sessionKeyUtilsBKB1OWzs.o)(normalizedSessionKey) : null;
  return {
    defaultAgentId,
    sessionAgentId: explicitAgentId ?? (parsed?.agentId ? (0, _sessionKeyBd0xquXF.c)(parsed.agentId) : defaultAgentId)
  };
}
...
function resolveAgentSkillsFilter(cfg, agentId) {
  return (0, _agentFilterDYKqj5SF.t)(cfg, agentId);
}
...
function findMutableAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyBd0xquXF.c)(agentId);
  return cfg.agents?.list?.find((entry) => (0, _sessionKeyBd0xquXF.c)(entry?.id) === id);
}
...
function resolveFallbackAgentId(params) {
  ...
  if (explicitAgentId) return (0, _sessionKeyBd0xquXF.c)(explicitAgentId);
  return (0, _sessionKeyBd0xquXF.u)(params.sessionKey);
}
...
function hasConfiguredModelFallbacks(params) {
  ...
  const defaultFallbacks = (0, _modelInputBhAB75wV.t)(params.cfg?.agents?.defaults?.model);
...
function normalizePathForComparison(input) {
  const resolved = _nodePath.default.resolve(stripNullBytes((0, _utilsDvkbxKCZ.p)(input)));
